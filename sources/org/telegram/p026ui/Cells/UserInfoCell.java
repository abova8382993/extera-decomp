...
public class UserInfoCell extends View implements NotificationCenter.NotificationCenterDelegate {
    ...
    public static /* synthetic */ void m6328$r8$lambda$YM6n_lhdmcTOaVmJNI5iXpaJR4(String str) {
    }
    ...
    private Row addRow(CharSequence charSequence, CharSequence charSequence2, boolean z) {
        if (!this.rows.isEmpty()) {
            this.height += AndroidUtilities.m1081dp(7.0f);
        }
        Row row = new Row(charSequence, charSequence2, z);
        this.rows.add(row);
        this.height += AndroidUtilities.m1081dp(14.0f);
        this.rowsKeysWidth = Math.max(this.rowsKeysWidth, row.key.getCurrentWidth());
        this.rowsValuesWidth = Math.max(this.rowsValuesWidth, row.value.getCurrentWidth() + (z ? AndroidUtilities.m1081dp(38.0f) : 0));
        return row;
    }
    ...
    public UserInfoCell(Context context, int i, Theme.ResourcesProvider resourcesProvider) {
        ...
        avatarsDrawable.width = AndroidUtilities.m1081dp(50.0f);
        avatarsDrawable.height = AndroidUtilities.m1081dp(13.0f);
        avatarsDrawable.drawStoriesCircle = false;
        avatarsDrawable.setSize(AndroidUtilities.m1081dp(13.0f));
        avatarsDrawable.setAvatarsTextSize(AndroidUtilities.m1081dp(18.0f));
        Drawable drawableMutate = context.getResources().getDrawable(C2702R.drawable.msg_mini_forumarrow).mutate();
    ...
    public void set(long j, TLRPC.PeerSettings peerSettings) {
        ...
        this.title = text;
        this.height += text.getHeight() + AndroidUtilities.m1081dp(3.0f);
        Text text2 = new Text(LocaleController.getString(ContactsController.getInstance(this.currentAccount).isContact(j) ? C2702R.string.ContactInfoIsContact : C2702R.string.ContactInfoIsNotContact), 14.0f);
        this.subtitle = text2;
        this.height += text2.getHeight() + AndroidUtilities.m1081dp(11.0f);
        if (peerSettings != null && peerSettings.phone_country != null) {
            addRow(LocaleController.getString(C2702R.string.ContactInfoPhone), LocaleController.getCountryWithFlag(peerSettings.phone_country, 12, C2702R.string.ContactInfoPhoneFragment), false);
        }
        if (peerSettings != null && peerSettings.registration_month != null) {
            if (!this.timestampSent) {
                RegDateController.getInstance(this.currentAccount).addRegistrationDate(j, getTimestamp(peerSettings.registration_month), new Consumer() { // from class: org.telegram.ui.Cells.UserInfoCell$$ExternalSyntheticLambda0
                    ...
                    public final void m939v(Object obj) {
                        UserInfoCell.m6328$r8$lambda$YM6n_lhdmcTOaVmJNI5iXpaJR4((String) obj);
                    }
            ...
            addRow(LocaleController.getString(C2702R.string.ContactInfoRegistration), displayDate(peerSettings.registration_month), false);
        }
        ...
        if (userFull != null) {
            ...
            if (iMax > 0) {
                this.groupsRow = addRow(LocaleController.getString(C2702R.string.ContactInfoCommonGroups), LocaleController.formatPluralString("Groups", iMax, new Object[0]), true);
        ...
        this.rowsWidth = this.rowsKeysWidth + AndroidUtilities.m1081dp(7.66f) + this.rowsValuesWidth;
        if (user != null && !user.verified && !UserObject.isService(user.f1775id)) {
            ...
            if (badgesController.isDeveloper(user)) {
                ...
                if (badge != null) {
                    ...
                    spannableStringBuilder.append((CharSequence) AndroidUtilities.replaceTags(badge.getText() != null ? badge.getText() : LocaleController.getString(C2702R.string.DeveloperCompact)));
                    ...
                    this.height += AndroidUtilities.m1081dp(12.0f) + this.footer.getHeight() + AndroidUtilities.m1081dp(15.33f);
                } else {
                    this.footer = null;
                    this.height += AndroidUtilities.m1081dp(14.0f);
                }
                ...
                ColoredImageSpan coloredImageSpan = new ColoredImageSpan(C2702R.drawable.filled_info);
                coloredImageSpan.setScale(0.55f, -0.55f);
                coloredImageSpan.translate(AndroidUtilities.m1081dp(1.0f), AndroidUtilities.m1081dp(-1.0f));
                spannableStringBuilder2.setSpan(coloredImageSpan, 0, 1, 33);
                spannableStringBuilder2.append((CharSequence) LocaleController.getString(C2702R.string.ContactInfoNotVerified));
        ...
        this.width = fMax3;
        this.width = Math.min(fMax3 + AndroidUtilities.m1081dp(32.0f), i);
    }
    ...
    protected void onMeasure(int i, int i2) {
        setMeasuredDimension(View.MeasureSpec.getSize(i), Math.max(0, ((int) this.height) + AndroidUtilities.m1081dp(16.0f)));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawRoundRect(this.fullBounds, AndroidUtilities.m1081dp(16.0f), AndroidUtilities.m1081dp(16.0f), Theme.getThemePaint("paintChatActionBackground", this.resourcesProvider));
        if (hasGradientService()) {
            canvas.drawRoundRect(this.fullBounds, AndroidUtilities.m1081dp(16.0f), AndroidUtilities.m1081dp(16.0f), Theme.getThemePaint("paintChatActionBackgroundDarken", this.resourcesProvider));
        }
        canvas.translate(0.0f, (getHeight() - this.height) / 2.0f);
        float height = ((getHeight() - this.height) / 2.0f) + 0.0f;
        float f2 = 14.0f;
        canvas.translate(0.0f, AndroidUtilities.m1081dp(14.0f));
        this.title.ellipsize(this.width - AndroidUtilities.m1081dp(32.0f)).draw(canvas, width - (this.title.getWidth() / 2.0f), this.title.getHeight() / 2.0f, this.textColor, 1.0f);
        canvas.translate(0.0f, this.title.getHeight() + AndroidUtilities.m1081dp(3.0f));
        float fM1081dp = height + AndroidUtilities.m1081dp(14.0f) + this.title.getHeight() + AndroidUtilities.m1081dp(3.0f);
        this.subtitle.ellipsize(this.width - AndroidUtilities.m1081dp(32.0f)).draw(canvas, width - (this.subtitle.getWidth() / 2.0f), this.subtitle.getHeight() / 2.0f, this.textColor, 0.7f);
        canvas.translate(0.0f, this.subtitle.getHeight() + AndroidUtilities.m1081dp(11.0f));
        float height2 = fM1081dp + this.subtitle.getHeight() + AndroidUtilities.m1081dp(11.0f);
        float f3 = width - (this.rowsWidth / 2.0f);
        int i = 0;
        while (i < this.rows.size()) {
            if (i > 0) {
                canvas.translate(0.0f, AndroidUtilities.m1081dp(7.0f));
                height2 += AndroidUtilities.m1081dp(7.0f);
            }
            canvas.save();
            Row row = (Row) this.rows.get(i);
            float currentWidth = (this.rowsKeysWidth + f3) - row.key.getCurrentWidth();
            float fM1081dp2 = this.rowsKeysWidth + f3 + AndroidUtilities.m1081dp(7.66f);
            float f4 = f;
            int i2 = i;
            float f5 = f2;
            row.key.ellipsize((fM1081dp2 - currentWidth) - AndroidUtilities.m1081dp(7.66f)).draw(canvas, currentWidth, row.key.getHeight() / f4, this.textColor, 0.7f);
            row.bounds.set(fM1081dp2, height2, fM1081dp2 + row.value.getCurrentWidth() + (row.avatars ? AndroidUtilities.m1081dp(5.0f) + (this.groupsArrow.getIntrinsicWidth() * 0.8f) + this.groupsAvatars.getMaxX() : 0.0f), row.value.getHeight() + height2);
            if (this.groupsRow == row) {
                this.groupsBounds.set(row.bounds);
                this.groupsBounds.inset(-AndroidUtilities.m1081dp(4.0f), -AndroidUtilities.m1081dp(f4));
            ...
            row.value.ellipsize((((this.width / f4) + width) - AndroidUtilities.m1081dp(8.0f)) - fM1081dp2).draw(canvas, fM1081dp2, row.value.getHeight() / f4, this.textColor, 1.0f);
            if (row.avatars) {
                canvas.save();
                canvas.translate(fM1081dp2 + row.value.getCurrentWidth() + AndroidUtilities.m1081dp(4.0f), AndroidUtilities.m1081dp(1.0f));
                this.groupsAvatars.onDraw(canvas);
                canvas.translate(this.groupsAvatars.getMaxX() + AndroidUtilities.m1081dp(1.0f), AndroidUtilities.m1081dp(13.0f) / f4);
            ...
            canvas.restore();
            canvas.translate(0.0f, AndroidUtilities.m1081dp(f5));
            height2 += AndroidUtilities.m1081dp(f5);
            i = i2 + 1;
            f2 = f5;
            f = f4;
        }
        float f6 = f;
        if (this.footer != null) {
            canvas.translate(0.0f, AndroidUtilities.m1081dp(12.0f));
            if (this.footer.isMultiline()) {
                Text text = this.footer;
                text.draw(canvas, width - (text.getWidth() / f6), 0.0f, this.textColor, 0.7f);
            } else {
                this.footer.ellipsize(this.width - AndroidUtilities.m1081dp(32.0f)).draw(canvas, width - (this.footer.getWidth() / f6), this.footer.getHeight() / f6, this.textColor, 0.7f);
            }
        }
        canvas.restore();
    }
