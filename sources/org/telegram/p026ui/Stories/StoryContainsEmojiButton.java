...
public class StoryContainsEmojiButton extends View {
    ...
    public StoryContainsEmojiButton(Context context, int i, TLObject tLObject, Object obj, boolean z, ArrayList arrayList, Theme.ResourcesProvider resourcesProvider) {
        ...
        setPadding(AndroidUtilities.m1081dp(13.0f), AndroidUtilities.m1081dp(8.0f), AndroidUtilities.m1081dp(13.0f), AndroidUtilities.m1081dp(8.0f));
    ...
    protected void onMeasure(int i, int i2) {
        ...
        int iM1081dp = AndroidUtilities.m1081dp(29.0f);
        StaticLayout staticLayout = this.layout;
        setMeasuredDimension(z ? View.MeasureSpec.getSize(i) : getMinimumWidth(), paddingTop + AndroidUtilities.lerp(iM1081dp, staticLayout == null ? AndroidUtilities.m1081dp(29.0f) : staticLayout.getHeight(), this.loadT) + getPaddingBottom());
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (f < 1.0f) {
            ...
            float paddingTop2 = getPaddingTop() + AndroidUtilities.m1081dp(12.0f);
            Path.Direction direction = Path.Direction.CW;
            path.addRect(paddingLeft, paddingTop, measuredWidth, paddingTop2, direction);
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop() + AndroidUtilities.m1081dp(16.0f), getPaddingLeft() + (((getMeasuredWidth() - getPaddingRight()) - getPaddingLeft()) * 0.46f), getPaddingTop() + AndroidUtilities.m1081dp(28.0f), direction);
    ...
    public void load(final int i, boolean z, TLObject tLObject, final ArrayList arrayList, final Object obj) {
        ...
        if (z) {
            ...
            final RequestDelegate requestDelegate = new RequestDelegate() { // from class: org.telegram.ui.Stories.StoryContainsEmojiButton$$ExternalSyntheticLambda1
        ...
        if (tLObject instanceof Vector) {
            ...
            if (this.inputSets != null && arrayList != null && !arrayList.isEmpty()) {
                for (int i3 = 0; i3 < arrayList.size(); i3++) {
                    ...
                    long j = inputStickerSet.f1635id;
                    int i4 = 0;
                    while (true) {
                        if (i4 >= this.inputSets.size()) {
                            this.inputSets.add(inputStickerSet);
                            break;
                        } else if (((TLRPC.InputStickerSet) this.inputSets.get(i4)).f1635id == j) {
                            break;
                        } else {
                            i4++;
                        }
    ...
    private void set(TLRPC.TL_messages_stickerSet tL_messages_stickerSet) {
        ...
        while (true) {
            if (i >= arrayList.size()) {
                ...
                if (((TLRPC.Document) arrayList.get(i)).f1618id == tL_messages_stickerSet.set.thumb_document_id) {
        ...
        if (z && this.stickers) {
            string = LocaleController.getString(C2702R.string.StoryContainsStickersEmojiFrom);
        } else if (z) {
            string = LocaleController.getString(C2702R.string.StoryContainsEmojiFrom);
        } else {
            string = LocaleController.getString(C2702R.string.StoryContainsStickersFrom);
        }
    ...
    private void set(TLRPC.StickerSetCovered stickerSetCovered) {
        ...
        if (document == null && (stickerSetCovered instanceof TLRPC.TL_stickerSetFullCovered)) {
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                if (((TLRPC.Document) arrayList.get(i)).f1618id == stickerSetCovered.set.thumb_document_id) {
                    document = (TLRPC.Document) arrayList.get(i);
                }
            }
            if (document == null && !arrayList.isEmpty()) {
    ...
    private void animateLoad(boolean z) {
        ...
        if (z) {
            ...
            final boolean z2 = this.layout == null || Math.abs(getMeasuredHeight() - ((getPaddingTop() + this.layout.getHeight()) + getPaddingBottom())) > AndroidUtilities.m1081dp(3.0f);
