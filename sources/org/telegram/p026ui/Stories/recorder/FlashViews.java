...
public class FlashViews {
    ...
    public void flash(final Utilities.Callback callback) {
        ...
        flashTo(1.0f, 320L, new Runnable() { // from class: org.telegram.ui.Stories.recorder.FlashViews$$ExternalSyntheticLambda4
    ...
    public /* synthetic */ void lambda$flash$2(Utilities.Callback callback) {
        callback.run(new Utilities.Callback() { // from class: org.telegram.ui.Stories.recorder.FlashViews$$ExternalSyntheticLambda6
    ...
    private void flashTo(final float f, long j, final Runnable runnable) {
        ...
        valueAnimatorOfFloat.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() { // from class: org.telegram.ui.Stories.recorder.FlashViews$$ExternalSyntheticLambda3
    ...
    public void invalidateGradient() {
        ...
        if (Build.VERSION.SDK_INT >= 29) {
            FlashViews$$ExternalSyntheticApiModelOutline1.m1281m();
            int i = this.lastWidth;
            int i2 = this.lastHeight;
            ColorSpace.Named named = ColorSpace.Named.EXTENDED_SRGB;
            this.gradient = FlashViews$$ExternalSyntheticApiModelOutline0.m1280m(i * 0.5f, i2 * 0.4f, (Math.min(i, i2) / 2.0f) * 1.35f * (2.0f - this.invert), new long[]{Color.valueOf(Color.red(this.color) / 255.0f, Color.green(this.color) / 255.0f, Color.blue(this.color) / 255.0f, 0.0f, ColorSpace.get(named)).pack(), Color.valueOf(Color.red(this.color) / 255.0f, Color.green(this.color) / 255.0f, Color.blue(this.color) / 255.0f, 1.0f, ColorSpace.get(named)).pack()}, new float[]{AndroidUtilities.lerp(0.9f, 0.22f, this.invert), 1.0f}, Shader.TileMode.CLAMP);
        } else {
            this.gradient = new RadialGradient(this.lastWidth * 0.5f, this.lastHeight * 0.4f, (Math.min(r1, r10) / 2.0f) * 1.35f * (2.0f - this.invert), new int[]{ColorUtils.setAlphaComponent(this.color, 0), this.color}, new float[]{AndroidUtilities.lerp(0.9f, 0.22f, this.invert), 1.0f}, Shader.TileMode.CLAMP);
        }
    ...
    public void drawGradient(Canvas canvas, boolean z) {
        if (this.gradient != null) {
            ...
            canvas.drawRoundRect(rectF, AndroidUtilities.m1081dp(12.0f) - 2, AndroidUtilities.m1081dp(12.0f) - 2, this.paint);
        }
    }
