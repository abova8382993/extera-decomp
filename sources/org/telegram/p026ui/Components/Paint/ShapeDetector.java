...
public class ShapeDetector {
    ...
    private static class Point {
        ...
        public double f1935x;

        /* JADX INFO: renamed from: y */
        public double f1936y;
        ...
        public double distance(double d, double d2) {
            return Math.sqrt(Math.pow(d - this.f1935x, 2.0d) + Math.pow(d2 - this.f1936y, 2.0d));
        }

        public double distance(Point point) {
            return distance(point.f1935x, point.f1936y);
        }
    }
    ...
    public /* synthetic */ void lambda$new$2() {
        ...
        synchronized (this) {
            try {
                ...
                translate(arrayListFullClone, -pointCentroid.f1935x, -pointCentroid.f1936y);
    ...
    private ArrayList resample(ArrayList arrayList, int i) {
        ...
        while (i4 < arrayList.size()) {
            ...
            if (d3 >= dPathLength) {
                ...
                i2 = i3;
                d = dPathLength;
                Point point = new Point(((Point) arrayList.get(i5)).f1935x + ((((Point) arrayList.get(i4)).f1935x - ((Point) arrayList.get(i5)).f1935x) * d4), ((Point) arrayList.get(i5)).f1936y + (d4 * (((Point) arrayList.get(i4)).f1936y - ((Point) arrayList.get(i5)).f1936y)));
    ...
    private double distanceAtAngle(ArrayList arrayList, Point point, ArrayList arrayList2, double d) {
        ...
        while (i < iMin) {
            ...
            double d2 = point3.f1935x;
            double d3 = dCos;
            double d4 = point2.f1935x;
            double d5 = (d2 - d4) * d3;
            double d6 = point3.f1936y;
            double d7 = point2.f1936y;
    ...
    private ArrayList fullClone(ArrayList arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            arrayList2.add(new Point(point.f1935x, point.f1936y));
        }
        return arrayList2;
    }
    ...
    private void rotate(ArrayList arrayList, double d, Point point) {
        ...
        while (i < arrayList.size()) {
            ...
            double d2 = point2.f1935x;
            double d3 = point.f1935x;
            double d4 = (d2 - d3) * dCos;
            double d5 = point2.f1936y;
            double d6 = dCos;
            double d7 = point.f1936y;
            point2.f1936y = ((d2 - d3) * dSin) + ((d5 - d7) * d6) + d7;
            point2.f1935x = (d4 - ((d5 - d7) * dSin)) + d3;
            i++;
            dCos = d6;
        }
    }
    ...
    private RectD boundingBox(ArrayList arrayList) {
        ...
        double d = ((Point) arrayList.get(0)).f1935x;
        double d2 = ((Point) arrayList.get(0)).f1936y;
        RectD rectD = new RectD(d, d2, d, d2);
        for (int i = 1; i < arrayList.size(); i++) {
            Point point = (Point) arrayList.get(i);
            rectD.union(point.f1935x, point.f1936y);
        }
        return rectD;
    }
    ...
    private Point centroid(ArrayList arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            Point point2 = (Point) arrayList.get(i);
            point.f1935x += point2.f1935x;
            point.f1936y += point2.f1936y;
        }
        point.f1935x /= (double) arrayList.size();
        point.f1936y /= (double) arrayList.size();
        return point;
    }

    private double indicativeAngle(ArrayList arrayList) {
        Point pointCentroid = centroid(arrayList);
        return Math.atan2(pointCentroid.f1936y - ((Point) arrayList.get(0)).f1936y, pointCentroid.f1935x - ((Point) arrayList.get(0)).f1935x);
    }
    ...
    private Shape constructShape(int i, ArrayList arrayList) {
        ...
        if (i == 4) {
            ...
            shape.centerX = (float) point.f1935x;
            shape.centerY = (float) point.f1936y;
            shape.middleX = (float) point2.f1935x;
            shape.middleY = (float) point2.f1936y;
            shape.radiusX = (float) point3.f1935x;
            shape.radiusY = (float) point3.f1936y;
            shape.arrowTriangleLength = 16.0f;
            return shape;
        }
        Point pointCentroid = centroid(arrayList);
        shape.centerX = (float) pointCentroid.f1935x;
        shape.centerY = (float) pointCentroid.f1936y;
        RectD rectDBoundingBox = boundingBox(arrayList);
        ...
        if (i == 2 && (iFindAnglePoint = findAnglePoint(arrayList, 1)) > 0) {
            ...
            shape.rotation = (float) Math.atan2(point4.f1936y - ((double) shape.centerY), point4.f1935x - ((double) shape.centerX));
        }
        return shape;
    }
    ...
    public /* synthetic */ void lambda$showSaveLearnDialog$3(ArrayList arrayList, DialogInterface dialogInterface, int i) {
        if (i == 0) {
            ...
            for (int i2 = 0; i2 < this.templates.size(); i2++) {
                ...
                for (int i3 = 0; i3 < template.points.size(); i3++) {
                    ...
                    sb.append("[");
                    sb.append(Math.round(point.f1935x));
                    sb.append(",");
                    sb.append(Math.round(point.f1936y));
                    sb.append("]");
                }
                sb.append("],\n\t\t\"freq\": ");
        ...
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            str = str + "," + Math.round(((Point) arrayList.get(i4)).f1935x) + "," + Math.round(((Point) arrayList.get(i4)).f1936y);
        }
