...
public class ProfileMusicView extends View {
    ...
    public ProfileMusicView(Context context, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.icon = context.getResources().getDrawable(C2702R.drawable.files_music).mutate();
    ...
    protected void onMeasure(int i, int i2) {
        super.onMeasure(View.MeasureSpec.makeMeasureSpec(View.MeasureSpec.getSize(i), TLObject.FLAG_30), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m1081dp(37.0f), TLObject.FLAG_30));
    }
    ...
    public void setMusicDocument(TLRPC.Document document) {
        ...
        if (TextUtils.isEmpty(author)) {
            if (TextUtils.isEmpty(title)) {
                author = LocaleController.getString(C2702R.string.AudioUnknownArtist);
                title = " - " + LocaleController.getString(C2702R.string.AudioUnknownTitle);
            } else {
    ...
    public void updatePosition(float f, float f2) {
        ...
        setTranslationY(f - AndroidUtilities.m1081dp(12.0f));
        invalidate();
    }

    @Override // android.view.View
    public boolean dispatchTouchEvent(MotionEvent motionEvent) {
        if (Utilities.clamp01(this.currentHeight / AndroidUtilities.m1081dp(21.0f)) <= 0.0f) {
            return false;
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float fClamp01 = Utilities.clamp01(this.currentHeight / AndroidUtilities.m1081dp(21.0f));
        float scale = this.bounce.getScale(0.02f);
        if (fClamp01 <= 0.0f) {
            return;
        }
        int width = getWidth() - (AndroidUtilities.m1081dp(12.0f) * 2);
        this.author.ellipsize((width - AndroidUtilities.m1081dp(35.0f)) / 2.0f);
        this.title.ellipsize((width - this.author.getWidth()) - AndroidUtilities.m1081dp(35.0f));
        float fM1081dp = AndroidUtilities.m1081dp(16.6f) + this.author.getWidth() + this.title.getWidth() + AndroidUtilities.m1081dp(8.0f);
        float fM1081dp2 = AndroidUtilities.m1081dp(16.0f) + fM1081dp;
        canvas.save();
        canvas.scale(scale, scale, getWidth() / 2.0f, getHeight() / 2.0f);
        this.rect.set((getWidth() - fM1081dp2) / 2.0f, AndroidUtilities.m1081dp(10.0f), (getWidth() + fM1081dp2) / 2.0f, AndroidUtilities.m1081dp(10.0f) + (AndroidUtilities.m1081dp(17.0f) * fClamp01));
        ...
        canvas.translate((getWidth() - fM1081dp) / 2.0f, 0.0f);
        float height = getHeight() / 2.0f;
        AndroidUtilities.m1081dp(6.0f);
        AndroidUtilities.m1081dp(2.0f);
        int iM1081dp = AndroidUtilities.m1081dp(13.0f);
        int i = (int) height;
        int i2 = iM1081dp / 2;
        this.icon.setBounds(0, i - i2, iM1081dp, i + i2);
        this.icon.draw(canvas);
        canvas.translate(AndroidUtilities.m1081dp(16.6f), 0.0f);
