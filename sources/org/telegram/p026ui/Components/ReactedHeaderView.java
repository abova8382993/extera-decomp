...
public class ReactedHeaderView extends FrameLayout {
    ...
    public ReactedHeaderView(Context context, int i, MessageObject messageObject, long j) {
        ...
        Drawable drawableMutate = ContextCompat.getDrawable(context, C2702R.drawable.msg_reactions).mutate();
    ...
    public static class UserSeen {
        ...
        public UserSeen(TLObject tLObject, int i) {
            ...
            if (tLObject instanceof TLRPC.User) {
                this.dialogId = ((TLRPC.User) tLObject).f1775id;
            } else if (tLObject instanceof TLRPC.Chat) {
                this.dialogId = -((TLRPC.Chat) tLObject).f1610id;
            }
        }
    }
    ...
    public /* synthetic */ void lambda$onAttachedToWindow$5(long j, TLRPC.Chat chat, TLObject tLObject, TLRPC.TL_error tL_error) {
        if (tLObject instanceof Vector) {
            ...
            if (ChatObject.isChannel(chat)) {
                ...
                tL_channels_getParticipants.channel = MessagesController.getInstance(this.currentAccount).getInputChannel(chat.f1610id);
    ...
    public /* synthetic */ void lambda$onAttachedToWindow$1(TLObject tLObject, List list, List list2, List list3, Runnable runnable) {
        if (tLObject != null) {
            ...
            for (int i = 0; i < tL_channels_channelParticipants.users.size(); i++) {
                ...
                int iIndexOf = list.indexOf(Long.valueOf(user.f1775id));
    ...
    public /* synthetic */ void lambda$loadReactions$6(int i, TLRPC.TL_messages_messageReactionsList tL_messages_messageReactionsList) {
        ...
        while (i2 < size) {
            ...
            TLRPC.Peer peer = this.message.messageOwner.from_id;
            if (peer != null && user.f1775id != peer.user_id) {
                int i3 = 0;
                while (true) {
                    if (i3 >= this.users.size()) {
                        this.users.add(new UserSeen(user, 0));
                        break;
                    } else if (((UserSeen) this.users.get(i3)).dialogId == user.f1775id) {
                        break;
                    } else {
                        i3++;
                    }
        ...
        while (i4 < size2) {
            ...
            if (peer2 != null && chat.f1610id != peer2.user_id) {
                int i5 = 0;
                while (true) {
                    if (i5 >= this.users.size()) {
                        this.users.add(new UserSeen(chat, 0));
                        break;
                    } else if (((UserSeen) this.users.get(i5)).dialogId == (-chat.f1610id)) {
                        break;
                    } else {
                        i5++;
                    }
    ...
    */
    private void updateView() {
        ...
        L39:
            ...
            goto L56
        L49:
            int r0 = org.telegram.messenger.AndroidUtilities.m1081dp(r3)
        L4d:
            float r0 = (float) r0
            goto L56
        L4f:
        ...
        L61:
            ...
            return
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p026ui.Components.ReactedHeaderView.updateView():void");
    }
