...
public class RecordStatusDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        Paint paint2 = paint;
        if (paint2.getStrokeWidth() != AndroidUtilities.m1081dp(2.0f)) {
            paint2.setStrokeWidth(AndroidUtilities.m1081dp(2.0f));
        }
        canvas.save();
        canvas.translate(0.0f, (getIntrinsicHeight() / 2) + AndroidUtilities.m1081dp(this.isChat ? 1.0f : 2.0f));
        for (int i = 0; i < 4; i++) {
            if (i == 0) {
            ...
            float fM1081dp = (AndroidUtilities.m1081dp(4.0f) * i) + (AndroidUtilities.m1081dp(4.0f) * this.progress);
            float f = -fM1081dp;
            this.rect.set(f, f, fM1081dp, fM1081dp);
