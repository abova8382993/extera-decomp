...
public class FireworksEffect {
    ...
    private class Particle {
        ...
        public void draw(Canvas canvas) {
            ...
            FireworksEffect.this.particlePaint.setStrokeWidth(AndroidUtilities.m1081dp(1.5f) * this.scale);
            FireworksEffect.this.particlePaint.setAlpha((int) (this.alpha * 255.0f));
            canvas.drawPoint(this.f1914x, this.f1915y, FireworksEffect.this.particlePaint);
        }
    }
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1914x;
                float f4 = particle.f1912vx;
                float f5 = particle.velocity;
                float f6 = j;
                particle.f1914x = f3 + (((f4 * f5) * f6) / 500.0f);
                float f7 = particle.f1915y;
                float f8 = particle.f1913vy;
                particle.f1915y = f7 + (((f5 * f8) * f6) / 500.0f);
                particle.f1913vy = f8 + (f6 / 100.0f);
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (Utilities.random.nextBoolean() && this.particles.size() + 8 < 150) {
            ...
            float fNextFloat2 = i2 + (Utilities.random.nextFloat() * ((view.getMeasuredHeight() - AndroidUtilities.m1081dp(20.0f)) - i2));
            ...
            for (int i4 = 0; i4 < 8; i4++) {
                ...
                particle.f1914x = fNextFloat;
                particle.f1915y = fNextFloat2;
                particle.f1912vx = fCos * 1.5f;
                particle.f1913vy = fSin;
                particle.color = i3;
                particle.alpha = 1.0f;
