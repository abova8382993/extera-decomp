...
public class SuggestBirthdayActionLayout {
    ...
    public SuggestBirthdayActionLayout(int i, ChatActionCell chatActionCell, Theme.ResourcesProvider resourcesProvider) {
        ...
        RLottieDrawable rLottieDrawable = new RLottieDrawable(C2702R.raw.cake, "cake", AndroidUtilities.m1081dp(66.0f), AndroidUtilities.m1081dp(66.0f), true, null);
    ...
    public void set(MessageObject messageObject) {
        ...
        this.text = new Text(TextUtils.concat(messageObject.messageText, ":"), 13.0f).multiline(6).align(Layout.Alignment.ALIGN_CENTER).setMaxWidth(width() - AndroidUtilities.m1081dp(32.0f));
        int i = (tL_messageActionSuggestBirthday.birthday.flags & 1) != 0 ? 3 : 2;
        Text[] textArr = new Text[i];
        this.titles = textArr;
        this.values = new Text[i];
        textArr[0] = new Text(LocaleController.getString(C2702R.string.DateDay), 11.0f);
        ...
        this.titles[1] = new Text(LocaleController.getString(C2702R.string.DateMonth), 11.0f);
        ...
        if ((tL_messageActionSuggestBirthday.birthday.flags & 1) != 0) {
            this.titles[2] = new Text(LocaleController.getString(C2702R.string.DateYear), 11.0f);
        ...
        this.button = new Text(LocaleController.getString(C2702R.string.SuggestedDateOfBirthView), 14.0f, AndroidUtilities.bold());
    }

    private final String getMonthName(int i) {
        int[] iArr = {C2702R.string.January, C2702R.string.February, C2702R.string.March, C2702R.string.April, C2702R.string.May, C2702R.string.June, C2702R.string.July, C2702R.string.August, C2702R.string.September, C2702R.string.October, C2702R.string.November, C2702R.string.December};
        if (i < 0 || i >= 12) {
    ...
    public void draw(Canvas canvas) {
        ...
        int width = (this.view.getWidth() - iM1081dp) / 2;
        this.sticker.setBounds(width, AndroidUtilities.m1081dp(13.0f), width + iM1081dp, AndroidUtilities.m1081dp(13.0f) + iM1081dp);
        this.sticker.draw(canvas);
        this.text.draw(canvas, (this.view.getWidth() - this.text.getWidth()) / 2.0f, AndroidUtilities.m1081dp(19.0f) + iM1081dp, -1, 1.0f);
        int iM1081dp2 = (int) (AndroidUtilities.m1081dp(19.0f) + iM1081dp + this.text.getHeight() + AndroidUtilities.m1081dp(17.0f));
        int iM1081dp3 = 0;
        for (int i = 0; i < this.titles.length; i++) {
            iM1081dp3 = (int) (iM1081dp3 + AndroidUtilities.m1081dp(9.0f) + Math.max(this.titles[i].getWidth(), this.values[i].getWidth()) + AndroidUtilities.m1081dp(9.0f));
        }
        int width2 = (this.view.getWidth() - iM1081dp3) / 2;
        int i2 = 0;
        while (i2 < this.titles.length) {
            float fM1081dp = AndroidUtilities.m1081dp(9.0f) + Math.max(this.titles[i2].getWidth(), this.values[i2].getWidth()) + AndroidUtilities.m1081dp(9.0f);
            float f = width2;
            float f2 = f + (fM1081dp / 2.0f);
            int i3 = (int) (f + fM1081dp);
            Text text = this.titles[i2];
            text.draw(canvas, f2 - (text.getWidth() / 2.0f), iM1081dp2, -1, 0.75f);
            Text text2 = this.values[i2];
            text2.draw(canvas, f2 - (text2.getWidth() / 2.0f), AndroidUtilities.m1081dp(16.0f) + iM1081dp2, -1, 1.0f);
            i2++;
            width2 = i3;
        }
        if (this.hasButton) {
            int iM1081dp4 = iM1081dp2 + AndroidUtilities.m1081dp(38.0f);
            canvas.save();
            float width3 = this.button.getWidth() + AndroidUtilities.m1081dp(26.0f);
            float fM1081dp2 = AndroidUtilities.m1081dp(30.0f);
            float f3 = iM1081dp4;
            this.buttonRect.set((this.view.getWidth() - width3) / 2.0f, f3, (this.view.getWidth() + width3) / 2.0f, f3 + fM1081dp2);
            ...
            this.button.draw(canvas, this.buttonRect.left + AndroidUtilities.m1081dp(13.0f), this.buttonRect.centerY(), -1, 1.0f);
            canvas.restore();
        }
    }

    public int width() {
        return AndroidUtilities.m1081dp(174.0f);
    }

    public int height() {
        return AndroidUtilities.m1081dp(140.0f) + ((int) this.text.getHeight()) + (this.hasButton ? AndroidUtilities.m1081dp(40.0f) : 0);
    }
    ...
    public void open() {
        AlertsCreator.createBirthdayPickerDialog(this.view.getContext(), LocaleController.getString(C2702R.string.DateOfBirth), LocaleController.getString(C2702R.string.DateOfBirthAddToProfile), this.birthday, new Utilities.Callback() { // from class: org.telegram.ui.Components.SuggestBirthdayActionLayout$$ExternalSyntheticLambda0
    ...
    public /* synthetic */ void lambda$open$0(TLObject tLObject, TLRPC.UserFull userFull, TL_account.TL_birthday tL_birthday, TLRPC.TL_error tL_error) {
        ...
        if (tLObject instanceof TLRPC.TL_boolTrue) {
            BulletinFactory.m1195of(safeLastFragment).createSimpleBulletin(C2702R.raw.gift, LocaleController.getString(C2702R.string.PrivacyBirthdaySetDone), LocaleController.getString(C2702R.string.PrivacyBirthdaySetDoneInfo)).setDuration(5000).show();
            return;
        }
        if (userFull != null) {
            if (tL_birthday == null) {
        ...
        if (tL_error != null && (str = tL_error.text) != null && str.startsWith("FLOOD_WAIT_")) {
            new AlertDialog.Builder(this.view.getContext()).setTitle(LocaleController.getString(C2702R.string.PrivacyBirthdayTooOftenTitle)).setMessage(LocaleController.getString(C2702R.string.PrivacyBirthdayTooOftenMessage)).setPositiveButton(LocaleController.getString(C2702R.string.f1556OK), null).show();
        } else {
            BulletinFactory.m1195of(safeLastFragment).createSimpleBulletin(C2702R.raw.error, LocaleController.getString(C2702R.string.UnknownError)).show();
        }
    }
}
