...
public class StickerSetLinkIcon extends Drawable {
    ...
    public int alpha = Function.USE_VARARGS;
    ...
    public boolean equals(ArrayList arrayList) {
        ...
        while (true) {
            ...
            if ((document == null ? 0L : document.f1618id) != ((TLRPC.Document) arrayList.get(i)).f1618id) {
                return false;
            }
            i++;
        }
    ...
    public void draw(Canvas canvas) {
        ...
        float intrinsicWidth = getIntrinsicWidth() / this.f2042N;
        float intrinsicHeight = getIntrinsicHeight() / this.f2042N;
        canvas.save();
        canvas.clipRect(fCenterX, fCenterY, getIntrinsicWidth() + fCenterX, getIntrinsicHeight() + fCenterY);
        for (int i = 0; i < this.f2042N; i++) {
            int i2 = 0;
            while (true) {
                int i3 = this.f2042N;
                if (i2 < i3) {
