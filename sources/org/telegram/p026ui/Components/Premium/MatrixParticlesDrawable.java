...
public class MatrixParticlesDrawable {
        ...
        while (i7 <= iWidth) {
            ...
            while (i8 < arrayList.size()) {
                ...
                if (jCurrentTimeMillis - particle2.time > 50) {
                    ...
                    if (particle2.f1978y > particle2.len && i8 == arrayList.size() - 1 && Math.abs(Utilities.fastRandom.nextInt(4)) == 0) {
                ...
                int i10 = i8;
                int iMin = Math.min(particle2.f1978y, iHeight + 1);
                int iMax = Math.max(i3, particle2.f1978y - particle2.len);
                while (iMax < iMin) {
                    int i11 = this.size;
                    float f = i11 * i7;
                    float f2 = i11 * iMax;
                    if (this.excludeRect.contains(f, f2)) {
                        ...
                        i = iMin;
                        i2 = iMax;
                        this.matrixTextParticles[i7][iMax].draw(canvas, f, f2, jCurrentTimeMillis, Utilities.clamp(((1.0f - ((particle2.f1978y - iMax) / (particle2.len - 1))) * 0.8f) + 0.2f, 1.0f, 0.0f));
                    }
                    iMax = i2 + 1;
                    iMin = i;
                }
    ...
    private class Particle {
        ...
        public void init(int i, long j) {
            this.f1978y = Math.abs(Utilities.fastRandom.nextInt() % i);
            this.time = j;
