...
public class ReassignBoostBottomSheet extends BottomSheetWithRecyclerListView {
    ...
    public ReassignBoostBottomSheet(BaseFragment baseFragment, TL_stories.TL_premium_myBoosts tL_premium_myBoosts, final TLRPC.Chat chat) {
        ...
        while (i < size) {
            ...
            if (peer != null && DialogObject.getPeerDialogId(peer) != (-chat.f1610id)) {
        ...
        selectorBtnCell.setPadding(AndroidUtilities.m1081dp(8.0f), AndroidUtilities.m1081dp(8.0f), AndroidUtilities.m1081dp(8.0f), AndroidUtilities.m1081dp(8.0f));
        ...
        recyclerListView.setPadding(i3, 0, i3, AndroidUtilities.m1081dp(64.0f));
    ...
    public /* synthetic */ void lambda$new$3(final TLRPC.Chat chat, View view) {
        ...
        BoostRepository.applyBoost(chat.f1610id, arrayList, new Utilities.Callback() { // from class: org.telegram.ui.Components.Premium.boosts.ReassignBoostBottomSheet$$ExternalSyntheticLambda2
    ...
    public /* synthetic */ void lambda$new$1(TLRPC.Chat chat, final List list, final HashSet hashSet, final TL_stories.TL_premium_myBoosts tL_premium_myBoosts) {
        MessagesController.getInstance(this.currentAccount).getBoostsController().getBoostsStats(-chat.f1610id, new Consumer() { // from class: org.telegram.ui.Components.Premium.boosts.ReassignBoostBottomSheet$$ExternalSyntheticLambda4
    ...
    public /* synthetic */ void lambda$new$4(TLRPC.Chat chat, View view, int i) {
        if (view instanceof SelectorUserCell) {
            ...
            if (selectorUserCell.getBoost().cooldown_until_date > 0) {
                BulletinFactory.m1194of(this.container, this.resourcesProvider).createSimpleBulletin(C2702R.raw.chats_infotip, AndroidUtilities.replaceTags(LocaleController.formatPluralString("BoostingWaitWarningPlural", BoostRepository.boostsPerSentGift(), new Object[0])), 5).show(true);
                return;
            }
    ...
    private void updateActionButton(boolean z) {
        ...
        if (this.selectedBoosts.size() > 1) {
            this.actionButton.setText(LocaleController.getString(C2702R.string.BoostingReassignBoosts), z);
        } else {
            this.actionButton.setText(LocaleController.getString(C2702R.string.BoostingReassignBoost), z);
        }
    ...
    protected CharSequence getTitle() {
        return LocaleController.getString(C2702R.string.BoostingReassignBoost);
    }
        ...
        return new RecyclerListView.SelectionAdapter() { // from class: org.telegram.ui.Components.Premium.boosts.ReassignBoostBottomSheet.3
            ...
            public void onBindViewHolder(RecyclerView.ViewHolder viewHolder, int i) {
                ...
                if (viewHolder.getItemViewType() == 2) {
                    ...
                    headerCell.setPadding(0, 0, 0, AndroidUtilities.m1081dp(2.0f));
                    headerCell.setText(LocaleController.getString(C2702R.string.BoostingRemoveBoostFrom));
                    return;
                }
    ...
    static class TopCell extends LinearLayout {
        ...
        public TopCell(Context context) {
            ...
            textView.setText(LocaleController.getString(C2702R.string.BoostingReassignBoost));
        ...
        public void showChats(List list, TLRPC.Chat chat) {
            ...
            while (true) {
                ...
                avatarHolderView3.setTranslationX((-size2) * AndroidUtilities.m1081dp(23.0f));
                avatarHolderView3.setAlpha(0.0f);
                avatarHolderView3.setScaleX(0.1f);
                avatarHolderView3.setScaleY(0.1f);
                avatarHolderView3.animate().alpha(1.0f).scaleX(1.0f).scaleY(1.0f).setInterpolator(cubicBezierInterpolator).setDuration(DataTypes.EMPTY).start();
                if (size2 == 0) {
            ...
            while (i4 < size3) {
                ...
                if (avatarHolderView != null) {
                    ...
                    ViewPropertyAnimator interpolator = avatarHolderView.animate().alpha(f).translationXBy(AndroidUtilities.m1081dp(23.0f)).scaleX(f2).scaleY(f2).setInterpolator(cubicBezierInterpolator);
                    ...
                    while (i6 < size5) {
                        ...
                        if (avatarHolderView4 != avatarHolderView) {
                            i5++;
                            avatarHolderView4.animate().translationX((-(size6 - i5)) * AndroidUtilities.m1081dp(23.0f)).setInterpolator(cubicBezierInterpolator).setDuration(j).start();
                        }
                    }
            ...
            if (this.addedChats.isEmpty() || this.addedChats.size() == 1) {
                this.avatarsContainer.animate().setInterpolator(cubicBezierInterpolator).translationX(0.0f).setDuration(DataTypes.EMPTY).start();
            } else {
                this.avatarsContainer.animate().setInterpolator(cubicBezierInterpolator).translationX(AndroidUtilities.m1081dp(11.5f) * (this.addedChats.size() - 1)).setDuration(DataTypes.EMPTY).start();
            }
            this.toAvatar.animate().cancel();
            this.avatarsWrapper.animate().cancel();
            if (this.addedChats.isEmpty()) {
                ViewPropertyAnimator viewPropertyAnimatorTranslationX = this.avatarsWrapper.animate().setInterpolator(cubicBezierInterpolator).translationX(0.0f);
                long j2 = DataTypes.EMPTY;
                viewPropertyAnimatorTranslationX.setDuration(j2).start();
                this.toAvatar.animate().setInterpolator(cubicBezierInterpolator).translationX(0.0f).setDuration(j2).start();
                return;
            }
            ViewPropertyAnimator viewPropertyAnimatorTranslationX2 = this.avatarsWrapper.animate().setInterpolator(cubicBezierInterpolator).translationX(-AndroidUtilities.m1081dp(48.0f));
            long j3 = DataTypes.EMPTY;
            viewPropertyAnimatorTranslationX2.setDuration(j3).start();
            this.toAvatar.animate().setInterpolator(cubicBezierInterpolator).translationX(AndroidUtilities.m1081dp(48.0f)).setDuration(j3).start();
        }
    }
    ...
    private static class AvatarHolderView extends FrameLayout {
        ...
        protected void dispatchDraw(Canvas canvas) {
            canvas.drawCircle(getMeasuredWidth() / 2.0f, getMeasuredHeight() / 2.0f, (getMeasuredHeight() / 2.0f) - AndroidUtilities.m1081dp(2.0f), this.bgPaint);
    ...
    private static class BoostIconView extends View {
        ...
        public BoostIconView(Context context) {
            ...
            this.boostDrawable = ContextCompat.getDrawable(getContext(), C2702R.drawable.mini_boost_remove);
        ...
        protected void onDraw(Canvas canvas) {
            ...
            this.boostDrawable.setBounds((int) (measuredWidth - fM1081dp), (int) (measuredHeight - fM1081dp), (int) (measuredWidth + fM1081dp), (int) (measuredHeight + fM1081dp));
