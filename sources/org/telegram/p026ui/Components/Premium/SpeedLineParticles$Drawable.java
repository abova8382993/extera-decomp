...
public class SpeedLineParticles$Drawable {
    ...
    private final float f1985dt = 1000.0f / AndroidUtilities.screenRefreshRate;
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < this.particles.size(); i++) {
            ...
            if (jCurrentTimeMillis > particle.lifeTime || !this.screenRect.contains(particle.f1986x, particle.f1987y)) {
    ...
    private class Particle {
        ...
        private float f1986x;

        /* JADX INFO: renamed from: y */
        private float f1987y;
        ...
        public void draw(Canvas canvas, int i, long j) {
            int i2 = i * 4;
            SpeedLineParticles$Drawable.this.lines[i2] = this.f1986x;
            SpeedLineParticles$Drawable.this.lines[i2 + 1] = this.f1987y;
            SpeedLineParticles$Drawable.this.lines[i2 + 2] = this.f1986x + (AndroidUtilities.m1081dp(30.0f) * this.vecX);
            SpeedLineParticles$Drawable.this.lines[i2 + 3] = this.f1987y + (AndroidUtilities.m1081dp(30.0f) * this.vecY);
            if (SpeedLineParticles$Drawable.this.paused) {
                return;
            }
            float fM1081dp = AndroidUtilities.m1081dp(4.0f) * (SpeedLineParticles$Drawable.this.f1985dt / 660.0f);
            SpeedLineParticles$Drawable speedLineParticles$Drawable = SpeedLineParticles$Drawable.this;
            float f = fM1081dp * speedLineParticles$Drawable.speedScale;
            this.f1986x += this.vecX * f;
            this.f1987y += this.vecY * f;
            float f2 = this.inProgress;
            if (f2 != 1.0f) {
                float f3 = f2 + (speedLineParticles$Drawable.f1985dt / 200.0f);
                this.inProgress = f3;
                if (f3 > 1.0f) {
        ...
        public void genPosition(long j, boolean z) {
            ...
            this.f1986x = fAbs;
            this.f1987y = fAbs2;
            double dAtan2 = Math.atan2(fAbs - SpeedLineParticles$Drawable.this.rect.centerX(), this.f1987y - SpeedLineParticles$Drawable.this.rect.centerY());
