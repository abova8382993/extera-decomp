...
import org.telegram.p026ui.Components.quickforward.QuickShareSelectorDrawable;
...
class QuickShareAvatarCell implements ValueAnimator.AnimatorUpdateListener {
    ...
    public void draw(Canvas canvas, float f, float f2, float f3, float f4, float f5, float f6, float f7, float f8, boolean z) {
        if (!z) {
            drawAvatarImpl(canvas, f5, f6, f7 + (AndroidUtilities.m1081dp(2.0f) * this.selectedFactor), f8);
        }
        ...
        float width = this.textLayout.getWidth() + (AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.TEXT_PADDING_INTERNAL) * 2);
        this.bgX1 = fixX(fixX(f5, width, f3, f4), width, f, f2) - (width / 2.0f);
        this.bgY = f6 - AndroidUtilities.m1081dp(58.0f);
        if (this.blurredTextDrawable == null && !this.parent.isDestroyed()) {
            ...
            blurVisibilityDrawable.render((int) width, AndroidUtilities.m1081dp(21.0f), AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.TEXT_BLUR_RADIUS), 3.0f);
        }
        BlurVisibilityDrawable blurVisibilityDrawable2 = this.blurredTextDrawable;
        if (blurVisibilityDrawable2 != null) {
            float f12 = this.bgX1;
            float f13 = this.bgY;
            blurVisibilityDrawable2.setBounds((int) f12, (int) f13, (int) (f12 + width), (int) (f13 + AndroidUtilities.m1081dp(21.0f)));
    ...
    public void drawBlurredAvatar(Canvas canvas, float f, float f2, float f3, float f4) {
        if (this.blurredAvatarDrawable == null) {
            ...
            blurVisibilityDrawable.render(AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR), AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR), AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.BLUR_RADIUS), 4.0f);
        }
        canvas.save();
        canvas.translate(f - f3, f2 - f3);
        canvas.scale(f3 / AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR_RADIUS), f3 / AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR_RADIUS));
    ...
    public void renderAvatar(Canvas canvas, int i) {
        float fM1081dp = AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR_RADIUS);
        drawAvatarImpl(canvas, fM1081dp, fM1081dp, fM1081dp, i / 255.0f);
    }
    ...
    private void drawTextImpl(Canvas canvas, float f, float f2, float f3) {
        ...
        float fM1081dp = AndroidUtilities.m1081dp(21.0f) / 2.0f;
        rectF.set(f, f2, this.textLayout.getWidth() + f + (AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.TEXT_PADDING_INTERNAL) * 2), AndroidUtilities.m1081dp(21.0f) + f2);
        ...
        if (paint != null) {
            ...
            canvas.drawRoundRect(rectF, fM1081dp, fM1081dp, themedPaint);
        ...
        if (zHasGradientService || paint != null) {
            ...
            canvas.drawRoundRect(rectF, fM1081dp, fM1081dp, Theme.chat_actionBackgroundGradientDarkenPaint);
            Theme.chat_actionBackgroundGradientDarkenPaint.setAlpha(alpha3);
        }
        canvas.save();
        canvas.translate(f + AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.TEXT_PADDING_INTERNAL), f2 + ((AndroidUtilities.m1081dp(21.0f) - this.textLayout.getHeight()) / 2.0f));
    ...
    private void setDialog(ChatMessageCell chatMessageCell, long j) {
        ...
        if (zIsUserDialog) {
            ...
            if (UserObject.isUserSelf(user)) {
                string = LocaleController.getString(C2702R.string.SavedMessages);
                ...
                this.imageReceiver.setRoundRadius(AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR / 2.0f));
                this.imageReceiver.setImageCoords(0.0f, 0.0f, AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR), AndroidUtilities.m1081dp(QuickShareSelectorDrawable.Sizes.AVATAR));
                Paint themedPaint = chatMessageCell.getThemedPaint("paintChatActionText");
                if (string != null || themedPaint == null) {
                }
                int iM1081dp = AndroidUtilities.displaySize.x - AndroidUtilities.m1081dp((QuickShareSelectorDrawable.Sizes.TEXT_PADDING_INTERNAL * 2) + (QuickShareSelectorDrawable.Sizes.TEXT_PADDING_EXTERNAL * 2));
                TextPaint textPaint = new TextPaint(themedPaint);
                this.textLayout = new StaticLayout(TextUtils.ellipsize(string, textPaint, iM1081dp, TextUtils.TruncateAt.END), textPaint, (int) Math.ceil(textPaint.measureText(r4, 0, r4.length())), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, false);
                return;
            }
            if (user != null) {
