...
public class SuggestionOffer {
    ...
    public void update(MessageObject messageObject) {
        ...
        MessageSuggestionParams messageSuggestionParamsM1103of = MessageSuggestionParams.m1103of(suggestedPost);
        TextPaint textPaint = (TextPaint) getThemedPaint("paintChatActionText3");
        this.height = AndroidUtilities.m1081dp(14.0f) * 2;
        this.rows.clear();
        AmountUtils$Amount amountUtils$Amount = messageSuggestionParamsM1103of.amount;
        if (amountUtils$Amount != null && !amountUtils$Amount.isZero()) {
            this.rows.add(new Row(new Text(LocaleController.getString(C2702R.string.SuggestionOfferInfoPrice), textPaint), new Text(LocaleController.bold(messageSuggestionParamsM1103of.amount.formatAsDecimalSpaced()), textPaint)));
        }
        if (suggestedPost.schedule_date > 0) {
            this.rows.add(new Row(new Text(LocaleController.getString(C2702R.string.SuggestionOfferInfoTime), textPaint), new Text(LocaleController.bold(LocaleController.formatDateTime(suggestedPost.schedule_date, true)), textPaint)));
        }
        ...
        while (i3 < size) {
            ...
            this.height = height;
            this.height = height + AndroidUtilities.m1081dp(7.0f);
        }
        int iM1081dp = (int) (fMax + fMax2 + AndroidUtilities.m1081dp(11.0f));
        int iMax = Math.max(iM1081dp, AndroidUtilities.m1081dp(160.0f));
        ...
        if (editedSuggestionFlags == 0) {
            if (messageObject.isOutOwner()) {
                spannableStringBuilder.append((CharSequence) LocaleController.getString(C2702R.string.SuggestionOfferInfoTitleYou));
            } else {
                spannableStringBuilder.append((CharSequence) LocaleController.formatString(C2702R.string.SuggestionOfferInfoTitle, name));
            }
            f = 11.0f;
            i2 = 0;
        } else {
            ...
            if (i7 != 0) {
                f = 11.0f;
                updateBuildTitleStep(sb, C2702R.string.SuggestionOfferInfoTitleEditedPrice, i8 == 1);
                i = 1;
            } else {
                f = 11.0f;
                i = 0;
            ...
            if (i5 != 0) {
                i++;
                i2 = 0;
                updateBuildTitleStep(sb, C2702R.string.SuggestionOfferInfoTitleEditedTime, i8 == i);
            } else {
                i2 = 0;
            }
            if (i4 != 0) {
                i++;
                updateBuildTitleStep(sb, C2702R.string.SuggestionOfferInfoTitleEditedText, i8 == i ? 1 : i2);
            }
            if (i6 != 0) {
                updateBuildTitleStep(sb, C2702R.string.SuggestionOfferInfoTitleEditedMedia, i8 == i + 1 ? 1 : i2);
            }
            if (messageObject.isOutOwner()) {
                int i9 = C2702R.string.SuggestionOfferInfoTitleEditedFromYou;
                Object[] objArr = new Object[1];
                objArr[i2] = sb;
                spannableStringBuilder.append((CharSequence) LocaleController.formatString(i9, objArr));
            } else {
                int i10 = C2702R.string.SuggestionOfferInfoTitleEditedFromX;
        ...
        this.height = height2;
        this.height = height2 + AndroidUtilities.m1081dp(5.0f);
        int iMax3 = Math.max(iM1081dp, iMax2) + (AndroidUtilities.m1081dp(24.0f) * 2);
        this.width = iMax3;
        this.titleX = (iMax3 - iMax) / 2;
        this.rowsTitleX = (iMax3 - iM1081dp) / 2;
        this.rowsInfoX = (int) (r1 + AndroidUtilities.m1081dp(f) + fMax2);
    }
    ...
    private void updateBuildTitleStep(StringBuilder sb, int i, boolean z) {
        if (sb.length() > 0) {
            if (z) {
                sb.append(' ');
                sb.append(LocaleController.getString(C2702R.string.SuggestionOfferInfoTitleEditedAnd));
                sb.append(' ');
            } else {
                sb.append(", ");
            }
    ...
    public void draw(Canvas canvas, int i, float f, float f2, float f3, float f4, boolean z) {
        ...
        canvas.drawRoundRect(rectF, AndroidUtilities.m1081dp(15.0f), AndroidUtilities.m1081dp(15.0f), themePaint);
        ...
        if (this.title != null) {
            canvas.save();
            canvas.translate(this.titleX + i2, iM1081dp);
            this.title.draw(canvas);
            canvas.restore();
            iM1081dp += this.title.getHeight() + AndroidUtilities.m1081dp(12.0f);
        }
        ArrayList arrayList = this.rows;
        int size = arrayList.size();
        int i3 = 0;
        while (i3 < size) {
            ...
            iM1081dp += row.getHeight() + AndroidUtilities.m1081dp(7.0f);
        }
        canvas.restore();
    }
