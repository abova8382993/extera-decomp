...
public abstract class AvatarConstructorPreviewCell extends FrameLayout {
    ...
    public static TLRPC.TL_emojiList getOrCreateEmojiList(int i, boolean z) {
        ...
        if (stickerSets.isEmpty()) {
            ...
            for (int i2 = 0; i2 < featuredEmojiSets.size(); i2++) {
                ...
                if (document != null) {
                    tL_emojiList2.document_id.add(Long.valueOf(document.f1618id));
                } else if (stickerSetCovered instanceof TLRPC.TL_stickerSetFullCovered) {
                    TLRPC.TL_stickerSetFullCovered tL_stickerSetFullCovered = (TLRPC.TL_stickerSetFullCovered) stickerSetCovered;
                    if (!tL_stickerSetFullCovered.documents.isEmpty()) {
                        tL_emojiList2.document_id.add(Long.valueOf(((TLRPC.Document) tL_stickerSetFullCovered.documents.get(0)).f1618id));
                    }
                }
            }
        } else {
            for (int i3 = 0; i3 < stickerSets.size(); i3++) {
                ...
                if (!tL_messages_stickerSet.documents.isEmpty()) {
                    tL_emojiList2.document_id.add(Long.valueOf(((TLRPC.Document) tL_messages_stickerSet.documents.get(Math.abs(Utilities.fastRandom.nextInt() % tL_messages_stickerSet.documents.size()))).f1618id));
                }
            }
        }
    ...
    public AvatarConstructorPreviewCell(Context context, boolean z) {
        ...
        this.textView.setText(LocaleController.getString(C2702R.string.UseEmoji));
