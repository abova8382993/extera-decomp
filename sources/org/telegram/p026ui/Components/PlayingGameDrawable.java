...
public class PlayingGameDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        int intrinsicHeight = getBounds().top + ((getIntrinsicHeight() - iM1081dp) / 2);
        if (!this.isChat) {
            intrinsicHeight += AndroidUtilities.m1081dp(1.0f);
        }
        ...
        this.rect.set(0.0f, i, iM1081dp, i + iM1081dp);
        float f = this.progress;
        int i2 = (int) (f < 0.5f ? (1.0f - (f / 0.5f)) * 35.0f : ((f - 0.5f) * 35.0f) / 0.5f);
        for (int i3 = 0; i3 < 3; i3++) {
            float fM1081dp = (AndroidUtilities.m1081dp(5.0f) * i3) + AndroidUtilities.m1081dp(9.2f);
            float fM1081dp2 = AndroidUtilities.m1081dp(5.0f);
            float f2 = this.progress;
            float f3 = fM1081dp - (fM1081dp2 * f2);
            if (i3 == 2) {
                this.paint.setAlpha(Math.min(Function.USE_VARARGS, (int) ((f2 * 255.0f) / 0.5f)));
            } else if (i3 != 0) {
            ...
            canvas.drawCircle(f3, (iM1081dp / 2) + i, AndroidUtilities.m1081dp(1.2f), this.paint);
        }
        this.paint.setAlpha(Function.USE_VARARGS);
        ...
        canvas.drawCircle(AndroidUtilities.m1081dp(4.0f), (i + (iM1081dp / 2)) - AndroidUtilities.m1081dp(2.0f), AndroidUtilities.m1081dp(1.0f), this.paint);
        checkUpdate();
    }
