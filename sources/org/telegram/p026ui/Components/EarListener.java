    ...
    public EarListener(Context context) {
        ...
        if (this.linearSensor == null || defaultSensor == null) {
            if (BuildVars.LOGS_ENABLED) {
                FileLog.m1090d("gravity or linear sensor not found");
            }
    ...
    public void onSensorChanged(SensorEvent sensorEvent) {
        ...
        if (this.attached && VoIPService.getSharedInstance() == null) {
            if (sensorEvent.sensor.getType() == 8) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1090d("proximity changed to " + sensorEvent.values[0] + " max value = " + sensorEvent.sensor.getMaximumRange());
                }
                        ...
                        if (i7 != 6 || i3 == 0) {
                            ...
                            if (i8 == 6) {
                                ...
                                if (BuildVars.LOGS_ENABLED && BuildVars.DEBUG_PRIVATE_VERSION) {
                                    FileLog.m1090d("motion detected");
                                }
                            }
                        }
            ...
            if (this.proximityWakeLock != null && disableWakeLockWhenNotUsed()) {
                ...
                if (zIsHeld && i11 == 0) {
                    if (BuildVars.LOGS_ENABLED) {
                        FileLog.m1090d("wake lock releasing");
                    }
                    this.proximityWakeLock.release();
                } else if (!zIsHeld && i11 != 0) {
                    if (BuildVars.LOGS_ENABLED) {
                        FileLog.m1090d("wake lock acquiring");
                    }
                    this.proximityWakeLock.acquire();
                }
            }
