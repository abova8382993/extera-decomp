...
import org.telegram.p026ui.Cells.ShareDialogCell$RepostStoryDrawable$$ExternalSyntheticLambda0;
...
public class ChatActivityEnterViewAnimatedIconView extends RLottieImageView {
    ...
    public ChatActivityEnterViewAnimatedIconView(Context context, int i) {
        ...
        this.stateMap = new HashMap() { // from class: org.telegram.ui.Components.ChatActivityEnterViewAnimatedIconView.1
            ...
            public RLottieDrawable get(Object obj) {
                ...
                return new RLottieDrawable(i2, String.valueOf(i2), AndroidUtilities.m1081dp(ChatActivityEnterViewAnimatedIconView.this.sizeDp), AndroidUtilities.m1081dp(ChatActivityEnterViewAnimatedIconView.this.sizeDp));
            }
        };
        this.sizeDp = i;
    }
    ...
    public void setState(State state, boolean z) {
        ...
        if (iOrdinal == 0) {
            setContentDescription(LocaleController.getString(C2702R.string.AccDescrVoiceMessage));
        } else {
            if (iOrdinal != 1) {
                return;
            }
            setContentDescription(LocaleController.getString(C2702R.string.AccDescrVideoMessage));
        }
    }
    ...
    private static final class TransitState {
        ...
        static {
            ...
            VOICE_TO_VIDEO = new TransitState("VOICE_TO_VIDEO", 0, state, state2, C2702R.raw.voice_and_video);
            State state3 = State.STICKER;
            State state4 = State.KEYBOARD;
            STICKER_TO_KEYBOARD = new TransitState("STICKER_TO_KEYBOARD", 1, state3, state4, C2702R.raw.sticker_to_keyboard);
            State state5 = State.SMILE;
            SMILE_TO_KEYBOARD = new TransitState("SMILE_TO_KEYBOARD", 2, state5, state4, C2702R.raw.smile_to_keyboard);
            VIDEO_TO_VOICE = new TransitState("VIDEO_TO_VOICE", 3, state2, state, C2702R.raw.voice_and_video);
            KEYBOARD_TO_STICKER = new TransitState("KEYBOARD_TO_STICKER", 4, state4, state3, C2702R.raw.keyboard_to_sticker);
            State state6 = State.GIF;
            KEYBOARD_TO_GIF = new TransitState("KEYBOARD_TO_GIF", 5, state4, state6, C2702R.raw.keyboard_to_gif);
            KEYBOARD_TO_SMILE = new TransitState("KEYBOARD_TO_SMILE", 6, state4, state5, C2702R.raw.keyboard_to_smile);
            GIF_TO_KEYBOARD = new TransitState("GIF_TO_KEYBOARD", 7, state6, state4, C2702R.raw.gif_to_keyboard);
            GIF_TO_SMILE = new TransitState("GIF_TO_SMILE", 8, state6, state5, C2702R.raw.gif_to_smile);
            SMILE_TO_GIF = new TransitState("SMILE_TO_GIF", 9, state5, state6, C2702R.raw.smile_to_gif);
            SMILE_TO_STICKER = new TransitState("SMILE_TO_STICKER", 10, state5, state3, C2702R.raw.smile_to_sticker);
            STICKER_TO_SMILE = new TransitState("STICKER_TO_SMILE", 11, state3, state5, C2702R.raw.sticker_to_smile);
            $VALUES = $values();
        }
