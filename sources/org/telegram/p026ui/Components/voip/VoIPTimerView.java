...
public class VoIPTimerView extends View {
    ...
    public VoIPTimerView(Context context) {
        ...
        Drawable drawable = ContextCompat.getDrawable(context, C2702R.drawable.calls_decline);
        this.callsDeclineDrawable = drawable;
        drawable.setBounds(0, 0, AndroidUtilities.m1081dp(24.0f), AndroidUtilities.m1081dp(24.0f));
    }
    ...
    protected void onMeasure(int i, int i2) {
        ...
        if (staticLayout != null) {
            ...
            setMeasuredDimension(View.MeasureSpec.getSize(i), AndroidUtilities.m1081dp(15.0f));
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        StaticLayout staticLayout = this.timerLayout;
        int i = 0;
        int width = staticLayout == null ? 0 : staticLayout.getWidth() + AndroidUtilities.m1081dp(21.0f);
        canvas.save();
        canvas.translate((getMeasuredWidth() - width) / 2.0f, 0.0f);
        canvas.save();
        if (this.isDrawCallIcon) {
            canvas.translate(-AndroidUtilities.m1081dp(7.0f), -AndroidUtilities.m1081dp(3.0f));
            this.callsDeclineDrawable.draw(canvas);
        } else {
            canvas.translate(0.0f, (getMeasuredHeight() - AndroidUtilities.m1081dp(11.0f)) / 2.0f);
            while (i < 4) {
                int i2 = i + 1;
                Paint paint = i2 > this.signalBarCount ? this.inactivePaint : this.activePaint;
                float f = i;
                this.rectF.set(AndroidUtilities.dpf2(4.16f) * f, AndroidUtilities.dpf2(2.75f) * (3 - i), (AndroidUtilities.dpf2(4.16f) * f) + AndroidUtilities.dpf2(2.75f), AndroidUtilities.m1081dp(11.0f));
        ...
        canvas.restore();
        if (staticLayout != null) {
            canvas.translate(AndroidUtilities.m1081dp(21.0f), 0.0f);
