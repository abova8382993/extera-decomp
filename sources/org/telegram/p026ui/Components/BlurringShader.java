...
import org.telegram.p026ui.Components.Paint.Shader;
...
public class BlurringShader {
    ...
    public boolean setup(float f, boolean z, int i) {
        ...
        String res = AndroidUtilities.readRes(C2702R.raw.blur_vrt);
        String res2 = AndroidUtilities.readRes(C2702R.raw.blur_frg);
    ...
    public void draw(float[] fArr, int i, int i2, int i3) {
        ...
        GLES20.glUseProgram(program.f1883gl);
    ...
    public static class BlurManager {
        ...
        public void setRenderNode(View view, Object obj, int i) {
            ...
            if (obj != null && Build.VERSION.SDK_INT >= 31) {
                ...
                RenderNode renderNodeM1086m = BotFullscreenButtons$$ExternalSyntheticApiModelOutline0.m1086m("blurRenderNode");
                renderNodeM1086m.setRenderEffect(RenderEffect.createBlurEffect(AndroidUtilities.m1081dp(35.0f), AndroidUtilities.m1081dp(35.0f), Shader.TileMode.CLAMP));
                renderNodeM1086m.setPosition(0, 0, renderNodeM1087m.getWidth(), renderNodeM1087m.getHeight());
                RecordingCanvas recordingCanvasBeginRecording = renderNodeM1086m.beginRecording();
                recordingCanvasBeginRecording.drawColor(i);
                recordingCanvasBeginRecording.drawRenderNode(renderNodeM1087m);
                renderNodeM1086m.endRecording();
    ...
    public static class StoryBlurDrawer {
        ...
        public void drawRect(Canvas canvas, float f, float f2, float f3, boolean z) {
            if (this.manager.hasRenderNode() && Build.VERSION.SDK_INT >= 31) {
                ...
                RenderNode renderNodeM1087m = BotFullscreenButtons$$ExternalSyntheticApiModelOutline1.m1087m(this.manager.blurRenderNode);
                if (!renderNodeM1087m.hasDisplayList()) {
                    RenderNode renderNodeM1087m2 = BotFullscreenButtons$$ExternalSyntheticApiModelOutline1.m1087m(this.manager.renderNode);
                    renderNodeM1087m.setPosition(0, 0, renderNodeM1087m2.getWidth(), renderNodeM1087m2.getHeight());
                    RecordingCanvas recordingCanvasBeginRecording = renderNodeM1087m.beginRecording();
                    recordingCanvasBeginRecording.drawColor(getBackgroundColor());
                    recordingCanvasBeginRecording.drawRenderNode(renderNodeM1087m2);
                ...
                canvas.drawColor(getBackgroundColor());
                if (setupMatrix(renderNodeM1087m.getWidth(), renderNodeM1087m.getHeight(), true) && renderNodeM1087m.hasDisplayList()) {
                    this.matrix.postTranslate(-f, -f2);
                    this.paint.setAlpha((int) (f3 * 255.0f));
                    canvas.saveLayer(null, this.paint);
                    canvas.concat(this.matrix);
                    if (z) {
                        if (this.clipPathWidth != renderNodeM1087m.getWidth() || this.clipPathHeight != renderNodeM1087m.getHeight()) {
                            this.clipPath.rewind();
                            RectF rectF = AndroidUtilities.rectTmp;
                            int width = renderNodeM1087m.getWidth();
                            this.clipPathWidth = width;
                            int height = renderNodeM1087m.getHeight();
                            this.clipPathHeight = height;
                            rectF.set(0.0f, 0.0f, width, height);
                            this.clipPath.addRoundRect(rectF, AndroidUtilities.m1081dp(12.0f), AndroidUtilities.m1081dp(12.0f), Path.Direction.CW);
                        }
