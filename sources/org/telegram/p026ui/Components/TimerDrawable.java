...
public class TimerDrawable extends Drawable {
    ...
    public TimerDrawable(Context context, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.timePaint.setTypeface(AndroidUtilities.getTypeface(AndroidUtilities.TYPEFACE_ROBOTO_CONDENSED_BOLD));
        this.linePaint.setStrokeWidth(AndroidUtilities.m1081dp(1.0f));
    ...
    public void setTime(int i) {
        ...
        if (this.time != i) {
            this.time = i;
            if (this.isDialog) {
                this.currentTtlIcon = ContextCompat.getDrawable(this.context, C2702R.drawable.msg_autodelete_badge2).mutate();
            } else {
                Drawable drawableMutate = ContextCompat.getDrawable(this.context, i == 0 ? C2702R.drawable.msg_mini_autodelete : C2702R.drawable.msg_mini_autodelete_empty).mutate();
            ...
            if (i2 >= 1 && i2 < 60) {
                ...
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerSeconds);
                }
            } else if (i2 >= 60 && i2 < 3600) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + (i / 60);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerMinutes);
                }
            } else if (i2 >= 3600 && i2 < 86400) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((i / 60) / 60);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerHours);
                }
            } else if (i2 >= 86400 && i2 < 604800) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + (((i / 60) / 60) / 24);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerDays);
                }
            } else if (i2 < 2678400) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((((i / 60) / 60) / 24) / 7);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerWeeks);
                } else if (str.length() > 2) {
                    str = "c";
                }
            } else if (i2 < 31449600) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((((i / 60) / 60) / 24) / 30);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerMonths);
                }
            } else {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((((i / 60) / 60) / 24) / 364);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2702R.string.SecretChatTimerYears);
                }
            }
            String str2 = str;
            this.timePaint.setTextSize(AndroidUtilities.m1081dp(11.0f));
            float fMeasureText = this.timePaint.measureText(str2);
            this.timeWidth = fMeasureText;
            if (fMeasureText > AndroidUtilities.m1081dp(13.0f)) {
                this.timePaint.setTextSize(AndroidUtilities.m1081dp(9.0f));
                this.timeWidth = this.timePaint.measureText(str2);
            }
            if (this.timeWidth > AndroidUtilities.m1081dp(13.0f)) {
                this.timePaint.setTextSize(AndroidUtilities.m1081dp(6.0f));
                this.timeWidth = this.timePaint.measureText(str2);
            }
            try {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.currentTtlIcon != null) {
            ...
            if (this.isDialog) {
                ...
                rect.set(getBounds().centerX() - AndroidUtilities.m1081dp(10.5f), getBounds().centerY() - AndroidUtilities.m1081dp(10.5f), (getBounds().centerX() - AndroidUtilities.m1081dp(10.5f)) + this.currentTtlIcon.getIntrinsicWidth(), (getBounds().centerY() - AndroidUtilities.m1081dp(10.5f)) + this.currentTtlIcon.getIntrinsicHeight());
        ...
        if (this.isDialog) {
            ...
            canvas.translate(((int) (((double) (intrinsicWidth / 2)) - Math.ceil(this.timeWidth / 2.0f))) + i, (intrinsicHeight - this.timeHeight) / 2.0f);
            this.timeLayout.draw(canvas);
        }
        canvas.restore();
    }
    ...
    public int getIntrinsicWidth() {
        return AndroidUtilities.m1081dp(23.0f);
    }
