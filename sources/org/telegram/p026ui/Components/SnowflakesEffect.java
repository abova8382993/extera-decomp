...
public class SnowflakesEffect {
    ...
    private class Particle {
        ...
        public void draw(Canvas canvas) {
            if (this.type == 0) {
                ...
                canvas.drawPoint(this.f2040x, this.f2041y, this.paintType == 0 ? SnowflakesEffect.this.particlePaint : SnowflakesEffect.this.particleThinPaint);
                return;
            }
            ...
            canvas.save();
            float f = this.scale;
            canvas.scale(f, f, this.f2040x, this.f2041y);
            SnowflakesEffect snowflakesEffect2 = SnowflakesEffect.this;
            canvas.drawBitmap(snowflakesEffect2.particleBitmap, this.f2040x, this.f2041y, snowflakesEffect2.bitmapPaint);
            canvas.restore();
        }
    }
    ...
    public SnowflakesEffect(int i) {
        ...
        paint.setStrokeWidth(AndroidUtilities.m1081dp(2.5f));
        ...
        paint2.setStrokeWidth(AndroidUtilities.m1081dp(1.0f));
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f2040x;
                float f4 = particle.f2038vx;
                float f5 = particle.velocity;
                float f6 = j;
                particle.f2040x = f3 + (((f4 * f5) * f6) / 500.0f);
                particle.f2041y += ((particle.f2039vy * f5) * f6) / 500.0f;
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (this.batchParticlesBuffer != null) {
            ...
            for (int i = 0; i < iMin; i++) {
                ...
                float f = particle2.f2040x;
                float f2 = particle2.f2041y;
                int i2 = particle2.type;
                float f3 = iM1081dp / 2.0f;
                if (i2 != 0) {
                    f3 *= particle2.scale;
                }
                float f4 = i2 == 0 ? iM1081dp : 0.0f;
        ...
        if (this.particles.size() < this.maxCount) {
            for (int i5 = 0; i5 < i4; i5++) {
                if (this.particles.size() < this.maxCount && Utilities.random.nextFloat() > 0.7f) {
                    ...
                    if (this.viewType == 0) {
                        measuredHeight = i6 + (Utilities.random.nextFloat() * ((view.getMeasuredHeight() - AndroidUtilities.m1081dp(20.0f)) - i6));
                    } else {
                    ...
                    particle.f2040x = fNextFloat;
                    particle.f2041y = measuredHeight;
                    particle.f2038vx = fCos;
                    particle.f2039vy = fSin;
                    particle.alpha = 0.0f;
    ...
    public static Bitmap createParticlesBitmap(boolean z) {
        Paint paint = new Paint(1);
        paint.setStrokeWidth(AndroidUtilities.m1081dp(0.5f));
        paint.setStrokeCap(Paint.Cap.ROUND);
        paint.setStyle(Paint.Style.STROKE);
        paint.setColor(-1);
        Bitmap bitmapCreateBitmap = Bitmap.createBitmap(z ? AndroidUtilities.m1081dp(20.0f) : AndroidUtilities.m1081dp(10.0f), AndroidUtilities.m1081dp(10.0f), Bitmap.Config.ARGB_8888);
        ...
        while (i < 6) {
            ...
            float f6 = fCos * 0.66f;
            float f7 = 0.66f * fSin;
            canvas.drawLine(fM1081dp, fM1081dp2, fCos + fM1081dp, fSin + fM1081dp2, paint);
            float f8 = fM1081dp;
            float f9 = fM1081dp2;
        ...
        if (z) {
            Paint paint2 = new Paint(1);
            paint2.setStrokeWidth(AndroidUtilities.m1081dp(1.5f));
            paint2.setStrokeCap(Paint.Cap.ROUND);
            paint2.setStyle(Paint.Style.STROKE);
            paint2.setColor(-1);
            canvas.drawPoint(AndroidUtilities.m1081dp(15.0f), AndroidUtilities.m1081dp(f12), paint2);
        }
        return bitmap2;
    }
}
