package org.telegram.p026ui.Charts.data;
...
public class StackLinearChartData extends ChartData {
    ...
    public StackLinearChartData(JSONObject jSONObject, boolean z) {
        ...
        if (z) {
            ...
            for (int i = 0; i < this.lines.size(); i++) {
                int length = this.f1864x.length;
                for (int i2 = 0; i2 < length; i2++) {
                    long j2 = ((ChartData.Line) this.lines.get(i)).f1866y[i2];
                    jArr[i] = jArr[i] + j2;
                    if (j2 == 0) {
                        iArr[i] = iArr[i] + 1;
                    }
            ...
            for (int i3 = 0; i3 < this.lines.size(); i3++) {
                if (jArr[i3] / j < 0.01d && iArr[i3] > this.f1864x.length / 2.0f) {
        ...
        int length2 = ((ChartData.Line) this.lines.get(0)).f1866y.length;
        ...
        for (int i5 = 0; i5 < length2; i5++) {
            ...
            for (int i6 = 0; i6 < size2; i6++) {
                ...
                jArr2[i5] = jArr2[i5] + ((ChartData.Line) this.lines.get(i6)).f1866y[i5];
            }
        }
    ...
    public StackLinearChartData(ChartData chartData, long j) {
        int iBinarySearch = Arrays.binarySearch(chartData.f1864x, j);
        ...
        while (length <= length2) {
            this.f1864x[i3] = chartData.f1864x[length];
            for (int i4 = 0; i4 < this.lines.size(); i4++) {
                ((ChartData.Line) this.lines.get(i4)).f1866y[i3] = ((ChartData.Line) chartData.lines.get(i4)).f1866y[length];
            }
            i3++;
            length++;
        }
    ...
    @Override // org.telegram.p026ui.Charts.data.ChartData
    protected void measure() {
        ...
        for (int i2 = 0; i2 < length; i2++) {
            for (int i3 = 0; i3 < size; i3++) {
                long j = ((ChartData.Line) this.lines.get(i3)).f1866y[i2];
                if (j > jArr[i3]) {
                    jArr[i3] = j;
                }
            }
