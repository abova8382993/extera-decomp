    ...
    public void queryServerSearch(final String str, boolean z, final boolean z2, final boolean z3, boolean z4, final boolean z5, long j, boolean z6, int i, final int i2, final long j2, final Runnable runnable) {
        ...
        if (str.length() > 0) {
            if (j != 0) {
                ...
                tL_channels_getParticipants.filter.f1609q = str;
        ...
        if (!z5 && z6 && str.startsWith("+") && str.length() > 3) {
            ...
            for (int i5 = 0; i5 < size2; i5++) {
                ...
                if (user != null && (str2 = user.phone) != null && str2.startsWith(strStripExceptNumbers)) {
                    ...
                    this.phoneSearchMap.put(user.f1775id, user);
                }
            }
            if (!z9) {
                ...
                for (int i5 = 0; i5 < arrayList.size(); i5++) {
                    ...
                    if (chat2 != null) {
                        if (z && ((!z2 || ChatObject.canAddBotsToChat(chat2)) && ((this.allowGlobalResults || !ChatObject.isNotInChat(chat2)) && filter(chat2)))) {
                            ...
                            this.globalSearchMap.put(-chat2.f1610id, chat2);
                        }
                        ...
                        this.globalSearchMap.put(user2.f1775id, user2);
                    }
                }
            }
            if (!this.allResultsAreGlobal) {
                for (int i6 = 0; i6 < tL_contacts_found.my_results.size(); i6++) {
                    ...
                    if (chat != null) {
                        if (z && ((!z2 || ChatObject.canAddBotsToChat(chat)) && (-chat.f1610id) != j && filter(chat))) {
                            this.localServerSearch.add(chat);
                            this.globalSearchMap.put(-chat.f1610id, chat);
                        }
                    } else if (user != null && !z2 && ((z3 || !user.bot) && ((z4 || !user.self) && user.f1775id != j && filter(user)))) {
                        this.localServerSearch.add(user);
                        this.globalSearchMap.put(user.f1775id, user);
                    }
                }
            }
    ...
    private void removeGroupSearchFromGlobal() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (user != null) {
                ...
                this.globalSearchMap.remove(user.f1775id);
            }
        }
    }
        ...
        if (this.globalSearchMap.size() != 0) {
            ...
            while (i < size2) {
                ...
                if (obj instanceof TLRPC.User) {
                    ...
                    TLRPC.User user2 = (TLRPC.User) this.globalSearchMap.get(user.f1775id);
                    if (user2 != null) {
                        this.globalSearch.remove(user2);
                        this.localServerSearch.remove(user2);
                        this.globalSearchMap.remove(user2.f1775id);
                    }
                    TLObject tLObject = (TLObject) this.groupSearchMap.get(user.f1775id);
                    if (tLObject != null) {
                        this.groupSearch.remove(tLObject);
                        this.groupSearchMap.remove(user.f1775id);
                    }
                    Object obj2 = this.phoneSearchMap.get(user.f1775id);
                    if (obj2 != null) {
                        this.phonesSearch.remove(obj2);
                        this.phoneSearchMap.remove(user.f1775id);
                    }
                } else if ((obj instanceof TLRPC.Chat) && (chat = (TLRPC.Chat) this.globalSearchMap.get(-((TLRPC.Chat) obj).f1610id)) != null) {
                    this.globalSearch.remove(chat);
                    this.localServerSearch.remove(chat);
                    this.globalSearchMap.remove(-chat.f1610id);
                }
                i++;
            }
        }
