...
public class StoryContainsEmojiButton extends View {
    ...
    protected void onMeasure(int i, int i2) {
        ...
        int iM1124dp = AndroidUtilities.m1124dp(29.0f);
        StaticLayout staticLayout = this.layout;
        setMeasuredDimension(z ? View.MeasureSpec.getSize(i) : getMinimumWidth(), paddingTop + AndroidUtilities.lerp(iM1124dp, staticLayout == null ? AndroidUtilities.m1124dp(29.0f) : staticLayout.getHeight(), this.loadT) + getPaddingBottom());
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (f < 1.0f) {
            ...
            float paddingTop2 = getPaddingTop() + AndroidUtilities.m1124dp(12.0f);
            Path.Direction direction = Path.Direction.CW;
            path.addRect(paddingLeft, paddingTop, measuredWidth, paddingTop2, direction);
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop() + AndroidUtilities.m1124dp(16.0f), getPaddingLeft() + (((getMeasuredWidth() - getPaddingRight()) - getPaddingLeft()) * 0.46f), getPaddingTop() + AndroidUtilities.m1124dp(28.0f), direction);
        ...
        if (tLObject instanceof Vector) {
            ...
            if (this.inputSets != null && arrayList != null && !arrayList.isEmpty()) {
                for (int i3 = 0; i3 < arrayList.size(); i3++) {
                    ...
                    long j = inputStickerSet.f1685id;
                    int i4 = 0;
                    while (true) {
                        if (i4 >= this.inputSets.size()) {
                            this.inputSets.add(inputStickerSet);
                            break;
                        } else if (((TLRPC.InputStickerSet) this.inputSets.get(i4)).f1685id == j) {
                            break;
                        } else {
                            i4++;
                        }
    ...
    private void set(TLRPC.TL_messages_stickerSet tL_messages_stickerSet) {
        ...
        while (true) {
            if (i >= arrayList.size()) {
                ...
                if (((TLRPC.Document) arrayList.get(i)).f1668id == tL_messages_stickerSet.set.thumb_document_id) {
        ...
        if (z && this.stickers) {
            string = LocaleController.getString(C2888R.string.StoryContainsStickersEmojiFrom);
        } else if (z) {
            string = LocaleController.getString(C2888R.string.StoryContainsEmojiFrom);
        } else {
            string = LocaleController.getString(C2888R.string.StoryContainsStickersFrom);
        }
    ...
    private void set(TLRPC.StickerSetCovered stickerSetCovered) {
        ...
        if (document == null && (stickerSetCovered instanceof TLRPC.TL_stickerSetFullCovered)) {
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                if (((TLRPC.Document) arrayList.get(i)).f1668id == stickerSetCovered.set.thumb_document_id) {
                    document = (TLRPC.Document) arrayList.get(i);
                }
            }
            if (document == null && !arrayList.isEmpty()) {
    ...
    private void animateLoad(boolean z) {
        ...
        if (z) {
            ...
            final boolean z2 = this.layout == null || Math.abs(getMeasuredHeight() - ((getPaddingTop() + this.layout.getHeight()) + getPaddingBottom())) > AndroidUtilities.m1124dp(3.0f);
