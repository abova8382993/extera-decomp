...
public class RequestPeerRequirementsCell extends LinearLayout {
    ...
    public void set(TLRPC.RequestPeerType requestPeerType) {
        if (this.requestPeerType != requestPeerType) {
            ...
            if (requestPeerType instanceof TLRPC.TL_requestPeerTypeUser) {
                checkRequirement(((TLRPC.TL_requestPeerTypeUser) requestPeerType).premium, C2888R.string.PeerRequirementPremiumTrue, C2888R.string.PeerRequirementPremiumFalse);
            } else {
                boolean z = requestPeerType instanceof TLRPC.TL_requestPeerTypeBroadcast;
                if (z) {
                    checkRequirement(requestPeerType.has_username, C2888R.string.PeerRequirementChannelPublicTrue, C2888R.string.PeerRequirementChannelPublicFalse);
                    Boolean bool = requestPeerType.bot_participant;
                    if (bool != null && bool.booleanValue()) {
                        this.requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(C2888R.string.PeerRequirementChannelBotParticipant))));
                    }
                    Boolean bool2 = requestPeerType.creator;
                    if (bool2 != null && bool2.booleanValue()) {
                        this.requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(C2888R.string.PeerRequirementChannelCreatorTrue))));
                    }
                } else {
                    checkRequirement(requestPeerType.has_username, C2888R.string.PeerRequirementGroupPublicTrue, C2888R.string.PeerRequirementGroupPublicFalse);
                    checkRequirement(requestPeerType.forum, C2888R.string.PeerRequirementForumTrue, C2888R.string.PeerRequirementForumFalse);
                    Boolean bool3 = requestPeerType.bot_participant;
                    if (bool3 != null && bool3.booleanValue()) {
                        this.requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(C2888R.string.PeerRequirementGroupBotParticipant))));
                    }
                    Boolean bool4 = requestPeerType.creator;
                    if (bool4 != null && bool4.booleanValue()) {
                        this.requirements.add(Requirement.make(AndroidUtilities.replaceTags(LocaleController.getString(C2888R.string.PeerRequirementGroupCreatorTrue))));
                    }
                }
                Boolean bool5 = requestPeerType.creator;
                if (bool5 == null || !bool5.booleanValue()) {
                    checkAdminRights(requestPeerType.user_admin_rights, z, C2888R.string.PeerRequirementUserRights, C2888R.string.PeerRequirementUserRight);
                }
            }
            ...
            headerCell.setText(LocaleController.getString(C2888R.string.PeerRequirements));
            ...
            addView(emptyView(12, Theme.getThemedDrawableByKey(getContext(), C2888R.drawable.greydivider_bottom, Theme.key_windowBackgroundGrayShadow)), LayoutHelper.createLinear(-1, -2));
        }
    }
    ...
    private View emptyView(final int i, Drawable drawable) {
        View view = new View(getContext()) { // from class: org.telegram.ui.Cells.RequestPeerRequirementsCell.1
            ...
            protected void onMeasure(int i2, int i3) {
                super.onMeasure(i2, View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m1124dp(i), TLObject.FLAG_30));
            }
        };
    ...
    public static CharSequence rightsToString(TLRPC.TL_chatAdminRights tL_chatAdminRights, boolean z) {
        ...
        if (tL_chatAdminRights.change_info) {
            if (z) {
                string = LocaleController.getString(C2888R.string.EditAdminChangeChannelInfo);
            } else {
                string = LocaleController.getString(C2888R.string.EditAdminChangeGroupInfo);
            }
            arrayList.add(Requirement.make(1, string));
        }
        if (tL_chatAdminRights.post_messages && z) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminPostMessages)));
        }
        if (tL_chatAdminRights.edit_messages && z) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminEditMessages)));
        }
        if (tL_chatAdminRights.delete_messages) {
            arrayList.add(Requirement.make(1, LocaleController.getString(z ? C2888R.string.EditAdminDeleteMessages : C2888R.string.EditAdminGroupDeleteMessages)));
        }
        if (tL_chatAdminRights.ban_users && !z) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminBanUsers)));
        }
        if (tL_chatAdminRights.invite_users) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminAddUsers)));
        }
        if (tL_chatAdminRights.pin_messages && !z) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminPinMessages)));
        }
        if (tL_chatAdminRights.add_admins) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminAddAdmins)));
        }
        if (tL_chatAdminRights.anonymous && !z) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.EditAdminSendAnonymously)));
        }
        if (tL_chatAdminRights.manage_call) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.StartVoipChatPermission)));
        }
        if (tL_chatAdminRights.manage_topics && !z) {
            arrayList.add(Requirement.make(1, LocaleController.getString(C2888R.string.ManageTopicsPermission)));
        }
        if (arrayList.size() == 1) {
    ...
    class RequirementCell extends LinearLayout {
        ...
        public RequirementCell(Context context, Requirement requirement) {
            ...
            this.imageView.setImageResource(requirement.padding <= 0 ? C2888R.drawable.list_check : C2888R.drawable.list_circle);
