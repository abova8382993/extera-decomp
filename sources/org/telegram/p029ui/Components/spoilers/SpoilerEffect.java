...
public class SpoilerEffect extends Drawable {
    ...
    public SpoilerEffect() {
        ...
        for (int i = 0; i < ALPHAS.length; i++) {
            ...
            if (i == 0) {
                this.particlePaints[i].setStrokeWidth(AndroidUtilities.m1124dp(1.4f));
                this.particlePaints[i].setStyle(Paint.Style.STROKE);
                this.particlePaints[i].setStrokeCap(Paint.Cap.ROUND);
            } else {
                this.particlePaints[i].setStrokeWidth(AndroidUtilities.m1124dp(1.2f));
                this.particlePaints[i].setStyle(Paint.Style.STROKE);
                this.particlePaints[i].setStrokeCap(Paint.Cap.ROUND);
            }
        }
    ...
    public void updateMaxParticles() {
        int iWidth = getBounds().width() / AndroidUtilities.m1124dp(6.0f);
    ...
    public void setBounds(int i, int i2, int i3, int i4) {
        ...
        while (it.hasNext()) {
            Particle particle = (Particle) it.next();
            if (!getBounds().contains((int) particle.f2110x, (int) particle.f2111y)) {
                it.remove();
            }
  ...
  0x024b: PHI (r4v9 int) = (r4v8 int), (r4v11 int) binds: [B:79:0x022b, B:83:0x0235] A[DONT_GENERATE, DONT_INLINE]] */
    ...
    */
    public void draw(android.graphics.Canvas r18) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p029ui.Components.spoilers.SpoilerEffect.draw(android.graphics.Canvas):void");
    }
    ...
    private boolean isOutOfBounds(int i, int i2, int i3, int i4, float f, float f2) {
        if (f < i || f > i3 || f2 < i2 + AndroidUtilities.m1124dp(2.5f) || f2 > i4 - AndroidUtilities.m1124dp(2.5f)) {
            return true;
        }
    ...
    private void generateRandomLocation(Particle particle, int i) {
        particle.f2110x = getBounds().left + (Utilities.fastRandom.nextFloat() * getBounds().width());
        particle.f2111y = getBounds().top + (Utilities.fastRandom.nextFloat() * getBounds().height());
    }
    ...
    public static void renderWithRipple(View view, boolean z, int i, int i2, AtomicReference atomicReference, int i3, Layout layout, List list, Canvas canvas, boolean z2) {
        ...
        if (layoutBuild == null || !layout.getText().toString().equals(layoutBuild.getText().toString()) || layout.getWidth() != layoutBuild.getWidth() || layout.getHeight() != layoutBuild.getHeight()) {
            ...
            if (i3 == 1) {
                staticLayout = new StaticLayout(spannableStringBuilder, layout.getPaint(), layout.getWidth(), Layout.Alignment.ALIGN_CENTER, 1.0f, AndroidUtilities.m1124dp(1.66f), false);
