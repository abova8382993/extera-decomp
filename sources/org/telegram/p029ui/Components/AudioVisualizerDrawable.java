...
public class AudioVisualizerDrawable {
    ...
    private final float[] f1923dt = new float[8];
    private final Random random = new Random();
    public float IDLE_RADIUS = AndroidUtilities.m1124dp(6.0f) * 0.33f;
    public float WAVE_RADIUS = AndroidUtilities.m1124dp(12.0f) * 0.36f;
    ...
    public AudioVisualizerDrawable() {
        for (int i = 0; i < 2; i++) {
            ...
            circleBezierDrawable.radius = AndroidUtilities.m1124dp(24.0f);
            circleBezierDrawable.radiusDiff = 0.0f;
            circleBezierDrawable.randomK = 1.0f;
        }
        this.f1924p1 = new Paint(1);
    }
        ...
        if (LiteMode.isEnabled(32)) {
            ...
            while (i < 7) {
                ...
                if (weakReference == null || weakReference.get() == null) {
                    ...
                    this.f1923dt[i] = (this.animateTo[i] - this.current[i]) / (this.ANIMATION_DURATION + 80.0f);
                } else {
                    this.f1923dt[i] = (this.animateTo[i] - this.current[i]) / f2;
                }
                i++;
            }
    ...
    public void draw(Canvas canvas, float f, float f2, boolean z, float f3, Theme.ResourcesProvider resourcesProvider) {
        if (LiteMode.isEnabled(32)) {
            if (z) {
                this.f1924p1.setColor(Theme.getColor(Theme.key_chat_outLoader, resourcesProvider));
                this.f1924p1.setAlpha((int) (this.ALPHA * f3));
            } else {
                this.f1924p1.setColor(Theme.getColor(Theme.key_chat_inLoader, resourcesProvider));
                this.f1924p1.setAlpha((int) (this.ALPHA * f3));
            }
            draw(canvas, f, f2);
        }
    ...
    public void draw(Canvas canvas, float f, float f2) {
        if (LiteMode.isEnabled(32)) {
            ...
            float fM1124dp = AndroidUtilities.m1124dp(22.0f) + (AndroidUtilities.m1124dp(4.0f) * f10) + (this.IDLE_RADIUS * f9);
            if (fM1124dp > AndroidUtilities.m1124dp(26.0f)) {
                fM1124dp = AndroidUtilities.m1124dp(26.0f);
            }
            CircleBezierDrawable[] circleBezierDrawableArr = this.drawables;
            CircleBezierDrawable circleBezierDrawable = circleBezierDrawableArr[0];
            circleBezierDrawableArr[1].radius = fM1124dp;
            circleBezierDrawable.radius = fM1124dp;
            canvas.save();
            ...
            this.drawables[0].draw(f, f2, canvas, this.f1924p1);
            canvas.restore();
            canvas.rotate(60.0f, f, f2);
            float f13 = ((1.0f - this.idleScale) * 0.04f) + 1.0f;
            canvas.scale(f13, f13, f, f2);
            this.drawables[1].draw(f, f2, canvas, this.f1924p1);
            canvas.restore();
        }
    }
