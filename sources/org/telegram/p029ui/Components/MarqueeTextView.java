...
public class MarqueeTextView extends TextView {
    ...
    private void invalidateGradient() {
        float fMin = Math.min(AndroidUtilities.m1124dp(10.0f) / this.originalWidth, 0.49f);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float fClamp = f < f2 ? MathUtils.clamp(f / AndroidUtilities.m1124dp(10.0f), 0.0f, 1.0f) : 0.0f;
        this.gradientMatrix.reset();
        Matrix matrix = this.gradientMatrix;
        float fM1124dp = AndroidUtilities.m1124dp(10.0f);
        int i = this.originalWidth;
        matrix.postScale(((fM1124dp / i) * (1.0f - fClamp)) + 1.0f, 1.0f, i, 0.0f);
        ...
        if ((this.needMarquee && this.marqueeIsStarted) || !z) {
            float fM1124dp2 = this.scrollX + (AndroidUtilities.m1124dp(60.0f) * (jMin / 1000.0f));
            this.scrollX = fM1124dp2;
            if (fM1124dp2 > measuredWidth + iM1124dp) {
                stopMarqueeInternal();
            }
            invalidate();
        }
