...
public class RadioButton extends View {
    ...
    public RadioButton(Context context) {
        super(context);
        this.size = AndroidUtilities.m1124dp(16.0f);
        if (paint == null) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (f2 <= 0.5f) {
            ...
            f = 2.0f - (f2 / 0.5f);
            float f3 = 1.0f - f;
            int iRgb = Color.rgb(Color.red(this.color) + ((int) ((Color.red(this.checkedColor) - r2) * f3)), Color.green(this.color) + ((int) ((Color.green(this.checkedColor) - r7) * f3)), Color.blue(this.color) + ((int) ((Color.blue(this.checkedColor) - r9) * f3)));
            paint.setColor(iRgb);
        ...
        if (this.icon == null) {
            if (this.progress <= 0.5f) {
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, f4 - AndroidUtilities.m1124dp(1.0f), checkedPaint);
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, (f4 - AndroidUtilities.m1124dp(1.0f)) * (1.0f - f), eraser);
            } else {
                canvas.drawCircle(getMeasuredWidth() / 2, getMeasuredHeight() / 2, (this.size / 4) + (((f4 - AndroidUtilities.m1124dp(1.0f)) - (this.size / 4)) * f), checkedPaint);
            }
        }
        canvas.restore();
        if (this.icon != null) {
