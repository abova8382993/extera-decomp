...
public class Input {
    ...
    private void fill(Brush brush, final boolean z, final Runnable runnable) {
        ...
        float fDistance = MathUtils.distance((float) point.f1981x, (float) point.f1982y, 0.0f, 0.0f);
        Point point2 = this.lastLocation;
        float fMax = Math.max(fDistance, MathUtils.distance((float) point2.f1981x, (float) point2.f1982y, size.width, 0.0f));
        Point point3 = this.lastLocation;
        float fDistance2 = MathUtils.distance((float) point3.f1981x, (float) point3.f1982y, 0.0f, size.height);
        Point point4 = this.lastLocation;
        final float fMax2 = Math.max(fMax, Math.max(fDistance2, MathUtils.distance((float) point4.f1981x, (float) point4.f1982y, size.width, size.height))) / 0.84f;
        ...
        final Point point6 = new Point(point5.f1981x, point5.f1982y, 1.0d);
  ...
  0x0094: PHI (r6v7 float) = (r6v6 float), (r6v35 float) binds: [B:13:0x0076, B:15:0x0090] A[DONT_GENERATE, DONT_INLINE]] */
    ...
    */
    public void process(android.view.MotionEvent r23, float r24) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p029ui.Components.Paint.Input.process(android.view.MotionEvent, float):void");
    }
    ...
    public /* synthetic */ void lambda$process$2(float f, Point point, float f2, float[] fArr, double d, boolean[] zArr, ValueAnimator valueAnimator) {
        ...
        double d3 = point.f1981x;
        double d4 = f2;
        double d5 = dCos * d4;
        float f3 = fArr[0];
        double d6 = dSin * d4;
        Point point2 = new Point(d3 + (((double) f3) * d5), point.f1982y + (((double) f3) * d6), d);
        double d7 = fFloatValue;
        paintPath(new Path(new Point[]{point2, new Point(point.f1981x + (d5 * d7), point.f1982y + (d6 * d7), d, true)}));
        double dCos2 = Math.cos(d2 + 2.5918139392115793d);
        double dSin2 = Math.sin(d2 + 2.748893571891069d);
        double d8 = point.f1981x;
        double d9 = dCos2 * d4;
        float f4 = fArr[0];
        double d10 = dSin2 * d4;
        paintPath(new Path(new Point[]{new Point(d8 + (((double) f4) * d9), point.f1982y + (((double) f4) * d10), d), new Point(point.f1981x + (d9 * d7), point.f1982y + (d10 * d7), d, true)}));
    ...
    private void smoothenAndPaintPoints(boolean z, float f) {
        ...
        if (i > 2) {
            ...
            while (i2 < iMin) {
                ...
                this.thicknessSum += pointSmoothPoint.f1983z;
    ...
    private Point smoothPoint(Point point, Point point2, Point point3, float f, float f2) {
        ...
        double d3 = f3 * f3;
        double d4 = f;
        return new Point((point.f1981x * d3) + (point3.f1981x * 2.0d * d4 * d) + (point2.f1981x * d2), (point.f1982y * d3) + (point3.f1982y * 2.0d * d4 * d) + (point2.f1982y * d2), (((((point.f1983z * Math.pow(d, 2.0d)) + (point3.f1983z * ((double) ((2.0f * f3) * f)))) + (point2.f1983z * d2)) - 1.0d) * ((double) AndroidUtilities.lerp(f2, 1.0f, androidx.core.math.MathUtils.clamp(this.realPointsCount / 16.0f, 0.0f, 1.0f)))) + 1.0d);
    }
