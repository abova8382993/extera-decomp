...
public class FireworksEffect {
    ...
    private class Particle {
        ...
        public void draw(Canvas canvas) {
            ...
            FireworksEffect.this.particlePaint.setStrokeWidth(AndroidUtilities.m1124dp(1.5f) * this.scale);
            FireworksEffect.this.particlePaint.setAlpha((int) (this.alpha * 255.0f));
            canvas.drawPoint(this.f1963x, this.f1964y, FireworksEffect.this.particlePaint);
        }
    }
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1963x;
                float f4 = particle.f1961vx;
                float f5 = particle.velocity;
                float f6 = j;
                particle.f1963x = f3 + (((f4 * f5) * f6) / 500.0f);
                float f7 = particle.f1964y;
                float f8 = particle.f1962vy;
                particle.f1964y = f7 + (((f5 * f8) * f6) / 500.0f);
                particle.f1962vy = f8 + (f6 / 100.0f);
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (Utilities.random.nextBoolean() && this.particles.size() + 8 < 150) {
            ...
            float fNextFloat2 = i2 + (Utilities.random.nextFloat() * ((view.getMeasuredHeight() - AndroidUtilities.m1124dp(20.0f)) - i2));
            ...
            for (int i4 = 0; i4 < 8; i4++) {
                ...
                particle.f1963x = fNextFloat;
                particle.f1964y = fNextFloat2;
                particle.f1961vx = fCos * 1.5f;
                particle.f1962vy = fSin;
                particle.color = i3;
                particle.alpha = 1.0f;
