...
public class ChatPullingDownDrawable implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ChatPullingDownDrawable(int i, View view, long j, int i2, int i3, long j2, Theme.ResourcesProvider resourcesProvider) {
        ...
        this.textPaint2.setTextSize(AndroidUtilities.m1124dp(14.0f));
    ...
    public void updateDialog(TLRPC.Chat chat) {
        ...
        this.nextDialogId = -chat.f1660id;
        ...
        MessagesController.getInstance(this.currentAccount).ensureMessagesLoaded(-chat.f1660id, 0, null);
        TLRPC.Dialog dialog = MessagesController.getInstance(this.currentAccount).getDialog(-chat.f1660id);
    ...
    public void updateDialog() {
        ...
        if (nextUnreadDialog != null) {
            this.nextDialogId = nextUnreadDialog.f1666id;
            int[] iArr = this.params;
            this.drawFolderBackground = iArr[0] == 1;
            this.dialogFolderId = iArr[1];
            this.dialogFilterId = iArr[2];
            this.emptyStub = false;
            TLRPC.Chat chat = MessagesController.getInstance(this.currentAccount).getChat(Long.valueOf(-nextUnreadDialog.f1666id));
            this.nextChat = chat;
            if (chat == null) {
                this.nextChat = MessagesController.getInstance(this.currentAccount).getChat(Long.valueOf(nextUnreadDialog.f1666id));
            }
            AvatarDrawable avatarDrawable = new AvatarDrawable();
            avatarDrawable.setInfo(this.currentAccount, this.nextChat);
            this.imageReceiver.setImage(ImageLocation.getForChat(this.nextChat, 1), "50_50", avatarDrawable, null, UserConfig.getInstance(0).getCurrentUser(), 0);
            MessagesController.getInstance(this.currentAccount).ensureMessagesLoaded(nextUnreadDialog.f1666id, 0, null);
    ...
    public void setWidth(int i) {
        ...
        if (i == this.lastWidth && (!this.isTopic || (tL_forumTopic = this.nextTopic) == null || this.lastWidthTopicId == tL_forumTopic.f1720id)) {
            return;
        }
        this.circleRadius = AndroidUtilities.m1124dp(56.0f) / 2.0f;
        this.lastWidth = i;
        ...
        if (chat != null) {
            ...
            if (tL_forumTopic2 != null) {
                ...
                string = LocaleController.formatString(C2888R.string.SwipeToGoNextTopicEnd, MessagesController.getInstance(this.currentAccount).getChat(Long.valueOf(-this.currentDialog)).title);
            } else {
                string = LocaleController.getString(C2888R.string.SwipeToGoNextChannelEnd);
            }
        }
        String str = string;
        int iMeasureText = (int) this.textPaint.measureText((CharSequence) str, 0, str.length());
        this.chatNameWidth = iMeasureText;
        int iMin = Math.min(iMeasureText, this.lastWidth - AndroidUtilities.m1124dp(60.0f));
        ...
        if (this.recommendedChannel) {
            string2 = LocaleController.getString(C2888R.string.SwipeToGoNextRecommendedChannel);
            string3 = LocaleController.getString(C2888R.string.ReleaseToGoNextRecommendedChannel);
        } else if (this.isTopic) {
            string2 = LocaleController.getString(C2888R.string.SwipeToGoNextUnreadTopic);
            string3 = LocaleController.getString(C2888R.string.ReleaseToGoNextUnreadTopic);
        } else {
            boolean z = this.drawFolderBackground;
            if (z && (i2 = this.dialogFolderId) != this.folderId && i2 != 0) {
                string2 = LocaleController.getString(C2888R.string.SwipeToGoNextArchive);
                string3 = LocaleController.getString(C2888R.string.ReleaseToGoNextArchive);
            } else if (z) {
                string2 = LocaleController.getString(C2888R.string.SwipeToGoNextFolder);
                string3 = LocaleController.getString(C2888R.string.ReleaseToGoNextFolder);
            } else {
                string2 = LocaleController.getString(C2888R.string.SwipeToGoNextChannel);
                string3 = LocaleController.getString(C2888R.string.ReleaseToGoNextChannel);
            }
        }
        String str2 = string2;
        int iMeasureText2 = (int) this.textPaint2.measureText(str2);
        this.layout1Width = iMeasureText2;
        this.layout1Width = Math.min(iMeasureText2, this.lastWidth - AndroidUtilities.m1124dp(60.0f));
        ...
        this.layout2Width = Math.min(iMeasureText3, this.lastWidth - AndroidUtilities.m1124dp(60.0f));
        this.layout2 = new StaticLayout(string3, this.textPaint2, this.layout2Width, alignment, 1.0f, 0.0f, false);
        this.imageReceiver.setImageCoords((this.lastWidth / 2.0f) - (AndroidUtilities.m1124dp(40.0f) / 2.0f), (AndroidUtilities.m1124dp(12.0f) + this.circleRadius) - (AndroidUtilities.m1124dp(40.0f) / 2.0f), AndroidUtilities.m1124dp(40.0f), AndroidUtilities.m1124dp(40.0f));
        this.imageReceiver.setRoundRadius(ExteraConfig.getAvatarCorners(40.0f));
        this.counterDrawable.setSize(AndroidUtilities.m1124dp(28.0f), AndroidUtilities.m1124dp(100.0f));
        if (this.isTopic) {
            this.lastWidthTopicId = this.nextTopic == null ? 0L : r14.f1720id;
        }
    }
    ...
    public void draw(Canvas canvas, View view, float f, float f2) {
        ...
        float fM1124dp = AndroidUtilities.m1124dp(110.0f) * f;
        if (fM1124dp < AndroidUtilities.m1124dp(8.0f)) {
            return;
        }
        float f9 = f < 0.2f ? 5.0f * f * f2 : f2;
        Theme.applyServiceShaderMatrix(this.lastWidth, view.getMeasuredHeight(), 0.0f, view.getMeasuredHeight() - fM1124dp);
        TextPaint textPaint = this.textPaint;
        ...
        float f11 = this.bounceProgress * (-AndroidUtilities.m1124dp(4.0f));
        if (this.emptyStub) {
            fM1124dp -= f11;
        }
        float f12 = fM1124dp / 2.0f;
        float fMax = Math.max(0.0f, Math.min(this.circleRadius, (f12 - (AndroidUtilities.m1124dp(16.0f) * f)) - AndroidUtilities.m1124dp(4.0f)));
        float fMax2 = ((Math.max(0.0f, Math.min(this.circleRadius * f, f12 - (AndroidUtilities.m1124dp(8.0f) * f))) * 2.0f) - AndroidUtilities.dp2(16.0f)) * (f4 - this.swipeToReleaseProgress);
        float fM1124dp2 = AndroidUtilities.m1124dp(56.0f);
        float f13 = this.swipeToReleaseProgress;
        float f14 = fMax2 + (fM1124dp2 * f13);
        if (f13 < f4 || this.emptyStub) {
            f5 = 36.0f;
            float f15 = -fM1124dp;
            float fM1124dp3 = ((-AndroidUtilities.m1124dp(8.0f)) * (f4 - this.swipeToReleaseProgress)) + (this.swipeToReleaseProgress * (AndroidUtilities.m1124dp(56.0f) + f15));
            RectF rectF = AndroidUtilities.rectTmp;
            f6 = f11;
            i = alpha3;
            rectF.set(f10 - fMax, f15, f10 + fMax, fM1124dp3);
            if (this.swipeToReleaseProgress <= 0.0f || this.emptyStub) {
                f7 = f4;
            } else {
                float fM1124dp4 = AndroidUtilities.m1124dp(16.0f) * this.swipeToReleaseProgress;
                rectF.inset(fM1124dp4, fM1124dp4);
                f7 = f4 - this.swipeToReleaseProgress;
            }
            drawBackground(canvas, rectF, f7);
            float fM1124dp5 = ((AndroidUtilities.m1124dp(24.0f) + f15) + (AndroidUtilities.m1124dp(8.0f) * (f4 - f))) - (AndroidUtilities.m1124dp(36.0f) * this.swipeToReleaseProgress);
            canvas.save();
            rectF.inset(AndroidUtilities.m1124dp(f4), AndroidUtilities.m1124dp(f4));
            canvas.clipRect(rectF);
            float f16 = this.swipeToReleaseProgress;
            if (f16 > 0.0f) {
                this.arrowPaint.setAlpha((int) ((f4 - f16) * 255.0f));
            }
            drawArrow(canvas, f10, fM1124dp5, AndroidUtilities.m1124dp(24.0f) * f);
            if (this.emptyStub) {
                float fDp2 = ((((-AndroidUtilities.m1124dp(8.0f)) - (AndroidUtilities.dp2(8.0f) * f)) - f14) * (f4 - this.swipeToReleaseProgress)) + ((f15 - AndroidUtilities.m1124dp(2.0f)) * this.swipeToReleaseProgress) + f6;
                this.arrowPaint.setAlpha(alpha4);
                canvas.save();
                canvas.scale(f, f, f10, AndroidUtilities.m1124dp(28.0f) + fDp2);
                drawCheck(canvas, f10, fDp2 + AndroidUtilities.m1124dp(28.0f));
                canvas.restore();
            }
            canvas.restore();
        } else {
        ...
        if (this.chatNameLayout == null || this.swipeToReleaseProgress <= 0.0f) {
            ...
            this.textPaint.setAlpha(i4);
            float fM1124dp6 = ((AndroidUtilities.m1124dp(20.0f) * (f4 - this.swipeToReleaseProgress)) - (AndroidUtilities.m1124dp(f5) * this.swipeToReleaseProgress)) + f6;
            RectF rectF2 = AndroidUtilities.rectTmp;
            int i5 = this.lastWidth;
            int i6 = this.chatNameWidth;
            rectF2.set((i5 - i6) / 2.0f, fM1124dp6, i5 - ((i5 - i6) / 2.0f), this.chatNameLayout.getHeight() + fM1124dp6);
            rectF2.inset(-AndroidUtilities.m1124dp(8.0f), -AndroidUtilities.m1124dp(4.0f));
            ...
            canvas.save();
            canvas.translate((this.lastWidth - this.chatNameWidth) / 2.0f, fM1124dp6);
            this.chatNameLayout.draw(canvas);
            canvas.restore();
        }
        if (!this.emptyStub && f14 > 0.0f) {
            float fDp22 = ((((-AndroidUtilities.m1124dp(8.0f)) - (AndroidUtilities.dp2(8.0f) * f)) - f14) * (f4 - this.swipeToReleaseProgress)) + (((-fM1124dp) + AndroidUtilities.m1124dp(4.0f)) * this.swipeToReleaseProgress) + f6;
            ...
            if (this.swipeToReleaseProgress > 0.0f && this.visibleCounterDrawable) {
                ...
                canvas.scale(f17, f17, AndroidUtilities.m1124dp(12.0f) + f10 + this.counterDrawable.getCenterX(), (fDp22 - AndroidUtilities.m1124dp(6.0f)) + AndroidUtilities.m1124dp(14.0f));
                canvas.translate(AndroidUtilities.m1124dp(12.0f) + f10, fDp22 - AndroidUtilities.m1124dp(6.0f));
                this.counterDrawable.updateBackgroundRect();
                this.counterDrawable.rectF.inset(-AndroidUtilities.m1124dp(2.0f), -AndroidUtilities.m1124dp(2.0f));
                ...
                canvas.scale(f18, f18, AndroidUtilities.m1124dp(12.0f) + f10 + this.counterDrawable.getCenterX(), (fDp22 - AndroidUtilities.m1124dp(6.0f)) + AndroidUtilities.m1124dp(14.0f));
                canvas.translate(AndroidUtilities.m1124dp(12.0f) + f10, fDp22 - AndroidUtilities.m1124dp(6.0f));
    ...
    private void drawCheck(Canvas canvas, float f, float f2) {
        if (this.animateCheck) {
            ...
            canvas.translate(f - AndroidUtilities.m1124dp(24.0f), f2 - AndroidUtilities.m1124dp(24.0f));
            float fM1124dp = AndroidUtilities.m1124dp(16.0f);
            float fM1124dp2 = AndroidUtilities.m1124dp(26.0f);
            float fM1124dp3 = AndroidUtilities.m1124dp(22.0f);
            float fM1124dp4 = AndroidUtilities.m1124dp(32.0f);
            float fM1124dp5 = AndroidUtilities.m1124dp(32.0f);
            float fM1124dp6 = AndroidUtilities.m1124dp(20.0f);
            float f8 = 1.0f - f6;
            canvas.drawLine(fM1124dp, fM1124dp2, (fM1124dp * f8) + (fM1124dp3 * f6), (f8 * fM1124dp2) + (f6 * fM1124dp4), this.arrowPaint);
            if (f7 > 0.0f) {
                float f9 = 1.0f - f7;
                canvas.drawLine(fM1124dp3, fM1124dp4, (fM1124dp3 * f9) + (fM1124dp5 * f7), (f9 * fM1124dp4) + (fM1124dp6 * f7), this.arrowPaint);
            }
            canvas.restore();
        }
    }
    ...
    private void drawArrow(Canvas canvas, float f, float f2, float f3) {
        ...
        canvas.scale(fDpf2, fDpf2, f, f2 - AndroidUtilities.m1124dp(20.0f));
        canvas.translate(f - AndroidUtilities.dp2(12.0f), f2 - AndroidUtilities.m1124dp(12.0f));
    ...
    public static TLRPC.Dialog getNextUnreadDialog(long j, int i, int i2, boolean z, int[] iArr) {
        ...
        for (int i3 = 0; i3 < dialogs.size(); i3++) {
            ...
            TLRPC.Chat chat = messagesController.getChat(Long.valueOf(-dialog.f1666id));
            if (chat != null && dialog.f1666id != j && dialog.unread_count > 0 && DialogObject.isChannel(dialog) && !chat.megagroup && !messagesController.isPromoDialog(dialog.f1666id, false) && messagesController.getRestrictionReason(chat.restriction_reason) == null) {
                return dialog;
            }
        }
        return null;
    ...
    private TLRPC.TL_forumTopic getNextUnreadTopic(long j) {
        ...
        if (topics != null && topics.size() > 1) {
            for (int i = 0; i < topics.size(); i++) {
                ...
                if (tL_forumTopic2.f1720id != this.topicId && !tL_forumTopic2.hidden && tL_forumTopic2.unread_count > 0 && (tL_forumTopic == null || ((message = tL_forumTopic2.topMessage) != null && (message2 = tL_forumTopic.topMessage) != null && message.date > message2.date))) {
    ...
    public void drawBottomPanel(Canvas canvas, int i, int i2, int i3) {
        ...
        if (this.layout1 != null) {
            ...
            if (f < 1.0f) {
                ...
                float height = (i + (((i2 - i) - this.layout1.getHeight()) / 2.0f)) - (AndroidUtilities.m1124dp(10.0f) * this.swipeToReleaseProgress);
                canvas.save();
        ...
        if (this.layout2 != null) {
            ...
            if (f2 > 0.0f) {
                ...
                float height2 = i + (((i2 - i) - this.layout2.getHeight()) / 2.0f) + (AndroidUtilities.m1124dp(10.0f) * (1.0f - this.swipeToReleaseProgress));
                canvas.save();
