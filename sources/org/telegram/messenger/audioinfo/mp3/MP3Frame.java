...
public class MP3Frame {
    ...
    public static class Header {
        ...
        private static final int[][] FREQUENCIES = {new int[]{11025, -1, 22050, 44100}, new int[]{12000, -1, 24000, 48000}, new int[]{8000, -1, CarAudioRecord.AUDIO_CONTENT_SAMPLING_RATE, 32000}, new int[]{-1, -1, -1, -1}};
        private static final int[][] BITRATES = {new int[]{0, 0, 0, 0, 0}, new int[]{32000, 32000, 32000, 32000, 8000}, new int[]{64000, 48000, OneUIUtilities.ONE_UI_4_0, 48000, CarAudioRecord.AUDIO_CONTENT_SAMPLING_RATE}, new int[]{96000, 56000, 48000, 56000, 24000}, new int[]{128000, 64000, 56000, 64000, 32000}, new int[]{160000, 80000, 64000, 80000, OneUIUtilities.ONE_UI_4_0}, new int[]{192000, 96000, 80000, 96000, 48000}, new int[]{224000, 112000, 96000, 112000, 56000}, new int[]{256000, 128000, 112000, 128000, 64000}, new int[]{288000, 160000, 128000, 144000, 80000}, new int[]{320000, 192000, 160000, 160000, 96000}, new int[]{352000, 224000, 192000, 176000, 112000}, new int[]{384000, 256000, 224000, 192000, 128000}, new int[]{416000, 320000, 256000, 224000, 144000}, new int[]{448000, 384000, 320000, 256000, 160000}, new int[]{-1, -1, -1, -1, -1}};
    ...
    public boolean isChecksumError() {
        ...
        if (this.header.getProtection() == 0 && this.header.getLayer() == 1) {
            ...
            while (true) {
                ...
                crc16.update(bArr[i + 6]);
                i++;
            }
            if ((((bArr[4] & UByte.MAX_VALUE) << 8) | (bArr[5] & UByte.MAX_VALUE)) != crc16.getValue()) {
                return true;
            }
        }
        return false;
    ...
    public boolean isXingFrame() {
        ...
        if (bArr.length >= xingOffset + 12 && xingOffset >= 0 && bArr.length >= xingOffset + 8) {
            byte b2 = bArr[xingOffset];
            if (b2 == 88 && bArr[xingOffset + 1] == 105 && bArr[xingOffset + 2] == 110 && bArr[xingOffset + 3] == 103) {
                return true;
            }
            if (b2 == 73 && bArr[xingOffset + 1] == 110 && bArr[xingOffset + 2] == 102 && bArr[xingOffset + 3] == 111) {
                return true;
            }
        }
        return false;
    ...
    public int getNumberOfFrames() {
        ...
        if (isXingFrame()) {
            ...
            i = ((bArr[xingOffset + 8] & UByte.MAX_VALUE) << 24) | ((bArr[xingOffset + 9] & UByte.MAX_VALUE) << 16) | ((bArr[xingOffset + 10] & UByte.MAX_VALUE) << 8);
            ...
            i = ((bArr2[vBRIOffset + 14] & UByte.MAX_VALUE) << 24) | ((bArr2[vBRIOffset + 15] & UByte.MAX_VALUE) << 16) | ((bArr2[vBRIOffset + 16] & UByte.MAX_VALUE) << 8);
