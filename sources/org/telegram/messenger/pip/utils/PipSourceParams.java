...
public class PipSourceParams {
    ...
    public PictureInPictureParams.Builder build() {
        ...
        if (i2 > 0 && (i = point.y) > 0) {
            ...
            builderM1125m.setAspectRatio(rational);
            if (Build.VERSION.SDK_INT >= 33) {
                builderM1125m.setExpandedAspectRatio(rational);
            }
        } else {
            builderM1125m.setAspectRatio(null);
            if (Build.VERSION.SDK_INT >= 33) {
                builderM1125m.setExpandedAspectRatio(null);
            }
        }
        if (!this.position.isEmpty()) {
            builderM1125m.setSourceRectHint(this.position);
            return builderM1125m;
        }
