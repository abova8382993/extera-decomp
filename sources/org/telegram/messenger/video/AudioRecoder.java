...
public class AudioRecoder {
    ...
    public boolean step(MediaCodecVideoConvertor.Muxer muxer, int i) {
        ...
        if (!this.encoderInputDone && (iDequeueInputBuffer = this.encoder.dequeueInputBuffer(2500L)) >= 0) {
            boolean zIsInputAvailable = isInputAvailable();
            MediaCodec mediaCodec = this.encoder;
            if (zIsInputAvailable) {
                ShortBuffer shortBufferAsShortBuffer = mediaCodec.getInputBuffer(iDequeueInputBuffer).asShortBuffer();
