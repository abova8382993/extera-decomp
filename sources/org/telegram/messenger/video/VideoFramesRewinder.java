...
public class VideoFramesRewinder {
    ...
    /* JADX INFO: Access modifiers changed from: private */
    /* JADX WARN: Removed duplicated region for block: B:27:0x00cb A[LOOP:1: B:25:0x00b6->B:27:0x00cb, LOOP_END] */
    /* JADX WARN: Removed duplicated region for block: B:30:0x0113  */
    /* JADX WARN: Removed duplicated region for block: B:33:0x0119  */
    /* JADX WARN: Removed duplicated region for block: B:46:0x00ee A[EDGE_INSN: B:46:0x00ee->B:28:0x00ee BREAK  A[LOOP:1: B:25:0x00b6->B:27:0x00cb], SYNTHETIC] */
    /*
    ...
    */
    ...
    public /* synthetic */ void lambda$new$1(ArrayList arrayList, long j) {
        FileLog.m1133d("[VideoFramesRewinder] total prepare of " + arrayList.size() + " took " + (System.currentTimeMillis() - j) + "ms");
        if (!arrayList.isEmpty()) {
            FileLog.m1133d("[VideoFramesRewinder] prepared from " + ((Frame) arrayList.get(0)).position + "ms to " + ((Frame) arrayList.get(arrayList.size() - 1)).position + "ms (requested up to " + this.prepareToMs + "ms)");
        }
        ...
        if (arrayList.size() > 0) {
            FileLog.m1133d("[VideoFramesRewinder] prepared " + arrayList.size() + " more frames than I could fit :(");
        }
    ...
    private void prepare(long j) {
        ...
        FileLog.m1133d("[VideoFramesRewinder] starting preparing " + j + "ms");
    ...
    public void seek(long j, float f) {
        ...
        while (it.hasNext()) {
            ...
            if (Math.abs(next.position - j) < f2) {
                if (this.currentFrame != next) {
                    FileLog.m1133d("[VideoFramesRewinder] found a frame " + next.position + "ms to fit to " + j + "ms from " + this.frames.size() + " frames");
                    ...
                    if (i > 0) {
                        FileLog.m1133d("[VideoFramesRewinder] also deleted " + i + " frames after this frame");
                    }
                }
        ...
        FileLog.m1133d("[VideoFramesRewinder] didn't find a frame, wanting to prepare " + j + "ms");
