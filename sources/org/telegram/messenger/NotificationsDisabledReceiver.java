    ...
    public void onReceive(Context context, Intent intent) {
        if ("android.app.action.NOTIFICATION_CHANNEL_BLOCK_STATE_CHANGED".equals(intent.getAction())) {
            ...
            if (BuildVars.LOGS_ENABLED) {
                FileLog.m1133d("received disabled notification channel event for " + stringExtra + " state = " + booleanExtra);
            }
            if (SystemClock.elapsedRealtime() - AccountInstance.getInstance(iIntValue).getNotificationsController().lastNotificationChannelCreateTime <= 1000) {
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1133d("received disable notification event right after creating notification channel, ignoring");
                    return;
                }
                return;
            }
            ...
            if (strArrSplit[1].startsWith("channel")) {
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1133d("apply channel{channel} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1133d("apply channel{groups} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1133d("apply channel{private} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1133d("apply channel{stories} " + stringExtra + " state");
                }
                ...
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1133d("apply channel{else} " + stringExtra + " state");
                }
