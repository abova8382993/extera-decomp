...
public class DialogObject {
    ...
    public static void initDialog(TLRPC.Dialog dialog) {
        if (dialog == null || dialog.f1666id != 0) {
            return;
        }
        if (dialog instanceof TLRPC.TL_dialog) {
            ...
            if (j2 != 0) {
                ...
                dialog.f1666id = -peer.channel_id;
                return;
            }
        }
        if (dialog instanceof TLRPC.TL_dialogFolder) {
            dialog.f1666id = makeFolderDialogId(((TLRPC.TL_dialogFolder) dialog).folder.f1719id);
        }
    }
    ...
    public static String setDialogPhotoTitle(ImageReceiver imageReceiver, AvatarDrawable avatarDrawable, TLObject tLObject) {
        if (tLObject instanceof TLRPC.User) {
            ...
            if (UserObject.isUserSelf(user)) {
                String string2 = LocaleController.getString(C2888R.string.SavedMessages);
                if (avatarDrawable != null) {
    ...
    public static String getStatus(int i, TLObject tLObject) {
        ...
        if (tLObject instanceof TLRPC.Chat) {
            ...
            if (ChatObject.isChannelAndNotMegaGroup(chat)) {
                return LocaleController.getString(C2888R.string.DiscussChannel);
            }
            return LocaleController.getString(C2888R.string.AccDescrGroup);
        }
        return _UrlKt.FRAGMENT_ENCODE_SET;
    }
