...
public class DialogObject {
    ...
    public static long makeFolderDialogId(int i) {
        return 2305843009213693952L | ((long) i);
    }
    ...
    public static void initDialog(TLRPC.Dialog dialog) {
        if (dialog == null || dialog.f1251id != 0) {
            return;
        }
        if (dialog instanceof TLRPC.TL_dialog) {
            ...
            if (j2 != 0) {
                ...
                dialog.f1251id = -peer.channel_id;
                return;
            }
        }
        if (dialog instanceof TLRPC.TL_dialogFolder) {
            dialog.f1251id = makeFolderDialogId(((TLRPC.TL_dialogFolder) dialog).folder.f1305id);
        }
    }
    ...
    public static String setDialogPhotoTitle(ImageReceiver imageReceiver, AvatarDrawable avatarDrawable, TLObject tLObject) {
        if (tLObject instanceof TLRPC.User) {
            ...
            if (UserObject.isUserSelf(user)) {
                String string2 = LocaleController.getString(C2797R.string.SavedMessages);
                if (avatarDrawable != null) {
        ...
        if (tLObject instanceof TLRPC.Chat) {
            ...
            if (chat.participants_count > 1) {
                ...
                if (zIsChannelAndNotMegaGroup) {
                    return LocaleController.formatPluralStringComma("Subscribers", i2);
                }
                return LocaleController.formatPluralStringComma("Members", i2);
            }
            if (ChatObject.isChannelAndNotMegaGroup(chat)) {
                return LocaleController.getString(C2797R.string.DiscussChannel);
            }
            return LocaleController.getString(C2797R.string.AccDescrGroup);
        }
        return _UrlKt.FRAGMENT_ENCODE_SET;
    }
    ...
    public static String getPublicUsername(TLObject tLObject, String str) {
        if (tLObject instanceof TLRPC.Chat) {
            ...
            return str == null ? getPublicUsername(str2, arrayList, false) : getSimilarPublicUsername(str2, arrayList, str);
        }
        if (!(tLObject instanceof TLRPC.User)) {
            return null;
        }
        ...
        return str == null ? getPublicUsername(str3, arrayList2, false) : getSimilarPublicUsername(str3, arrayList2, str);
    }
