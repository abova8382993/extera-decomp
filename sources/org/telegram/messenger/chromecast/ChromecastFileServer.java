...
public class ChromecastFileServer extends NanoHTTPD {
    ...
    static {
        ...
        ASSET_FILES_MAP = new HashMap<>();
        for (ChromecastMedia chromecastMedia : chromecastMediaArr) {
            ASSET_FILES_MAP.put(chromecastMedia.externalPath, chromecastMedia);
        }
    }
    ...
    private void check() {
        boolean zIsEmpty = this.castedFiles.isEmpty();
    ...
    public NanoHTTPD.Response serve(NanoHTTPD.IHTTPSession iHTTPSession) {
        ...
        Log.d("CAST_SERVER", "Request " + iIncrementAndGet + " " + iHTTPSession.getMethod() + " " + iHTTPSession.getUri() + " " + iHTTPSession.getHeaders().get("range"));
        try {
    ...
    private NanoHTTPD.Response serveImpl(NanoHTTPD.IHTTPSession iHTTPSession) {
        ...
        if (NanoHTTPD.Method.OPTIONS.equals(iHTTPSession.getMethod())) {
            return NanoHTTPD.newFixedLengthResponse(NanoHTTPD.Response.Status.OK, "text/plain", _UrlKt.FRAGMENT_ENCODE_SET);
        }
    ...
    private NanoHTTPD.Response serveAvailableRoutes(String str) {
        ...
        int i = 0;
        while (i < 2) {
            for (Map.Entry<String, ChromecastMedia> entry : (i == 0 ? ASSET_FILES_MAP : this.castedFiles).entrySet()) {
                if (sb.length() > 0) {
                    sb.append('\n');
                }
    ...
    private NanoHTTPD.Response serveFileImpl(NanoHTTPD.IHTTPSession iHTTPSession, ChromecastMedia chromecastMedia) {
        ...
        Range rangeHeader = !zEquals ? parseRangeHeader(iHTTPSession.getHeaders().get("range"), jOpen) : null;
        ...
        if (j != 0) {
            ...
            responseNewFixedLengthResponse = NanoHTTPD.newFixedLengthResponse(NanoHTTPD.Response.Status.NO_CONTENT, chromecastMedia.mimeType, _UrlKt.FRAGMENT_ENCODE_SET);
        }
        if (rangeHeader != null) {
    ...
    public static class DataSourceInputStream extends InputStream {
        ...
        public int read() {
            ...
            return this.tmpByte[0] & UByte.MAX_VALUE;
        }
