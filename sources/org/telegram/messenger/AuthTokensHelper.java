...
public class AuthTokensHelper {
    public static ArrayList<TLRPC.TL_auth_loggedOut> getSavedLogOutTokens() {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            SerializedData serializedData = new SerializedData(Utilities.hexToBytes(sharedPreferences.getString("log_out_token_" + i2, _UrlKt.FRAGMENT_ENCODE_SET)));
    ...
    public static ArrayList<TLRPC.TL_auth_authorization> getSavedLogInTokens() {
        ...
        for (int i2 = 0; i2 < i; i2++) {
            try {
                SerializedData serializedData = new SerializedData(Utilities.hexToBytes(sharedPreferences.getString("log_in_token_" + i2, _UrlKt.FRAGMENT_ENCODE_SET)));
    ...
    public static void saveLogInToken(TLRPC.TL_auth_authorization tL_auth_authorization) {
        if (BuildVars.DEBUG_VERSION) {
            FileLog.m1045d("saveLogInToken ".concat(new String(tL_auth_authorization.future_auth_token, StandardCharsets.UTF_8)));
        }
