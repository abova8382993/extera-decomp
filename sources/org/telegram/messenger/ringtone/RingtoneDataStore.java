...
public class RingtoneDataStore {
    ...
    public void loadUserRingtones(boolean z) {
        boolean z2 = z || System.currentTimeMillis() - lastReloadTimeMs > DurationKt.MILLIS_IN_DAY;
    ...
    private void saveTones(ArrayList<TLRPC.Document> arrayList) {
        ...
        ArrayList<CachedTone> arrayList2 = this.userRingtones;
        int size = arrayList2.size();
        int i = 0;
        while (i < size) {
            CachedTone cachedTone = arrayList2.get(i);
            i++;
            CachedTone cachedTone2 = cachedTone;
            if (cachedTone2.localUri != null && (document = cachedTone2.document) != null) {
                map.put(Long.valueOf(document.f1253id), cachedTone2.localUri);
            }
        }
        this.userRingtones.clear();
        SharedPreferences sharedPreferences = getSharedPreferences();
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            ...
            String str = (String) map.get(Long.valueOf(document2.f1253id));
            ...
            CachedTone cachedTone3 = new CachedTone();
            cachedTone3.document = document2;
            cachedTone3.localUri = str;
            int i3 = this.localIds;
            this.localIds = i3 + 1;
            cachedTone3.localId = i3;
            this.userRingtones.add(cachedTone3);
        }
        editorEdit.apply();
        NotificationCenter.getInstance(this.currentAccount).lambda$postNotificationNameOnUIThread$1(NotificationCenter.onUserRingtonesUpdated, new Object[0]);
    }
    ...
    public void saveTones() {
        ...
        for (int i2 = 0; i2 < this.userRingtones.size(); i2++) {
            if (!this.userRingtones.get(i2).uploading) {
                i++;
                TLRPC.Document document = this.userRingtones.get(i2).document;
    ...
    public String getSoundPath(long j) {
        ...
        for (int i = 0; i < this.userRingtones.size(); i++) {
            if (this.userRingtones.get(i).document != null && this.userRingtones.get(i).document.f1253id == j) {
    ...
    public void remove(TLRPC.Document document) {
        ...
        for (int i = 0; i < this.userRingtones.size(); i++) {
            if (this.userRingtones.get(i).document != null && this.userRingtones.get(i).document.f1253id == document.f1253id) {
                this.userRingtones.remove(i);
                return;
            }
        }
    ...
    public void addTone(TLRPC.Document document) {
        if (document == null || contains(document.f1253id)) {
            return;
        }
        CachedTone cachedTone = new CachedTone();
    ...
    public TLRPC.Document getDocument(long j) {
        ...
        for (int i = 0; i < this.userRingtones.size(); i++) {
            try {
                if (this.userRingtones.get(i) != null && this.userRingtones.get(i).document != null && this.userRingtones.get(i).document.f1253id == j) {
