...
public class FilePathDatabase {
    ...
    public void createDatabase(int i, boolean z) {
        ...
        try {
            ...
            if (!zExists) {
                sQLiteDatabase2.executeFast("CREATE TABLE paths(document_id INTEGER, dc_id INTEGER, type INTEGER, path TEXT, flags INTEGER, PRIMARY KEY(document_id, dc_id, type));").stepThis().dispose();
                ...
                int iIntValue = sQLiteDatabase2.executeInt("PRAGMA user_version", new Object[0]).intValue();
                if (BuildVars.LOGS_ENABLED) {
                    FileLog.m1045d("current files db version = " + iIntValue);
                }
                if (iIntValue == 0) {
            ...
            FileLog.m1045d("files db created from_backup= " + z);
        } catch (Exception e) {
            if (i < 4) {
                if (!z && restoreBackup()) {
    ...
    private void createBackup() {
        ...
        try {
            ...
            FileLog.m1045d("file db backup created " + file2.getAbsolutePath());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
        ...
        if (sQLiteDatabase != null) {
            ...
            try {
                ...
                if (sQLiteCursorQueryFinalized.next()) {
                    ...
                    if (BuildVars.DEBUG_VERSION) {
                        FileLog.m1045d("get file path id=" + j + " dc=" + i + " type=" + i2 + " path=" + strArr[0] + " in " + (System.currentTimeMillis() - j2) + "ms");
                    }
                }
            } catch (Throwable th) {
                try {
    ...
    */
    ...
    public void checkMediaExistance(ArrayList<MessageObject> arrayList) {
        ...
        FileLog.m1045d("checkMediaExistance size=" + arrayList.size() + " time=" + (System.currentTimeMillis() - jCurrentTimeMillis) + " thread_time=" + jArr[0]);
        if (BuildVars.DEBUG_VERSION && Thread.currentThread() == Looper.getMainLooper().getThread()) {
            FileLog.m1048e(new Exception("warning, not allowed in main thread"));
        }
    }
    ...
    public static class PathData {
        ...
        public final int f1142dc;

        /* JADX INFO: renamed from: id */
        public final long f1143id;
