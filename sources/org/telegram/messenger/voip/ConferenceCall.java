...
public class ConferenceCall {
    ...
    private void checkEmojiHash() {
        ...
        if (m1200eq(verificationEmojis, this.lastVerificationEmojis)) {
            return;
        }
    ...
    private void checkParticipants() {
        ...
        if (m1199eq(hashSet, this.lastParticipants)) {
            return;
        }
    ...
    public /* synthetic */ void lambda$checkParticipants$1() {
        ...
        if (this.groupCall == null || (sharedInstance = VoIPService.getSharedInstance()) == null || (call = sharedInstance.groupCall) == null || (groupCall = call.call) == null || groupCall.f1675id != this.groupCall.f1675id) {
            return;
        }
        ...
        NotificationCenter.getInstance(this.currentAccount).lambda$postNotificationNameOnUIThread$1(NotificationCenter.groupCallUpdated, 0L, Long.valueOf(this.groupCall.f1675id), Boolean.FALSE);
    }

    /* JADX INFO: renamed from: eq */
    private boolean m1200eq(String[] strArr, String[] strArr2) {
        if (strArr == strArr2) {
            return true;
        }
    ...
    /* JADX INFO: renamed from: eq */
    private boolean m1199eq(HashSet<Long> hashSet, HashSet<Long> hashSet2) {
        if (hashSet == hashSet2) {
            return true;
        }
    ...
    public byte[] generateAddSelfBlock() {
        ...
        if (bArr == null) {
            ...
            FileLog.m1133d("[tde2e] call_create_zero_block(" + this.my_private_key_id + ", " + this.state + ")");
        } else {
            try {
                byte[] bArrCall_create_self_add_block = call_create_self_add_block(this.my_private_key_id, bArr, callParticipant);
                FileLog.m1133d("[tde2e] call_create_self_add_block(" + this.my_private_key_id + ", " + blockStr(this.last_block) + ", " + this.state + ") = " + blockStr(bArrCall_create_self_add_block));
                StringBuilder sb = new StringBuilder();
    ...
    private void readQueue(int i) {
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.readQueue(" + i + ") but destroyed!");
            return;
        }
        if (i != 0 && this.call_id < 0) {
            FileLog.m1133d("[tde2e] conference.readQueue(" + i + ") but there is no call yet!");
            return;
        }
        int i2 = this.last_offset[i];
        if (i2 == -1) {
            FileLog.m1133d("[tde2e] conference.readQueue(" + i + ") but last_offset == -1!");
            return;
        }
        int iMax = Math.max(0, i2);
        FileLog.m1133d("[tde2e] {subchain: " + i + "} processing blocks queue from " + iMax);
        while (true) {
            long j = iMax;
            byte[] bArr = this.blocksQueue[i].get(j);
            if (bArr == null) {
                FileLog.m1133d("[tde2e] {subchain: " + i + "} got into hole (might be the end) in " + i + " subchain at #" + iMax + ", when our last_offset[" + i + "] = " + this.last_offset[i]);
                this.last_offset[i] = iMax;
                return;
            }
            try {
                FileLog.m1133d("[tde2e] {subchain: " + i + "} processing #" + iMax + " block from queue");
                this.blocksQueue[i].remove(j);
                long j2 = this.call_id;
                if (j2 < 0) {
                    FileLog.m1133d("[tde2e] #" + iMax + " call_create block=" + call_describe_block(bArr));
                    ...
                    if (iMax > call_get_height(j2)) {
                        FileLog.m1133d("[tde2e] #" + iMax + " call_apply_block block=" + call_describe_block(bArr));
                        FileLog.m1133d("[tde2e] #" + iMax + " call_apply_block(" + this.call_id + ", " + blockStr(bArr) + ") = " + call_apply_block(this.call_id, bArr));
                    } else {
                        FileLog.m1133d("[tde2e] #" + iMax + " block from queue is under call's height!");
                    }
                } else if (i == 1) {
                    FileLog.m1133d("[tde2e] #" + iMax + " call_receive_inbound_message message=" + call_describe_message(bArr));
                    FileLog.m1133d("[tde2e] #" + iMax + " call_receive_inbound_message(" + this.call_id + ", " + blockStr(bArr) + ") = " + call_receive_inbound_message(this.call_id, bArr));
                }
                iMax++;
                this.last_offset[i] = iMax;
            } catch (Exception e) {
                FileLog.m1135e("[tde2e] {subchain: " + i + "} #" + iMax + " block got into error: ", e);
                return;
            }
        }
    }
    ...
    public boolean applyUpdate(Integer num, TLRPC.TL_updateGroupCallChainBlocks tL_updateGroupCallChainBlocks, boolean z, Long l) {
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.applyUpdate but destroyed!");
            return false;
        }
        ...
        if (groupCall == null) {
            FileLog.m1133d("[tde2e] received updateGroupCallChainBlocks but we dont have groupcall yet!");
            return false;
        }
        if (tL_updateGroupCallChainBlocks.call.f1682id != groupCall.f1675id) {
            FileLog.m1133d("[tde2e] received updateGroupCallChainBlocks for " + tL_updateGroupCallChainBlocks.call.f1682id + " but we have " + this.groupCall.f1675id);
            return false;
        }
        StringBuilder sb = new StringBuilder();
        ...
        if (i == 0 || i == 1) {
            for (int i3 = 0; i3 < tL_updateGroupCallChainBlocks.blocks.size(); i3++) {
                ...
                if (num == null || num.intValue() != -1) {
                    if (size >= this.last_offset[i]) {
                        FileLog.m1133d("[tde2e] {subchain: " + i + "} put #" + size + " into queue");
                        this.blocksQueue[i].put((long) size, bArr);
                    } else {
                        FileLog.m1133d("[tde2e] {subchain: " + i + "} received #" + size + " that was already processed from queue");
                    }
                } else if (i == 0) {
                    this.last_block = bArr;
                }
            ...
            if (this.last_offset[i] == -1) {
                if (num != null && num.intValue() == 0) {
                    ...
                    FileLog.m1133d("[tde2e] no offset, but we were asking for last block anyway");
                } else {
                    FileLog.m1134e("[tde2e] received update where we can't know what the start offset is of " + i + " sub chain (we requested " + num + ")");
                }
            }
            if (this.last_offset[i] != -1) {
    ...
    private void pull_outbound() {
        int i;
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.pull_outbound but destroyed!");
            return;
        }
        long j = this.call_id;
        if (j < 0) {
        ...
        try {
            ...
            FileLog.m1133d("[tde2e] call_pull_outbound_messages(" + this.call_id + ") = " + bArrCall_pull_outbound_messages.length + " blocks");
            i = 0;
            while (i2 < bArrCall_pull_outbound_messages.length) {
                try {
                    ...
                    FileLog.m1133d("[tde2e] pull outbound block to server!");
                    FileLog.m1133d("[tde2e] call_pull_outbound_messages(" + this.call_id + ")[" + i2 + "] = " + call_describe_message(bArrCall_pull_outbound_messages[i2]));
                    final long jCurrentTimeMillis = System.currentTimeMillis();
        ...
        try {
            FileLog.m1133d("[tde2e] state = " + call_get_verification_state(this.call_id));
        } catch (Exception e3) {
            FileLog.m1136e(e3);
        }
        try {
            FileLog.m1133d("[tde2e] call_describe(" + this.call_id + "): " + call_describe(this.call_id));
        } catch (Exception e4) {
            FileLog.m1136e(e4);
        }
        checkEmojiHash();
    ...
    public void poll() {
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.poll but destroyed!");
            return;
        }
        if (!this.joined) {
            FileLog.m1133d("[tde2e] conference.poll but not joined!");
            return;
        }
        this.polling = true;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            FileLog.m1133d("[tde2e] requesting getGroupCallChainBlocks sub_chain_id=" + getgroupcallchainblocks.sub_chain_id + " offset=" + getgroupcallchainblocks.offset + " limit=10");
            final long jCurrentTimeMillis = System.currentTimeMillis();
            ...
            try {
                ...
                sb.append(TextUtils.join("\n ", (Iterable) DesugarArrays.stream(call_get_state(this.call_id).participants).map(new Function() { // from class: org.telegram.messenger.voip.ConferenceCall$$ExternalSyntheticLambda14
                    ...
                    public final Object apply(Object obj) {
                        return ConferenceCall.m5170$r8$lambda$l2DLYo9iQuRY1Qymj89k3Qq7zM((ConferenceCall.CallParticipant) obj);
                    }
    ...
    /* JADX INFO: renamed from: $r8$lambda$l2DLYo9iQuRY1Qymj89-k3Qq7zM, reason: not valid java name */
    public static /* synthetic */ String m5170$r8$lambda$l2DLYo9iQuRY1Qymj89k3Qq7zM(CallParticipant callParticipant) {
        return "[" + callParticipant.user_id + "]: " + DialogObject.getName(callParticipant.user_id);
    }

    public void forcePoll() {
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.forcePoll but destroyed!");
            return;
        }
        if (!this.joined) {
            FileLog.m1133d("[tde2e] conference.forcePoll but not joined!");
        } else {
            if (this.polling) {
                return;
            }
    ...
    public void updateParticipants(ArrayList<TLRPC.GroupCallParticipant> arrayList, boolean z) {
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.updateParticipants but destroyed!");
            return;
        }
        ...
        if (!shadyLeftParticipants.isEmpty()) {
            try {
                ...
                FileLog.m1133d("[tde2e] call_create_change_state_block from " + this.state + " to " + callState);
    ...
    public void kick(long j) {
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.kick but destroyed!");
            return;
        }
        ...
        while (true) {
            ...
            if (i >= callParticipantArr.length) {
                ...
                FileLog.m1133d("[tde2e] kick: call_create_change_state_block from " + this.state + " to " + callState);
                byte[] bArrCall_create_change_state_block = call_create_change_state_block(this.call_id, callState);
                StringBuilder sb = new StringBuilder();
    ...
    public HashSet<Long> getBlockchainParticipants() {
        ...
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.getBlockchainParticipants but destroyed!");
            return hashSet;
        }
        long j = this.call_id;
        if (j >= 0) {
    ...
    public HashSet<Long> getShadyJoiningParticipants(ArrayList<TLRPC.GroupCallParticipant> arrayList) {
        ...
        HashSet<Long> hashSet = new HashSet<>();
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.getShadyJoiningParticipants but destroyed!");
            return hashSet;
        }
        long j = this.call_id;
        if (j >= 0) {
    ...
    public HashSet<Long> getShadyLeftParticipants(ArrayList<TLRPC.GroupCallParticipant> arrayList) {
        ...
        HashSet<Long> hashSet = new HashSet<>();
        if (this.destroyed) {
            FileLog.m1133d("[tde2e] conference.getShadyLeftParticipants but destroyed!");
            return hashSet;
        }
        long j = this.call_id;
        if (j >= 0) {
    ...
    public void reset() {
        ...
        if (this.call_id != -1) {
            ...
            FileLog.m1133d("[tde2e] call_destroy(" + this.call_id + ")");
            this.call_id = -1L;
        }
