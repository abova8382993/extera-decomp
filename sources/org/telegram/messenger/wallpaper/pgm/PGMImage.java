...
public abstract class PGMImage {
    public static void write(Bitmap bitmap, OutputStream outputStream, List<String> list) throws IOException {
        if (bitmap.getConfig() != Bitmap.Config.ALPHA_8) {
            g$$ExternalSyntheticBUOutline1.m207m("Only Bitmap.Config.ALPHA_8 is supported");
            return;
        }
        ...
        if (list != null && !list.isEmpty()) {
            ...
            while (it.hasNext()) {
                String next = it.next();
                outputStream.write(("#" + (next == null ? _UrlKt.FRAGMENT_ENCODE_SET : next.replace('\r', ' ').replace('\n', ' ')) + "\n").getBytes(StandardCharsets.US_ASCII));
            }
        }
    ...
    public static Bitmap read(InputStream inputStream, List<String> list) throws IOException {
        BufferedInputStream bufferedInputStream = new BufferedInputStream(inputStream);
        String strNextToken = nextToken(bufferedInputStream, list);
        if (!"P5".equals(strNextToken)) {
            ZipFileSystem$$ExternalSyntheticBUOutline0.m996m("Not a binary PGM (P5), got: ", strNextToken);
            return null;
        }
        ...
        if (positiveInt3 != 255) {
            ZipFilesKt$$ExternalSyntheticBUOutline0.m998m("Only 8-bit PGM supported (maxval=255), got: ", positiveInt3);
            return null;
        }
    ...
    private static String nextToken(BufferedInputStream bufferedInputStream, List<String> list) throws IOException {
    ...
    private static String nextNonCommentToken(BufferedInputStream bufferedInputStream, List<String> list) throws IOException {
        String strNextToken;
        do {
            strNextToken = nextToken(bufferedInputStream, list);
            if (strNextToken == null) {
                Model$$ExternalSyntheticBUOutline0.m1247m("Unexpected EOF in header");
                return null;
            }
    ...
    private static void readFully(InputStream inputStream, byte[] bArr, int i, int i2) throws IOException {
        ...
        while (i3 < i2) {
            ...
            if (i4 < 0) {
                Model$$ExternalSyntheticBUOutline0.m1247m("Unexpected EOF");
                return;
            }
            i3 += i4;
        }
