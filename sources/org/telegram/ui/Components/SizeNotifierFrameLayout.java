    ...
    public void startBlur() {
        ...
        if (this.blurIsRunning && !this.blurGeneratingTuskIsRunning && this.invalidateBlur && SharedConfig.chatBlurEnabled() && !DRAW_USING_RENDERNODE()) {
            if (Color.alpha(Theme.getColor(Theme.key_chat_BlurAlphaSlow)) != 255 || ExteraConfig.forceBlur) {
                ...
                this.blurBackgroundTask.radius = (int) (((int) (Math.max(6, Math.max(iDp, measuredWidth) / Opcodes.GETFIELD) * 2.5f)) * BlurSettingsBottomSheet.blurRadius);
        ...
        if (DRAW_USING_RENDERNODE()) {
            ...
            if (i3 >= 255 || !this.blurNodeInvalidated[i4] || this.blurNodeInvalidatedThisFrame[i4]) {
                ...
                if (renderNodeArr[i4] == null) {
                    ...
                    sb.append("blurNode");
                    sb.append(i4);
                    renderNodeArr[i4] = BotFullscreenButtons$$ExternalSyntheticApiModelOutline0.m(sb.toString());
