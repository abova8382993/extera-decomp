...
import org.telegram.p035ui.Components.FloatSeekBarAccessibilityDelegate;
...
public abstract class ActionBarMenuSlider extends FrameLayout {
    ...
    public abstract int getColorValue(float f);

    public abstract String getLeftStringValue(float f);

    public abstract String getRightStringValue(float f);
    ...
    public ActionBarMenuSlider(Context context, Theme.ResourcesProvider resourcesProvider) {
        ...
        AnimatedTextView.AnimatedTextDrawable animatedTextDrawable = new AnimatedTextView.AnimatedTextDrawable(z2, z, z) { // from class: org.telegram.ui.ActionBar.ActionBarMenuSlider.1
    ...
    public void setDrawShadow(boolean z) {
        this.drawShadow = z;
        int iM1036dp = z ? AndroidUtilities.m1036dp(8.0f) : 0;
        setPadding(iM1036dp, iM1036dp, iM1036dp, iM1036dp);
        invalidate();
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.drawShadow) {
            canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.shadowPaint);
        }
        if (this.drawBlur) {
            ...
            if (f < 1.0f) {
                ...
                canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.pseudoBlurPaint);
            }
            if (this.blurBitmap != null && this.value < 1.0f && f > 0.0f) {
                this.blurPaint.setAlpha((int) (f * 255.0f));
                canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.blurPaint);
            }
            canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.brightenBlurPaint);
            canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.darkenBlurPaint);
            this.fillPaint.setColor(-1);
        } else {
            canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.backgroundPaint);
        }
        drawStops(canvas);
        ...
        canvas.drawRoundRect(rectF, AndroidUtilities.m1036dp(this.roundRadiusDp), AndroidUtilities.m1036dp(this.roundRadiusDp), this.fillPaint);
        drawStops(canvas);
        if (!this.backgroundDark) {
    ...
    private void drawStops(Canvas canvas) {
        ...
        while (true) {
            ...
            float f = fArr[i];
            RectF rectF = AndroidUtilities.rectTmp;
            canvas.drawRect(rectF.left + (rectF.width() * f), rectF.top, rectF.left + (rectF.width() * f) + AndroidUtilities.m1036dp(0.66f), rectF.bottom, this.stopPaint);
            i++;
        }
    }
    ...
    private void drawText(Canvas canvas, boolean z) {
        ...
        this.leftTextDrawable.setBounds(getPaddingLeft() + AndroidUtilities.m1036dp(20.0f), getMeasuredHeight() / 2, (getMeasuredWidth() - getPaddingRight()) - AndroidUtilities.m1036dp(20.0f), getMeasuredHeight() / 2);
        ...
        this.rightTextDrawable.setBounds(getPaddingLeft() + AndroidUtilities.m1036dp(20.0f), getMeasuredHeight() / 2, (getMeasuredWidth() - getPaddingRight()) - AndroidUtilities.m1036dp(20.0f), getMeasuredHeight() / 2);
    ...
    private Pair<Integer, Integer> getBitmapGradientColors(Bitmap bitmap) {
        ...
        float measuredWidth = (r1 + getMeasuredWidth()) / AndroidUtilities.displaySize.x;
        ...
        if (width < 0 || width >= bitmap.getWidth() || width2 < 0 || width2 >= bitmap.getWidth() || currentActionBarHeight < 0 || currentActionBarHeight >= bitmap.getHeight()) {
            return null;
        }
        return new Pair<>(Integer.valueOf(bitmap.getPixel(width, currentActionBarHeight)), Integer.valueOf(bitmap.getPixel(width2, currentActionBarHeight)));
    }
    ...
    public static class SpeedSlider extends ActionBarMenuSlider {
        ...
        public SpeedSlider(Context context, Theme.ResourcesProvider resourcesProvider) {
            ...
            FloatSeekBarAccessibilityDelegate floatSeekBarAccessibilityDelegate = new FloatSeekBarAccessibilityDelegate(false) { // from class: org.telegram.ui.ActionBar.ActionBarMenuSlider.SpeedSlider.1
                ...
                public CharSequence getContentDescription(View view) {
                    return SpeedIconDrawable.formatNumber(SpeedSlider.this.getSpeed()) + "x  " + LocaleController.getString(C2797R.string.AccDescrSpeedSlider);
                }
            };
        ...
        @Override // org.telegram.p035ui.ActionBar.ActionBarMenuSlider
        public String getLeftStringValue(float f) {
        ...
        @Override // org.telegram.p035ui.ActionBar.ActionBarMenuSlider
        public String getRightStringValue(float f) {
            if (this.label == null) {
                return null;
            }
            return SpeedIconDrawable.formatNumber((f * 2.8f) + 0.2f) + "x";
