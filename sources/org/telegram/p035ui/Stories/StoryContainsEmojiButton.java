...
public class StoryContainsEmojiButton extends View {
    ...
    public void onMeasure(int i, int i2) {
        ...
        int iM1036dp = AndroidUtilities.m1036dp(29.0f);
        StaticLayout staticLayout = this.layout;
        setMeasuredDimension(z ? View.MeasureSpec.getSize(i) : getMinimumWidth(), paddingTop + AndroidUtilities.lerp(iM1036dp, staticLayout == null ? AndroidUtilities.m1036dp(29.0f) : staticLayout.getHeight(), this.loadT) + getPaddingBottom());
    ...
    public void dispatchDraw(Canvas canvas) {
        ...
        if (f < 1.0f) {
            ...
            float paddingTop2 = getPaddingTop() + AndroidUtilities.m1036dp(12.0f);
            Path.Direction direction = Path.Direction.CW;
            path.addRect(paddingLeft, paddingTop, measuredWidth, paddingTop2, direction);
            this.loadingPath.addRect(getPaddingLeft(), getPaddingTop() + AndroidUtilities.m1036dp(16.0f), getPaddingLeft() + (((getMeasuredWidth() - getPaddingRight()) - getPaddingLeft()) * 0.46f), getPaddingTop() + AndroidUtilities.m1036dp(28.0f), direction);
    ...
    public void load(final int i, boolean z, TLObject tLObject, final ArrayList<TLRPC.InputStickerSet> arrayList, final Object obj) {
        Vector<TLRPC.StickerSetCovered> vector;
        ...
        if (this.inputSets.size() == 1) {
            MediaDataController.getInstance(i).getStickerSet(this.inputSets.get(0), 0, false, new Utilities.Callback() { // from class: org.telegram.ui.Stories.StoryContainsEmojiButton$$ExternalSyntheticLambda3
    ...
    public /* synthetic */ void lambda$load$1(TLObject tLObject, Object obj, ArrayList arrayList, boolean[] zArr, int i) {
        if (tLObject instanceof Vector) {
            Vector<TLRPC.StickerSetCovered> vector = (Vector) tLObject;
            this.vector = vector;
            ...
            if (this.inputSets != null && arrayList != null && !arrayList.isEmpty()) {
                for (int i3 = 0; i3 < arrayList.size(); i3++) {
                    ...
                    long j = inputStickerSet.f1270id;
                    int i4 = 0;
                    while (true) {
                        int size3 = this.inputSets.size();
                        ArrayList<TLRPC.InputStickerSet> arrayList3 = this.inputSets;
                        if (i4 >= size3) {
                            arrayList3.add(inputStickerSet);
                            break;
                        } else if (arrayList3.get(i4).f1270id == j) {
                            break;
                        } else {
                            i4++;
                        }
    ...
    private void set(TLRPC.TL_messages_stickerSet tL_messages_stickerSet) {
        ...
        while (true) {
            if (i >= arrayList.size()) {
                ...
                if (arrayList.get(i).f1253id == tL_messages_stickerSet.set.thumb_document_id) {
        ...
        if (z && this.stickers) {
            string = LocaleController.getString(C2797R.string.StoryContainsStickersEmojiFrom);
        } else if (z) {
            string = LocaleController.getString(C2797R.string.StoryContainsEmojiFrom);
        } else {
            string = LocaleController.getString(C2797R.string.StoryContainsStickersFrom);
        }
    ...
    private void set(TLRPC.StickerSetCovered stickerSetCovered) {
        ...
        if (document == null && (stickerSetCovered instanceof TLRPC.TL_stickerSetFullCovered)) {
            ...
            for (int i = 0; i < arrayList.size(); i++) {
                if (arrayList.get(i).f1253id == stickerSetCovered.set.thumb_document_id) {
                    document = arrayList.get(i);
                }
            }
            if (document == null && !arrayList.isEmpty()) {
    ...
    private void animateLoad(boolean z) {
        ...
        if (z) {
            ...
            final boolean z2 = this.layout == null || Math.abs(getMeasuredHeight() - ((getPaddingTop() + this.layout.getHeight()) + getPaddingBottom())) > AndroidUtilities.m1036dp(3.0f);
