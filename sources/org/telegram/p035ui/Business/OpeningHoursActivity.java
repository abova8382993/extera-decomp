...
public class OpeningHoursActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    public View createView(Context context) {
        ...
        this.actionBar.setTitle(LocaleController.getString(C2797R.string.BusinessHours));
    ...
    public boolean hasChanges() {
        ...
        if (this.currentValue != null && this.enabled) {
            ...
            int i = 0;
            loop0: while (true) {
                ArrayList<Period>[] arrayListArr = this.currentValue;
                ...
                for (int i2 = 0; i2 < this.value[i].size(); i2++) {
                    Period period = this.currentValue[i].get(i2);
                    Period period2 = this.value[i].get(i2);
    ...
    */
    public static java.util.ArrayList<org.telegram.tgnet.tl.TL_account.TL_businessWeeklyOpen> adaptWeeklyOpen(java.util.ArrayList<org.telegram.tgnet.tl.TL_account.TL_businessWeeklyOpen> r9, int r10) {
        /*
        ...
        L3e:
            ...
            int r6 = r4.end_minute
            r7 = 10079(0x275f, float:1.4124E-41)
            r8 = 10080(0x2760, float:1.4125E-41)
            if (r5 >= 0) goto L73
            if (r6 >= 0) goto L60
            int r5 = r5 + 10080
            r4.start_minute = r5
        ...
        L96:
            ...
            return r9
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p035ui.Business.OpeningHoursActivity.adaptWeeklyOpen(java.util.ArrayList, int):java.util.ArrayList");
    }

    /* JADX INFO: renamed from: $r8$lambda$e8LPlc3wp-Ocbipb8BSgQa5RZ4c, reason: not valid java name */
    public static /* synthetic */ int m7668$r8$lambda$e8LPlc3wpOcbipb8BSgQa5RZ4c(TL_account.TL_businessWeeklyOpen tL_businessWeeklyOpen, TL_account.TL_businessWeeklyOpen tL_businessWeeklyOpen2) {
        return tL_businessWeeklyOpen.start_minute - tL_businessWeeklyOpen2.start_minute;
    }

    public static ArrayList<Period>[] getDaysHours(ArrayList<TL_account.TL_businessWeeklyOpen> arrayList) {
        int i;
        ...
        for (int i3 = 0; i3 < arrayList.size(); i3++) {
            TL_account.TL_businessWeeklyOpen tL_businessWeeklyOpen = arrayList.get(i3);
        ...
        while (i6 < 7) {
            ...
            for (int i10 = 0; i10 < arrayList.size(); i10++) {
                TL_account.TL_businessWeeklyOpen tL_businessWeeklyOpen2 = arrayList.get(i10);
            ...
            if (i9 >= i8) {
                ...
                if (iMin >= 1440 && !arrayList2.isEmpty() && arrayList2.get(0).start < iMin - 1440) {
                    iMin = arrayList2.get(0).start + 1439;
                }
    ...
    public static ArrayList<TL_account.TL_businessWeeklyOpen> fromDaysHours(ArrayList<Period>[] arrayListArr) {
        ArrayList<TL_account.TL_businessWeeklyOpen> arrayList = new ArrayList<>();
    ...
    public static String toString(int i, TLRPC.User user, TL_account.TL_businessWorkHours tL_businessWorkHours) {
        ...
        ArrayList<Period>[] daysHours = getDaysHours(tL_businessWorkHours.weekly_open);
        StringBuilder sb = new StringBuilder();
        if (user != null) {
            sb.append(LocaleController.formatString(C2797R.string.BusinessHoursCopyHeader, UserObject.getUserName(user)));
            sb.append("\n");
        }
        for (int i2 = 0; i2 < daysHours.length; i2++) {
            ...
            sb.append(": ");
            if (isFull(arrayList)) {
                sb.append(LocaleController.getString(C2797R.string.BusinessHoursProfileOpen));
            } else if (arrayList.isEmpty()) {
                sb.append(LocaleController.getString(C2797R.string.BusinessHoursProfileClose));
            } else {
                for (int i3 = 0; i3 < arrayList.size(); i3++) {
                    if (i3 > 0) {
                        sb.append(", ");
        ...
        if (((Calendar.getInstance().getTimeZone().getRawOffset() / MediaDataController.MAX_STYLE_RUNS_COUNT) - (tL_timezoneFindTimezone != null ? tL_timezoneFindTimezone.utc_offset : 0)) / 60 != 0 && tL_timezoneFindTimezone != null) {
            sb.append(LocaleController.formatString(C2797R.string.BusinessHoursCopyFooter, TimezonesController.getInstance(i).getTimezoneName(tL_timezoneFindTimezone, true)));
        }
        return sb.toString();
    }
    ...
    public void processDone() {
        ...
        ArrayList<TL_account.TL_businessWeeklyOpen> arrayListFromDaysHours = fromDaysHours(this.value);
    ...
    public static class Period {
        ...
        public static String timeToString(int i, boolean z) {
            ...
            return (i <= 1440 || !z) ? str : LocaleController.formatString(C2797R.string.BusinessHoursNextDay, str);
        }
    }
    ...
    private String getPeriodsValue(ArrayList<Period> arrayList) {
        if (arrayList.isEmpty()) {
            return LocaleController.getString(C2797R.string.BusinessHoursDayClosed);
        }
        if (isFull(arrayList)) {
            return LocaleController.getString(C2797R.string.BusinessHoursDayFullOpened);
        }
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            strConcat = strConcat + Period.timeToString(period.start) + " - " + Period.timeToString(period.end);
        }
        return strConcat;
    }
    ...
    private int maxPeriodsFor(int i) {
        int iMax = 0;
        for (int i2 = 0; i2 < 7; i2++) {
            ArrayList<Period> arrayList = this.value[i2];
            if (arrayList != null) {
    ...
    public void fillItems(ArrayList<UItem> arrayList, UniversalAdapter universalAdapter) {
        arrayList.add(UItem.asTopView(LocaleController.getString(C2797R.string.BusinessHours), LocaleController.getString(C2797R.string.BusinessHoursInfo), C2797R.raw.biz_clock));
        arrayList.add(UItem.asCheck(-1, LocaleController.getString(C2797R.string.BusinessHoursShow)).setChecked(this.enabled));
        arrayList.add(UItem.asShadow(-100, null));
        if (!this.enabled) {
            return;
        }
        arrayList.add(UItem.asHeader(LocaleController.getString(C2797R.string.BusinessHours)));
        int i = 0;
        while (true) {
            ArrayList<Period>[] arrayListArr = this.value;
            if (i < arrayListArr.length) {
                ...
                arrayList.add(UItem.asButton(-2, LocaleController.getString(C2797R.string.BusinessHoursTimezone), TimezonesController.getInstance(this.currentAccount).getTimezoneName(this.timezoneId, false)));
    ...
    public void onClick(final UItem uItem, final View view, int i, float f, float f2) {
        ...
        if (!LocaleController.isRTL ? f >= view.getMeasuredWidth() - AndroidUtilities.m1036dp(76.0f) : f <= AndroidUtilities.m1036dp(76.0f)) {
            if (this.value[uItem.f1708id].isEmpty()) {
                ((NotificationsCheckCell) view).setChecked(true);
                this.value[uItem.f1708id].add(new Period(0, 1439));
                adaptPrevDay(uItem.f1708id);
            } else {
                this.value[uItem.f1708id].clear();
                ((NotificationsCheckCell) view).setChecked(false);
            }
            ((NotificationsCheckCell) view).setValue(getPeriodsValue(this.value[uItem.f1708id]));
            checkDone(true);
            return;
        }
        int i3 = (uItem.f1708id + 6) % 7;
        int i4 = 0;
