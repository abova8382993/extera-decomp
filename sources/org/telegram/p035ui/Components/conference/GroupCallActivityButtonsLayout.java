package org.telegram.p035ui.Components.conference;
...
public abstract class GroupCallActivityButtonsLayout extends ViewGroup {
    ...
    public void onMeasure(int i, int i2) {
        ...
        int iMakeMeasureSpec = View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m1036dp(76.0f), TLObject.FLAG_30);
        int iMakeMeasureSpec2 = View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m1036dp(76.0f), TLObject.FLAG_30);
    ...
    private void doLayout(boolean z, boolean z2) {
        ...
        if (!z3) {
            int iMin2 = Math.min(AndroidUtilities.m1036dp(50.0f) + Math.max((int) ((measuredWidth2 - (AndroidUtilities.m1036dp(50.0f) * i2)) / (i2 + 0.333f)), 0), measuredWidth2 / i2);
            iMin = AndroidUtilities.m1036dp(76.0f);
            i = (measuredWidth2 - (i2 * iMin2)) / 2;
            measuredWidth2 = iMin2;
        } else {
            iMin = Math.min(AndroidUtilities.m1036dp(50.0f) + Math.max((int) ((measuredHeight2 - (AndroidUtilities.m1036dp(50.0f) * i2)) / (i2 + 0.333f)), 0), measuredHeight2 / i2);
        ...
        while (it2.hasNext()) {
            ...
            if (value.isVisible) {
                if (!z3) {
                    measuredWidth = (measuredWidth2 * i3) + i + ((measuredWidth2 - value.view.getMeasuredWidth()) / 2);
                    measuredHeight = getMeasuredHeight() - AndroidUtilities.m1036dp(76.0f);
                } else {
                    measuredWidth = (getMeasuredWidth() - measuredWidth2) + ((measuredWidth2 - value.view.getMeasuredWidth()) / 2);
                    measuredHeight = (iMin * i3) + i;
                }
                if (!z2 && ((z || value.xAnimator.isAnimating()) && value.visibility.getValue())) {
                    value.xAnimator.animateTo(measuredWidth);
                } else {
                    value.xAnimator.forceFactor(measuredWidth);
                }
                if (!z2 && ((z || value.yAnimator.isAnimating()) && value.visibility.getValue())) {
                    value.yAnimator.animateTo(measuredHeight);
                } else {
                    value.yAnimator.forceFactor(measuredHeight);
                }
                i3++;
            }
            value.visibility.setValue(value.isVisible, !z2 && (z || value.visibility.isAnimating()));
        }
        invalidate();
    }
