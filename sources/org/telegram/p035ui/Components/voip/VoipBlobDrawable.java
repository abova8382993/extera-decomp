...
public class VoipBlobDrawable extends BlobDrawable {
    ...
    public void generateBlob(float[] fArr, float[] fArr2, int i, float f) {
        float f2 = (360.0f / this.f1532N) * 0.05f;
        float f3 = this.maxRadius;
        float f4 = this.minRadius;
        fArr[i] = f4 + (Math.abs((this.random.nextInt() % 100.0f) / 100.0f) * (f3 - f4) * f);
        fArr2[i] = ((360.0f / this.f1532N) * i) + ((((this.random.nextInt() * f) % 100.0f) / 100.0f) * f2);
