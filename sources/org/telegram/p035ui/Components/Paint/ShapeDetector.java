...
public class ShapeDetector {
    ...
    public static class Point {
        ...
        public double f1588x;

        /* JADX INFO: renamed from: y */
        public double f1589y;
        ...
        public double distance(double d, double d2) {
            return Math.sqrt(Math.pow(d - this.f1588x, 2.0d) + Math.pow(d2 - this.f1589y, 2.0d));
        }

        public double distance(Point point) {
            return distance(point.f1588x, point.f1589y);
        }
    }
            ...
            if (this.isLearning && (string = this.preferences.getString("moretemplates", null)) != null) {
                ...
                while (i5 < strArrSplit.length) {
                    ...
                    if (strArrSplit2.length <= 1) {
                        ...
                        template2.shapeType = Integer.parseInt(strArrSplit2[c2]);
    ...
    public /* synthetic */ void lambda$new$2() {
        ...
        synchronized (this) {
            try {
                ...
                ArrayList<Point> arrayListResample = resample(fullClone(this.points), 48);
                final ArrayList<Point> arrayListFullClone = fullClone(arrayListResample);
                rotate(arrayListFullClone, indicativeAngle(arrayListFullClone));
                Point pointCentroid = centroid(arrayListFullClone);
                translate(arrayListFullClone, -pointCentroid.f1588x, -pointCentroid.f1589y);
                ...
                while (i < this.templates.size()) {
                    int i4 = i;
                    int i5 = i2;
                    double dDistanceAtBestAngle = distanceAtBestAngle(arrayListFullClone, pointCentroid2, this.templates.get(i).points, -1.5707963267948966d, 1.5707963267948966d, 0.06981317007977318d);
    ...
    private ArrayList<Point> resample(ArrayList<Point> arrayList, int i) {
        ...
        while (i4 < arrayList.size()) {
            int i5 = i4 - 1;
            double dDistance = arrayList.get(i5).distance(arrayList.get(i4));
            ...
            if (d3 >= dPathLength) {
                ...
                i2 = i3;
                d = dPathLength;
                Point point = new Point(arrayList.get(i5).f1588x + ((arrayList.get(i4).f1588x - arrayList.get(i5).f1588x) * d4), arrayList.get(i5).f1589y + (d4 * (arrayList.get(i4).f1589y - arrayList.get(i5).f1589y)));
    ...
    private double distanceAtAngle(ArrayList<Point> arrayList, Point point, ArrayList<Point> arrayList2, double d) {
        ...
        while (i < iMin) {
            ...
            double d2 = point3.f1588x;
            double d3 = dCos;
            double d4 = point2.f1588x;
            double d5 = (d2 - d4) * d3;
            double d6 = point3.f1589y;
            double d7 = point2.f1589y;
    ...
    private ArrayList<Point> fullClone(ArrayList<Point> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            ...
            arrayList2.add(new Point(point.f1588x, point.f1589y));
        }
        return arrayList2;
    }
    ...
    private void rotate(ArrayList<Point> arrayList, double d, Point point) {
        ...
        while (i < arrayList.size()) {
            ...
            double d2 = point2.f1588x;
            double d3 = point.f1588x;
            double d4 = (d2 - d3) * dCos;
            double d5 = point2.f1589y;
            double d6 = dCos;
            double d7 = point.f1589y;
            point2.f1589y = ((d2 - d3) * dSin) + ((d5 - d7) * d6) + d7;
            point2.f1588x = (d4 - ((d5 - d7) * dSin)) + d3;
            i++;
            dCos = d6;
        }
    }
    ...
    private RectD boundingBox(ArrayList<Point> arrayList) {
        ...
        double d = arrayList.get(0).f1588x;
        double d2 = arrayList.get(0).f1589y;
        RectD rectD = new RectD(d, d2, d, d2);
        for (int i = 1; i < arrayList.size(); i++) {
            Point point = arrayList.get(i);
            rectD.union(point.f1588x, point.f1589y);
        }
        return rectD;
    }
    ...
    private Point centroid(ArrayList<Point> arrayList) {
        ...
        for (int i = 0; i < arrayList.size(); i++) {
            Point point2 = arrayList.get(i);
            point.f1588x += point2.f1588x;
            point.f1589y += point2.f1589y;
        }
        point.f1588x /= (double) arrayList.size();
        point.f1589y /= (double) arrayList.size();
        return point;
    }

    private double indicativeAngle(ArrayList<Point> arrayList) {
        Point pointCentroid = centroid(arrayList);
        return Math.atan2(pointCentroid.f1589y - arrayList.get(0).f1589y, pointCentroid.f1588x - arrayList.get(0).f1588x);
    }

    private double pathLength(ArrayList<Point> arrayList) {
        double dDistance = 0.0d;
        for (int i = 1; i < arrayList.size(); i++) {
            dDistance += arrayList.get(i - 1).distance(arrayList.get(i));
        }
        return dDistance;
    }
    ...
    private int findAnglePoint(ArrayList<Point> arrayList, int i) {
        ...
        while (iMax < arrayList.size() - 1) {
            Point point = arrayList.get(iMax - 1);
            Point point2 = arrayList.get(iMax);
            int i2 = iMax + 1;
    ...
    private Shape constructShape(int i, ArrayList<Point> arrayList) {
        ...
        if (i == 4) {
            ...
            Point point = arrayList.get(iFindAnglePoint2);
            Point point2 = arrayList.get(iFindAnglePoint2 / 2);
            Point point3 = arrayList.get(0);
            shape.centerX = (float) point.f1588x;
            shape.centerY = (float) point.f1589y;
            shape.middleX = (float) point2.f1588x;
            shape.middleY = (float) point2.f1589y;
            shape.radiusX = (float) point3.f1588x;
            shape.radiusY = (float) point3.f1589y;
            shape.arrowTriangleLength = 16.0f;
            return shape;
        }
        Point pointCentroid = centroid(arrayList);
        shape.centerX = (float) pointCentroid.f1588x;
        shape.centerY = (float) pointCentroid.f1589y;
        RectD rectDBoundingBox = boundingBox(arrayList);
        ...
        if (i == 2 && (iFindAnglePoint = findAnglePoint(arrayList, 1)) > 0) {
            Point point4 = arrayList.get(iFindAnglePoint);
            shape.rotation = (float) Math.atan2(point4.f1589y - ((double) shape.centerY), point4.f1588x - ((double) shape.centerX));
        }
        return shape;
    }
    ...
    public /* synthetic */ void lambda$showSaveLearnDialog$3(ArrayList arrayList, DialogInterface dialogInterface, int i) {
        if (i == 0) {
            ...
            for (int i2 = 0; i2 < this.templates.size(); i2++) {
                ...
                for (int i3 = 0; i3 < template.points.size(); i3++) {
                    ...
                    sb.append("[");
                    sb.append(Math.round(point.f1588x));
                    sb.append(",");
                    sb.append(Math.round(point.f1589y));
                    sb.append("]");
                }
                sb.append("],\n\t\t\"freq\": ");
        ...
        for (int i4 = 0; i4 < arrayList.size(); i4++) {
            str = str + "," + Math.round(((Point) arrayList.get(i4)).f1588x) + "," + Math.round(((Point) arrayList.get(i4)).f1589y);
        }
