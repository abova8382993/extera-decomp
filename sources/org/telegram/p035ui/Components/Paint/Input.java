...
public class Input {
    ...
    private void fill(Brush brush, boolean z, Runnable runnable) {
        ...
        float fDistance = MathUtils.distance((float) point.f1585x, (float) point.f1586y, 0.0f, 0.0f);
        Point point2 = this.lastLocation;
        float fMax = Math.max(fDistance, MathUtils.distance((float) point2.f1585x, (float) point2.f1586y, size.width, 0.0f));
        Point point3 = this.lastLocation;
        float fDistance2 = MathUtils.distance((float) point3.f1585x, (float) point3.f1586y, 0.0f, size.height);
        Point point4 = this.lastLocation;
        final float fMax2 = Math.max(fMax, Math.max(fDistance2, MathUtils.distance((float) point4.f1585x, (float) point4.f1586y, size.width, size.height))) / 0.84f;
        ...
        final Point point6 = new Point(point5.f1585x, point5.f1586y, 1.0d);
        ...
        this.fillAnimator.addListener(new AnimatorListenerAdapter() { // from class: org.telegram.ui.Components.Paint.Input.1
            ...
            final /* synthetic */ Brush val$finalBrush;
            final /* synthetic */ Runnable val$onDone;
            final /* synthetic */ Point val$point;
            final /* synthetic */ boolean val$registerUndo;

            public C45631(final Point point62, final float fMax22, final Brush brush22, boolean z2, Runnable runnable2) {
                point = point62;
                f = fMax22;
                brush = brush22;
                z = z2;
            ...
            public void onAnimationEnd(Animator animator) {
                ...
                path.setup(Input.this.renderView.getCurrentColor(), f * 1.0f, brush);
                Input.this.renderView.getPainting().commitPath(path, brush.isEraser() ? -1 : Input.this.renderView.getCurrentColor(), z, null);
                if (z) {
    ...
    /* JADX INFO: renamed from: org.telegram.ui.Components.Paint.Input$1 */
    public class C45631 extends AnimatorListenerAdapter {
        final /* synthetic */ float val$R;
        final /* synthetic */ Brush val$finalBrush;
        final /* synthetic */ Runnable val$onDone;
        final /* synthetic */ Point val$point;
    ...
    /* JADX WARN: Removed duplicated region for block: B:144:0x0094 A[PHI: r6
  0x0094: PHI (r6v7 float) = (r6v6 float), (r6v35 float) binds: [B:140:0x0076, B:142:0x0090] A[DONT_GENERATE, DONT_INLINE]] */
    /*
    ...
    */
    public void process(android.view.MotionEvent r23, float r24) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p035ui.Components.Paint.Input.process(android.view.MotionEvent, float):void");
    }
    ...
    public /* synthetic */ void lambda$process$2(float f, Point point, float f2, float[] fArr, double d, boolean[] zArr, ValueAnimator valueAnimator) {
        ...
        double d3 = point.f1585x;
        double d4 = f2;
        double d5 = dCos * d4;
        float f3 = fArr[0];
        double d6 = dSin * d4;
        Point point2 = new Point(d3 + (((double) f3) * d5), point.f1586y + (((double) f3) * d6), d);
        double d7 = fFloatValue;
        paintPath(new Path(new Point[]{point2, new Point(point.f1585x + (d5 * d7), point.f1586y + (d6 * d7), d, true)}));
        double dCos2 = Math.cos(d2 + 2.5918139392115793d);
        double dSin2 = Math.sin(d2 + 2.748893571891069d);
        double d8 = point.f1585x;
        double d9 = dCos2 * d4;
        float f4 = fArr[0];
        double d10 = dSin2 * d4;
        paintPath(new Path(new Point[]{new Point(d8 + (d9 * ((double) f4)), point.f1586y + (((double) f4) * d10), d), new Point(point.f1585x + (d9 * d7), point.f1586y + (d10 * d7), d, true)}));
    ...
    /* JADX INFO: renamed from: org.telegram.ui.Components.Paint.Input$2 */
    public class C45642 extends AnimatorListenerAdapter {
        public C45642() {
        }
    ...
    private void smoothenAndPaintPoints(boolean z, float f) {
        ...
        if (i > 2) {
            ...
            int iMin = (int) Math.min(48.0d, Math.max(Math.floor(pointMultiplySum.getDistanceTo(pointMultiplySum2) / 1.0f), 24.0d));
            float f2 = 1.0f / iMin;
            float f3 = 0.0f;
            int i2 = 0;
            while (i2 < iMin) {
                ...
                this.thicknessSum += pointSmoothPoint.f1587z;
    ...
    private Point smoothPoint(Point point, Point point2, Point point3, float f, float f2) {
        ...
        double d3 = f3 * f3;
        double d4 = f;
        return new Point((point.f1585x * d3) + (point3.f1585x * 2.0d * d4 * d) + (point2.f1585x * d2), (point.f1586y * d3) + (point3.f1586y * 2.0d * d4 * d) + (point2.f1586y * d2), (((((point.f1587z * Math.pow(d, 2.0d)) + (point3.f1587z * ((double) ((2.0f * f3) * f)))) + (point2.f1587z * d2)) - 1.0d) * ((double) AndroidUtilities.lerp(f2, 1.0f, androidx.core.math.MathUtils.clamp(this.realPointsCount / 16.0f, 0.0f, 1.0f)))) + 1.0d);
    }
