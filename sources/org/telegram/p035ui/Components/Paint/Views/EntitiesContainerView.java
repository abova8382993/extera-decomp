...
public abstract class EntitiesContainerView extends FrameLayout {
    ...
    private float f1595px;

    /* JADX INFO: renamed from: py */
    private float f1596py;
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (motionEvent.getPointerCount() == 1) {
            ...
            if (actionMasked == 0) {
                ...
                this.f1595px = motionEvent.getX();
                this.f1596py = motionEvent.getY();
                this.cancelled = false;
            } else if (!this.cancelled && actionMasked == 2) {
                float x = motionEvent.getX();
                float y = motionEvent.getY();
                if (this.hasTransformed || MathUtils.distance(x, y, this.f1595px, this.f1596py) > AndroidUtilities.touchSlop) {
                    this.hasTransformed = true;
                    entityViewOnSelectedEntityRequest.hasPanned = true;
                    entityViewOnSelectedEntityRequest.pan(x - this.f1595px, y - this.f1596py);
                    this.f1595px = x;
                    this.f1596py = y;
                }
