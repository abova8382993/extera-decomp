...
public class TransformableLoginButtonView extends View {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.drawBackground) {
            float fM1036dp = AndroidUtilities.m1036dp(((this.transformType == 0 ? this.progress : 1.0f) * 26.0f) + 6.0f);
            this.rect.set(0.0f, 0.0f, getWidth(), getHeight());
            canvas2.drawRoundRect(this.rect, fM1036dp, fM1036dp, this.backgroundPaint);
        }
        ...
        if (i == 0) {
            ...
            if (textPaint != null && this.buttonText != null) {
                ...
                canvas2.drawText(this.buttonText, (getWidth() - this.buttonWidth) / 2.0f, ((getHeight() / 2.0f) + (this.textPaint.getTextSize() / 2.0f)) - AndroidUtilities.m1036dp(1.75f), this.textPaint);
                this.textPaint.setAlpha(alpha);
            }
            float fMax = (Math.max(0.4f, this.progress) - 0.4f) / 0.6f;
            if (fMax != 0.0f) {
                float fM1036dp2 = AndroidUtilities.m1036dp(21.0f) + ((getWidth() - (AndroidUtilities.m1036dp(21.0f) * 2)) * fMax);
                float height = getHeight() / 2.0f;
                canvas2.drawLine(AndroidUtilities.m1036dp(21.0f), height, fM1036dp2, height, this.outlinePaint);
                double dM1036dp = AndroidUtilities.m1036dp(9.0f) * fMax;
                float fCos = (float) (((double) fM1036dp2) - (Math.cos(0.7853981633974483d) * dM1036dp));
                float fSin = (float) (Math.sin(0.7853981633974483d) * dM1036dp);
                canvas2 = canvas;
                canvas2.drawLine(fM1036dp2, height, fCos, height - fSin, this.outlinePaint);
                canvas2.drawLine(fM1036dp2, height, fCos, height + fSin, this.outlinePaint);
            }
        } else if (i == 1) {
            float fM1036dp3 = AndroidUtilities.m1036dp(21.0f);
            float width = getWidth() - AndroidUtilities.m1036dp(21.0f);
            float height2 = getHeight() / 2.0f;
            canvas2.save();
            canvas2.translate((-AndroidUtilities.m1036dp(2.0f)) * this.progress, 0.0f);
            canvas2.rotate(this.progress * 90.0f, getWidth() / 2.0f, getHeight() / 2.0f);
            canvas2.drawLine(fM1036dp3 + ((width - fM1036dp3) * this.progress), height2, width, height2, this.outlinePaint);
            int iM1036dp = AndroidUtilities.m1036dp((this.progress * (-1.0f)) + 9.0f);
            int iM1036dp2 = AndroidUtilities.m1036dp((this.progress * 7.0f) + 9.0f);
            double d = width;
            double d2 = iM1036dp;
            double d3 = height2;
