...
public class ClippingImageView extends View {
    ...
    public void setAnimationProgress(float f) {
        ...
        float f2 = fArr2[0][0];
        setScaleX(f2 + ((fArr2[1][0] - f2) * f));
        float[][] fArr3 = this.animationValues;
        float f3 = fArr3[0][1];
        setScaleY(f3 + ((fArr3[1][1] - f3) * this.animationProgress));
        float[][] fArr4 = this.animationValues;
        float f4 = fArr4[0][2];
        float f5 = this.additionalTranslationX;
        setTranslationX(f4 + f5 + ((((fArr4[1][2] + f5) - f4) - f5) * this.animationProgress));
        float[][] fArr5 = this.animationValues;
        float f6 = fArr5[0][3];
        setTranslationY(f6 + ((fArr5[1][3] - f6) * this.animationProgress));
        float[][] fArr6 = this.animationValues;
        float f7 = fArr6[0][4];
        setClipHorizontal((int) (f7 + ((fArr6[1][4] - f7) * this.animationProgress)));
        float[][] fArr7 = this.animationValues;
        float f8 = fArr7[0][5];
        setClipTop((int) (f8 + ((fArr7[1][5] - f8) * this.animationProgress)));
        float[][] fArr8 = this.animationValues;
        float f9 = fArr8[0][6];
        setClipBottom((int) (f9 + ((fArr8[1][6] - f9) * this.animationProgress)));
        int i = 0;
        while (true) {
            ...
            int i2 = i + 7;
            float f10 = fArr[0][i2];
            iArr[i] = (int) (f10 + ((fArr[1][i2] - f10) * this.animationProgress));
            setRadius(iArr);
            i++;
        }
        float[] fArr9 = fArr[0];
        if (fArr9.length > 11) {
            float f11 = fArr9[11];
            setImageY((int) (f11 + ((fArr[1][11] - f11) * this.animationProgress)));
            float[][] fArr10 = this.animationValues;
            float f12 = fArr10[0][12];
            setImageX((int) (f12 + ((fArr10[1][12] - f12) * this.animationProgress)));
        }
        if (this.fade) {
            if (!this.f1548in) {
                f = 1.0f - f;
