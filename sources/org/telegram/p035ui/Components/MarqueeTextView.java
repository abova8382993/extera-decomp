...
public class MarqueeTextView extends TextView {
    ...
    private void invalidateGradient() {
        float fMin = Math.min(AndroidUtilities.m1036dp(10.0f) / this.originalWidth, 0.49f);
    ...
    public void onDraw(Canvas canvas) {
        ...
        float fClamp = f < f2 ? MathUtils.clamp(f / AndroidUtilities.m1036dp(10.0f), 0.0f, 1.0f) : 0.0f;
        this.gradientMatrix.reset();
        Matrix matrix = this.gradientMatrix;
        float fM1036dp = AndroidUtilities.m1036dp(10.0f);
        int i = this.originalWidth;
        matrix.postScale(((fM1036dp / i) * (1.0f - fClamp)) + 1.0f, 1.0f, i, 0.0f);
        ...
        if ((this.needMarquee && this.marqueeIsStarted) || !z) {
            float fM1036dp2 = this.scrollX + (AndroidUtilities.m1036dp(60.0f) * (jMin / 1000.0f));
            this.scrollX = fM1036dp2;
            if (fM1036dp2 > measuredWidth + iM1036dp) {
                stopMarqueeInternal();
            }
            invalidate();
        }
