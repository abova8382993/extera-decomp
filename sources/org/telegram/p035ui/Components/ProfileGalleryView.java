...
import org.telegram.p035ui.Components.CircularViewPager;
...
public class ProfileGalleryView extends CircularViewPager implements NotificationCenter.NotificationCenterDelegate {
        ...
        if (action == 0) {
            ...
            if (!this.isDownReleased) {
                ...
                if (realCount > 1) {
                    ...
                    ViewPagerAdapter viewPagerAdapter = this.adapter;
                    if (x > width) {
                        extraCount = viewPagerAdapter.getExtraCount();
                        int i = currentItem + 1;
                        ...
                        extraCount = i2 < viewPagerAdapter.getExtraCount() ? (realCount + r0) - 1 : i2;
                    }
            ...
            boolean z = Math.abs(y) >= ((float) this.touchSlop) || Math.abs(x2) >= ((float) this.touchSlop);
            if (z) {
                this.isDownReleased = true;
            ...
            } else if (z2 && !canScrollHorizontally(-1) && x2 > this.touchSlop) {
                return false;
            }
        }
    ...
    public ImageLocation getCurrentAvatarImageLocation() {
        int currentAvatarPhotoIndex = getCurrentAvatarPhotoIndex();
        if (currentAvatarPhotoIndex >= 0) {
            return getImageLocation(currentAvatarPhotoIndex);
        }
        int realPosition = getRealPosition();
        if (realPosition >= 0) {
            return getImageLocation(realPosition);
        }
        return null;
    }
    ...
    public void resetCurrentItem() {
        int currentAvatarPage;
        if (this.adapter == null || (currentAvatarPage = getCurrentAvatarPage()) < 0 || getCurrentItem() == currentAvatarPage) {
            return;
        }
        setCurrentItem(currentAvatarPage, false);
    }
    ...
    private int getCurrentAvatarPage() {
        ...
        int currentAvatarPhotoIndex = getCurrentAvatarPhotoIndex();
        if (currentAvatarPhotoIndex >= 0) {
            return this.adapter.getExtraCount() + currentAvatarPhotoIndex;
        }
        int currentItem = getCurrentItem();
        return (currentItem < 0 || currentItem >= count) ? this.adapter.getExtraCount() : currentItem;
    }

    private int getCurrentAvatarPhotoIndex() {
        TLRPC.Photo currentAvatarPhoto = getCurrentAvatarPhoto();
        if (currentAvatarPhoto == null) {
            return -1;
        }
        for (int i = 0; i < this.photos.size(); i++) {
            TLRPC.Photo photo = this.photos.get(i);
            if (photo != null && photo.f1276id == currentAvatarPhoto.f1276id) {
                return i;
            }
        }
        return -1;
    ...
    private TLRPC.Photo getCurrentAvatarPhoto() {
        ...
        if (DialogObject.isChatDialog(this.dialogId)) {
            ...
            if (chatFull != null) {
                return chatFull.chat_photo;
            }
            return null;
        }
        MessagesController messagesController = MessagesController.getInstance(this.currentAccount);
        TLRPC.User user = messagesController.getUser(Long.valueOf(this.dialogId));
        TLRPC.UserFull userFull = messagesController.getUserFull(this.dialogId);
        if (user == null || userFull == null) {
            return null;
        }
        TLRPC.UserProfilePhoto userProfilePhoto = user.photo;
        return (userProfilePhoto == null || !userProfilePhoto.personal || (photo = userFull.personal_photo) == null) ? userFull.profile_photo : photo;
    }
    ...
    public boolean removePhotoAtIndex(int i) {
        ...
        if (photo != null && (dialogPhotos = this.dialogPhotos) != null) {
            dialogPhotos.removePhoto(photo.f1276id);
            return true;
        }
        this.photos.remove(i);
        this.thumbsFileNames.remove(i);
    ...
    */
    public void didReceivedNotification(int r18, int r19, java.lang.Object... r20) {
        ...
        */
        throw new UnsupportedOperationException("Method not decompiled: org.telegram.p035ui.Components.ProfileGalleryView.didReceivedNotification(int, int, java.lang.Object[]):void");
    }
            ...
            if (pinchToZoomHelper == null || !pinchToZoomHelper.isInOverlayMode()) {
                if (this.radialProgress != null) {
                    ...
                    if (ProfileGalleryView.this.roundTopRadius == 0 && ProfileGalleryView.this.roundBottomRadius == 0) {
                        ...
                        canvas2 = canvas;
                        int i = ProfileGalleryView.this.roundTopRadius;
                        int i2 = ProfileGalleryView.this.roundBottomRadius;
                        ProfileGalleryView profileGalleryView2 = ProfileGalleryView.this;
                        if (i == i2) {
                            profileGalleryView2.rect.set(0.0f, 0.0f, getWidth(), getHeight());
                            canvas2.drawRoundRect(ProfileGalleryView.this.rect, r10.roundTopRadius, ProfileGalleryView.this.roundTopRadius, this.placeholderPaint);
                        } else {
                            profileGalleryView2.path.reset();
                            ProfileGalleryView.this.rect.set(0.0f, 0.0f, getWidth(), getHeight());
                            int i3 = 0;
                            while (true) {
                                profileGalleryView = ProfileGalleryView.this;
                                if (i3 >= 4) {
                                    break;
                                }
                                profileGalleryView.radii[i3] = profileGalleryView.roundTopRadius;
                                ProfileGalleryView.this.radii[i3 + 4] = r0.roundBottomRadius;
                                i3++;
                            }
