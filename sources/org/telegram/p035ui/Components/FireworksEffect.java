...
public class FireworksEffect {
    ...
    public class Particle {
        ...
        public void draw(Canvas canvas) {
            ...
            FireworksEffect.this.particlePaint.setStrokeWidth(AndroidUtilities.m1036dp(1.5f) * this.scale);
            FireworksEffect.this.particlePaint.setAlpha((int) (this.alpha * 255.0f));
            canvas.drawPoint(this.f1566x, this.f1567y, FireworksEffect.this.particlePaint);
        }
    }
    ...
    private void updateParticles(long j) {
        ...
        while (i < size) {
            ...
            if (f >= f2) {
                ...
                float f3 = particle.f1566x;
                float f4 = particle.f1564vx;
                float f5 = particle.velocity;
                float f6 = j;
                particle.f1566x = f3 + (((f4 * f5) * f6) / 500.0f);
                float f7 = particle.f1567y;
                float f8 = particle.f1565vy;
                particle.f1567y = f7 + (((f5 * f8) * f6) / 500.0f);
                particle.f1565vy = f8 + (f6 / 100.0f);
    ...
    public void onDraw(View view, Canvas canvas) {
        ...
        if (Utilities.random.nextBoolean() && this.particles.size() + 8 < 150) {
            ...
            float fNextFloat2 = i2 + (Utilities.random.nextFloat() * ((view.getMeasuredHeight() - AndroidUtilities.m1036dp(20.0f)) - i2));
            ...
            for (int i4 = 0; i4 < 8; i4++) {
                ...
                particle.f1566x = fNextFloat;
                particle.f1567y = fNextFloat2;
                particle.f1564vx = fCos * 1.5f;
                particle.f1565vy = fSin;
                particle.color = i3;
                particle.alpha = 1.0f;
