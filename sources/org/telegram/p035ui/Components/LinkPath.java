...
public class LinkPath extends CornerPath {
    ...
    private WeakReference<Layout> currentLayout;
    ...
    @Override // org.telegram.p035ui.Components.CornerPath, android.graphics.Path
    public void addRect(float f, float f2, float f3, float f4, Path.Direction direction) {
        WeakReference<Layout> weakReference = this.currentLayout;
        ...
        try {
            ...
            float lineRight = this.currentLayout.get().getLineRight(this.currentLine);
            float lineLeft = this.currentLayout.get().getLineLeft(this.currentLine);
            if (f < lineRight) {
                if (f > lineLeft || f3 > lineLeft) {
                    ...
                    if (Build.VERSION.SDK_INT < 28) {
                        spacingAdd -= spacingAdd != ((float) this.currentLayout.get().getHeight()) ? this.currentLayout.get().getSpacingAdd() : 0.0f;
                    } else if (spacingAdd - f6 > this.lineHeight) {
                        spacingAdd = this.yOffset + (spacingAdd != ((float) this.currentLayout.get().getHeight()) ? this.currentLayout.get().getLineBottom(this.currentLine) - this.currentLayout.get().getSpacingAdd() : 0.0f);
                    }
