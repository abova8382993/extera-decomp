...
public class StickerSetLinkIcon extends Drawable {
    ...
    public boolean equals(ArrayList<TLRPC.Document> arrayList) {
        ...
        if (arrayList == null) {
            return animatedEmojiDrawableArr.length == 0;
        }
        ...
        int i = 0;
        while (true) {
            AnimatedEmojiDrawable[] animatedEmojiDrawableArr2 = this.drawables;
            if (i >= animatedEmojiDrawableArr2.length) {
                return true;
            }
            TLRPC.Document document = animatedEmojiDrawableArr2[i].getDocument();
            if ((document == null ? 0L : document.f1253id) != arrayList.get(i).f1253id) {
                return false;
            }
            i++;
        }
    ...
    public void draw(Canvas canvas) {
        ...
        float intrinsicWidth = getIntrinsicWidth() / this.f1694N;
        float intrinsicHeight = getIntrinsicHeight() / this.f1694N;
        canvas.save();
        canvas.clipRect(fCenterX, fCenterY, getIntrinsicWidth() + fCenterX, getIntrinsicHeight() + fCenterY);
        for (int i = 0; i < this.f1694N; i++) {
            int i2 = 0;
            while (true) {
                int i3 = this.f1694N;
                if (i2 < i3) {
