...
public class TimerDrawable extends Drawable {
    ...
    public void setTime(int i) {
        ...
        if (this.time != i) {
            ...
            if (z) {
                this.currentTtlIcon = ContextCompat.getDrawable(context, C2797R.drawable.msg_autodelete_badge2).mutate();
            } else {
                Drawable drawableMutate = ContextCompat.getDrawable(context, i == 0 ? C2797R.drawable.msg_mini_autodelete : C2797R.drawable.msg_mini_autodelete_empty).mutate();
            ...
            if (i2 >= 1 && i2 < 60) {
                ...
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerSeconds);
                }
            } else if (i2 >= 60 && i2 < 3600) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + (i / 60);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerMinutes);
                }
            } else if (i2 >= 3600 && i2 < 86400) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((i / 60) / 60);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerHours);
                }
            } else if (i2 >= 86400 && i2 < 604800) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + (((i / 60) / 60) / 24);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerDays);
                }
            } else if (i2 < 2678400) {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((((i / 60) / 60) / 24) / 7);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerWeeks);
                ...
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerMonths);
                }
            } else {
                str = _UrlKt.FRAGMENT_ENCODE_SET + ((((i / 60) / 60) / 24) / 364);
                if (str.length() < 2) {
                    str = str + LocaleController.getString(C2797R.string.SecretChatTimerYears);
                }
            }
            String str2 = str;
            this.timePaint.setTextSize(AndroidUtilities.m1036dp(11.0f));
            float fMeasureText = this.timePaint.measureText(str2);
            this.timeWidth = fMeasureText;
            if (fMeasureText > AndroidUtilities.m1036dp(13.0f)) {
                this.timePaint.setTextSize(AndroidUtilities.m1036dp(9.0f));
                this.timeWidth = this.timePaint.measureText(str2);
            }
            if (this.timeWidth > AndroidUtilities.m1036dp(13.0f)) {
                this.timePaint.setTextSize(AndroidUtilities.m1036dp(6.0f));
                this.timeWidth = this.timePaint.measureText(str2);
            }
            try {
    ...
    public void draw(Canvas canvas) {
        ...
        if (this.currentTtlIcon != null) {
            ...
            if (this.isDialog) {
                ...
                rect.set(getBounds().centerX() - AndroidUtilities.m1036dp(10.5f), getBounds().centerY() - AndroidUtilities.m1036dp(10.5f), (getBounds().centerX() - AndroidUtilities.m1036dp(10.5f)) + this.currentTtlIcon.getIntrinsicWidth(), (getBounds().centerY() - AndroidUtilities.m1036dp(10.5f)) + this.currentTtlIcon.getIntrinsicHeight());
    ...
    public int getIntrinsicWidth() {
        return AndroidUtilities.m1036dp(23.0f);
    }
