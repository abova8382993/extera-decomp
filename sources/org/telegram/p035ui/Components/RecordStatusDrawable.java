...
public class RecordStatusDrawable extends StatusDrawable {
    ...
    public void draw(Canvas canvas) {
        ...
        Paint paint2 = paint;
        if (paint2.getStrokeWidth() != AndroidUtilities.m1036dp(2.0f)) {
            paint2.setStrokeWidth(AndroidUtilities.m1036dp(2.0f));
        }
        canvas.save();
        canvas.translate(0.0f, (getIntrinsicHeight() / 2) + AndroidUtilities.m1036dp(this.isChat ? 1.0f : 2.0f));
        for (int i = 0; i < 4; i++) {
            if (i == 0) {
                ...
                int i2 = this.alpha;
                if (i == 3) {
                    paint2.setAlpha((int) (i2 * (1.0f - this.progress)));
                } else {
                    paint2.setAlpha(i2);
                }
            }
            float fM1036dp = (AndroidUtilities.m1036dp(4.0f) * i) + (AndroidUtilities.m1036dp(4.0f) * this.progress);
            float f = -fM1036dp;
            this.rect.set(f, f, fM1036dp, fM1036dp);
