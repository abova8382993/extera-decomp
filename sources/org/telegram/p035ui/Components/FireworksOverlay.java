...
public class FireworksOverlay extends View {
    ...
    public class Particle {
        ...
        public void draw(Canvas canvas) {
            byte b2 = this.type;
            if (b2 == 0) {
                canvas.drawCircle(this.f1568x, this.f1569y, AndroidUtilities.m1036dp(this.typeSize), FireworksOverlay.paint[this.colorType]);
                return;
            }
            if (b2 == 1) {
                FireworksOverlay.this.rect.set(this.f1568x - AndroidUtilities.m1036dp(this.typeSize), this.f1569y - AndroidUtilities.m1036dp(2.0f), this.f1568x + AndroidUtilities.m1036dp(this.typeSize), this.f1569y + AndroidUtilities.m1036dp(2.0f));
                canvas.save();
                canvas.rotate(this.rotation, FireworksOverlay.this.rect.centerX(), FireworksOverlay.this.rect.centerY());
                canvas.drawRoundRect(FireworksOverlay.this.rect, AndroidUtilities.m1036dp(2.0f), AndroidUtilities.m1036dp(2.0f), FireworksOverlay.paint[this.colorType]);
                canvas.restore();
                return;
            }
            if (b2 == 2) {
                ...
                if (drawable != null) {
                    ...
                    float f = this.f1568x;
                    float f2 = this.f1569y;
                    drawable.setBounds(((int) f) - intrinsicWidth, ((int) f2) - intrinsicHeight, ((int) f) + intrinsicWidth, ((int) f2) + intrinsicHeight);
                    canvas.save();
                    canvas.rotate(this.rotation, this.f1568x, this.f1569y);
                    byte b3 = this.typeSize;
                    canvas.scale(b3 / 6.0f, b3 / 6.0f, this.f1568x, this.f1569y);
        ...
        public boolean update(int i) {
            float f = i / 16.0f;
            float f2 = this.f1568x;
            float f3 = this.moveX;
            this.f1568x = f2 + (f3 * f);
            this.f1569y += this.moveY * f;
            if (this.xFinished != 0) {
                float fM1036dp = AndroidUtilities.m1036dp(1.0f) * 0.5f;
                byte b2 = this.xFinished;
                float f4 = this.moveX;
                if (b2 == 1) {
                    float f5 = f4 + (fM1036dp * f * 0.05f);
                    this.moveX = f5;
                    if (f5 >= fM1036dp) {
                        this.xFinished = (byte) 2;
                    }
                } else {
                    float f6 = f4 - ((fM1036dp * f) * 0.05f);
                    this.moveX = f6;
                    if (f6 <= (-fM1036dp)) {
            ...
            float f9 = (-AndroidUtilities.m1036dp(1.0f)) / 2.0f;
            float f10 = this.moveY;
            boolean z = f10 < f9;
            if (f10 > f9) {
                this.moveY = f10 + ((AndroidUtilities.m1036dp(1.0f) / 3.0f) * f * FireworksOverlay.this.speedCoef);
            } else {
                this.moveY = f10 + ((AndroidUtilities.m1036dp(1.0f) / 3.0f) * f);
            }
            if (z && this.moveY > f9) {
            ...
            return this.f1569y >= ((float) FireworksOverlay.this.getHeightForAnimation());
        }
    }
    ...
    private void loadHeartDrawables() {
        ...
        while (true) {
            ...
            drawableArr[i] = ApplicationLoader.applicationContext.getResources().getDrawable(C2797R.drawable.heart_confetti).mutate();
    ...
    private void loadStarsDrawables() {
        ...
        while (true) {
            ...
            drawableArr[i] = ApplicationLoader.applicationContext.getResources().getDrawable(C2797R.drawable.msg_settings_premium).mutate();
    ...
    private Particle createParticle(boolean z) {
        ...
        try {
            ...
            if (z) {
                particle.f1569y = (-Utilities.random.nextFloat()) * getHeightForAnimation() * 1.2f;
                particle.f1568x = AndroidUtilities.m1036dp(5.0f) + Utilities.random.nextInt(Math.max(1, getWidthForAnimation() - AndroidUtilities.m1036dp(10.0f)));
                particle.xFinished = particle.finishedStart;
                return particle;
            }
            int iM1036dp = AndroidUtilities.m1036dp(Utilities.random.nextInt(10) + 4);
            int heightForAnimation = getHeightForAnimation() / 4;
            if (particle.side == 0) {
                particle.f1568x = -iM1036dp;
            } else {
                particle.f1568x = getWidthForAnimation() + iM1036dp;
            }
            particle.moveX = (particle.side == 0 ? 1 : -1) * (AndroidUtilities.m1036dp(1.2f) + (Utilities.random.nextFloat() * AndroidUtilities.m1036dp(4.0f)));
            particle.moveY = -(AndroidUtilities.m1036dp(4.0f) + (Utilities.random.nextFloat() * AndroidUtilities.m1036dp(4.0f)));
            particle.f1569y = (heightForAnimation / 2) + Utilities.random.nextInt(Math.max(1, heightForAnimation * 2));
            return particle;
        } catch (Exception e) {
            FileLog.m1048e(e);
            return particle;
