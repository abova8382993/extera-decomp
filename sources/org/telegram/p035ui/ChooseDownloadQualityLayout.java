...
public class ChooseDownloadQualityLayout {
    ...
    public boolean update(final MessageObject messageObject) {
        ...
        ArrayList<VideoPlayer.Quality> qualities = VideoPlayer.getQualities(i, messageMedia.document, messageMedia.alt_documents, 0, false);
        this.buttonsLayout.removeAllViews();
        for (int i2 = 0; i2 < qualities.size(); i2++) {
            final VideoPlayer.Quality quality = qualities.get(i2);
            VideoPlayer.VideoUri downloadUri = quality.getDownloadUri();
            StringBuilder sb = new StringBuilder();
            sb.append(LocaleController.formatString(C2797R.string.QualitySaveIn, Integer.valueOf(quality.m1170p())));
            sb.append(quality.original ? " (" + LocaleController.getString(C2797R.string.QualitySource) + ")" : _UrlKt.FRAGMENT_ENCODE_SET);
            ...
            if (downloadUri.isCached()) {
                ...
                spannableStringBuilder.append(LocaleController.getString(C2797R.string.QualityCached));
            } else {
                SpannableString spannableString = new SpannableString("s ");
                ColoredImageSpan coloredImageSpan = new ColoredImageSpan(C2797R.drawable.msg_mini_arrow_mediabold);
