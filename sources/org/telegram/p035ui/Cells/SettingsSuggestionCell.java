...
public abstract class SettingsSuggestionCell extends LinearLayout {
    ...
    public void setType(int i) {
        ...
        if (i != 0) {
            if (i == 1) {
                this.textView.setText(LocaleController.getString(C2797R.string.YourPasswordHeader));
                this.detailTextView.setText(LocaleController.getString(C2797R.string.YourPasswordRemember));
                this.yesButton.setText(LocaleController.getString(C2797R.string.YourPasswordRememberYes));
                this.noButton.setVisibility(0);
                this.noButton.setText(LocaleController.getString(C2797R.string.YourPasswordRememberNo));
                return;
            }
            if (i == 2) {
                this.textView.setText(LocaleController.getString(C2797R.string.GraceSuggestionTitle));
                this.detailTextView.setText(LocaleController.getString(C2797R.string.GraceSuggestionMessage));
                this.yesButton.setText(LocaleController.getString(C2797R.string.GraceSuggestionButton));
        ...
        this.textView.setText(LocaleController.formatString(C2797R.string.CheckPhoneNumber, PhoneFormat.getInstance().format("+" + user.phone)));
        String string = LocaleController.getString(C2797R.string.CheckPhoneNumberInfo);
        SpannableStringBuilder spannableStringBuilder = new SpannableStringBuilder(string);
        int iIndexOf = string.indexOf("**");
        int iLastIndexOf = string.lastIndexOf("**");
        if (iIndexOf >= 0 && iLastIndexOf >= 0 && iIndexOf != iLastIndexOf) {
            spannableStringBuilder.replace(iLastIndexOf, iLastIndexOf + 2, (CharSequence) _UrlKt.FRAGMENT_ENCODE_SET);
            spannableStringBuilder.replace(iIndexOf, iIndexOf + 2, (CharSequence) _UrlKt.FRAGMENT_ENCODE_SET);
            try {
                spannableStringBuilder.setSpan(new URLSpanNoUnderline(LocaleController.getString(C2797R.string.CheckPhoneNumberLearnMoreUrl)), iIndexOf, iLastIndexOf - 2, 33);
            } catch (Exception e) {
                FileLog.m1048e(e);
            }
        }
        this.detailTextView.setText(spannableStringBuilder);
        this.yesButton.setText(LocaleController.getString(C2797R.string.CheckPhoneNumberYes));
        this.noButton.setVisibility(0);
        this.noButton.setText(LocaleController.getString(C2797R.string.CheckPhoneNumberNo));
    }
