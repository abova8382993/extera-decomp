...
final class Constants {
    ...
    public static boolean isWhitelisted(String str) {
        if (str.startsWith("org/mvel2/asm/")) {
            return str.contains("Test$") || Pattern.matches("org/mvel2/asm/util/Trace(Annotation|Class|Field|Method|Module|RecordComponent|Signature)Visitor(\\$.*)?", str) || Pattern.matches("org/mvel2/asm/util/Check(Annotation|Class|Field|Method|Module|RecordComponent|Signature)Adapter(\\$.*)?", str);
        }
        return false;
    }
    ...
    public static void checkIsPreview(InputStream inputStream) {
        if (inputStream == null) {
            Segment$$ExternalSyntheticBUOutline1.m992m("Bytecode not available, can't check class version");
            return;
        }
        try {
            ...
            try {
                ...
                Segment$$ExternalSyntheticBUOutline1.m992m("ASM9_EXPERIMENTAL can only be used by classes compiled with --enable-preview");
            } catch (Throwable th) {
                try {
                    dataInputStream.close();
            ...
            Constants$$ExternalSyntheticBUOutline0.m1007m("I/O error, can't check class version", e);
        }
    }
}
