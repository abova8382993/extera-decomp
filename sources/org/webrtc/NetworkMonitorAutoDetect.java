...
public class NetworkMonitorAutoDetect extends BroadcastReceiver implements NetworkChangeDetector {
    ...
    public class SimpleNetworkCallback extends ConnectivityManager.NetworkCallback {
        ...
        public void onAvailable(Network network) {
            Logging.m1252d(NetworkMonitorAutoDetect.TAG, "Network becomes available: " + network.toString());
            onNetworkChanged(network);
        }

        @Override // android.net.ConnectivityManager.NetworkCallback
        public void onCapabilitiesChanged(Network network, NetworkCapabilities networkCapabilities) {
            Logging.m1252d(NetworkMonitorAutoDetect.TAG, "capabilities changed: " + networkCapabilities.toString());
            onNetworkChanged(network);
        }

        @Override // android.net.ConnectivityManager.NetworkCallback
        public void onLinkPropertiesChanged(Network network, LinkProperties linkProperties) {
            Logging.m1252d(NetworkMonitorAutoDetect.TAG, "link properties changed");
            onNetworkChanged(network);
        }

        @Override // android.net.ConnectivityManager.NetworkCallback
        public void onLosing(Network network, int i) {
            Logging.m1252d(NetworkMonitorAutoDetect.TAG, "Network " + network.toString() + " is about to lose in " + i + "ms");
        }

        @Override // android.net.ConnectivityManager.NetworkCallback
        public void onLost(Network network) {
            Logging.m1252d(NetworkMonitorAutoDetect.TAG, "Network " + network.toString() + " is disconnected");
    ...
    public static class ConnectivityManagerDelegate {
        ...
        public NetworkState getNetworkState(Network network) {
            ...
            if (networkInfo == null) {
                Logging.m1256w(NetworkMonitorAutoDetect.TAG, "Couldn't retrieve information from network " + network.toString());
                return new NetworkState(false, -1, -1, -1, -1);
            }
            ...
            for (Network network : getAllNetworks()) {
                if (hasInternetCapability(network) && (networkInfo = this.connectivityManager.getNetworkInfo(network)) != null && networkInfo.getType() == activeNetworkInfo.getType()) {
                    if (jNetworkToNetId != -1) {
                        GlShader$$ExternalSyntheticBUOutline1.m1250m("Multiple connected networks of same type are not supported.");
                        return 0L;
                    }
        ...
        public NetworkChangeDetector.NetworkInformation networkToInfo(Network network) {
            ...
            if (linkProperties == null) {
                Logging.m1256w(NetworkMonitorAutoDetect.TAG, "Detected unknown network: " + network.toString());
                return null;
            }
            if (linkProperties.getInterfaceName() == null) {
                Logging.m1256w(NetworkMonitorAutoDetect.TAG, "Null interface name for network " + network.toString());
                return null;
            }
            ...
            if (connectionType == NetworkChangeDetector.ConnectionType.CONNECTION_UNKNOWN || connectionType == NetworkChangeDetector.ConnectionType.CONNECTION_UNKNOWN_CELLULAR) {
                Logging.m1252d(NetworkMonitorAutoDetect.TAG, "Network " + network.toString() + " connection type is " + connectionType + " because it has type " + networkState.getNetworkType() + " and subtype " + networkState.getNetworkSubType());
            }
        ...
        public void releaseCallback(ConnectivityManager.NetworkCallback networkCallback) {
            if (supportNetworkCallback()) {
                Logging.m1252d(NetworkMonitorAutoDetect.TAG, "Unregister network callback");
    ...
    public static class WifiManagerDelegate {
        ...
        public String getWifiSSID() {
            ...
            if (i >= 33) {
                intentRegisterReceiver = context.registerReceiver(null, new IntentFilter("android.net.wifi.STATE_CHANGE"), 4);
            } else {
                intentRegisterReceiver = context.registerReceiver(null, new IntentFilter("android.net.wifi.STATE_CHANGE"));
            }
            return (intentRegisterReceiver == null || (wifiInfo = (WifiInfo) intentRegisterReceiver.getParcelableExtra("wifiInfo")) == null || (ssid = wifiInfo.getSSID()) == null) ? _UrlKt.FRAGMENT_ENCODE_SET : ssid;
        }
    }
    ...
    public static class WifiDirectManagerDelegate extends BroadcastReceiver {
        ...
        public void lambda$new$0(WifiP2pGroup wifiP2pGroup) {
            ...
            try {
                ...
                Logging.m1254e(NetworkMonitorAutoDetect.TAG, "Unable to get WifiP2p network interface", e);
            }
        }
    ...
    public NetworkMonitorAutoDetect(NetworkChangeDetector.Observer observer, Context context) {
        ...
        if (this.connectivityManagerDelegate.supportNetworkCallback()) {
            ...
            try {
                ...
                Logging.m1256w(TAG, "Unable to obtain permission to request a cellular network.");
    ...
    private void registerReceiver() {
        ...
        Context context = this.context;
        if (i >= 33) {
            context.registerReceiver(this, this.intentFilter, 4);
        } else {
    ...
    private void connectionTypeChanged(NetworkState networkState) {
        ...
        Logging.m1252d(TAG, "Network connectivity changed, type is: " + this.connectionType);
