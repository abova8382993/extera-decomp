...
public class WebRtcAudioRecord {
    ...
    public static void setErrorCallback(WebRtcAudioRecordErrorCallback webRtcAudioRecordErrorCallback) {
        Logging.m1252d(TAG, "Set error callback");
    ...
    public WebRtcAudioRecord(long j, int i) {
    ...
    private boolean enableBuiltInAEC(boolean z) {
        ...
        if (webRtcAudioEffects == null) {
            Logging.m1253e(TAG, "Built-in AEC is not supported on this platform");
            return false;
        }
    ...
    private boolean enableBuiltInNS(boolean z) {
        ...
        WebRtcAudioEffects webRtcAudioEffects = this.effects;
        if (webRtcAudioEffects == null) {
            Logging.m1253e(TAG, "Built-in NS is not supported on this platform");
            return false;
        }
        ...
        if (this.captureType == 1) {
            if (Build.VERSION.SDK_INT >= 29) {
                try {
                    ...
                    WebRtcAudioRecord$$ExternalSyntheticApiModelOutline1.m1263m();
                    AudioPlaybackCaptureConfiguration.Builder builderM1262m = WebRtcAudioRecord$$ExternalSyntheticApiModelOutline0.m1262m(mediaProjection);
                    builderM1262m.addMatchingUsage(1);
                    builderM1262m.addMatchingUsage(14);
                    builderM1262m.addMatchingUsage(0);
                    AudioRecord.Builder builder = new AudioRecord.Builder();
                    builder.setAudioPlaybackCaptureConfig(builderM1262m.build());
                    builder.setAudioFormat(new AudioFormat.Builder().setChannelMask(iChannelCountToConfiguration).setSampleRate(i).setEncoding(2).build());
    ...
    public void initDeviceAudioRecord(MediaProjection mediaProjection) {
        ...
        try {
            ...
            AudioRecord.Builder builder = new AudioRecord.Builder();
            builder.setAudioPlaybackCaptureConfig(builderM1262m.build());
            builder.setAudioFormat(new AudioFormat.Builder().setChannelMask(iChannelCountToConfiguration).setSampleRate(this.requestedSampleRate).setEncoding(2).build());
            builder.setBufferSizeInBytes(iMax);
            AudioRecord audioRecordBuild = builder.build();
    ...
    public static synchronized void setAudioSource(int i) {
        Logging.m1256w(TAG, "Audio source is changed from: " + audioSource + " to " + i);
        audioSource = i;
    }
    ...
    private void releaseAudioResources(boolean z) {
        Logging.m1252d(TAG, "releaseAudioResources " + z);
        if (z) {
            AudioRecord audioRecord = this.deviceAudioRecord;
            if (audioRecord != null) {
                audioRecord.release();
