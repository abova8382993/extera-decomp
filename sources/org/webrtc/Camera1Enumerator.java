...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                Logging.m1252d(TAG, "Index: " + i + ". " + deviceName);
            } else {
                Logging.m1253e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            Logging.m1254e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        Logging.m1252d(TAG, "Get supported formats for camera index " + i + ".");
        long jElapsedRealtime = SystemClock.elapsedRealtime();
        Camera cameraOpen = null;
        try {
            try {
                Logging.m1252d(TAG, "Opening camera with index " + i);
                ...
                try {
                    ...
                    Logging.m1254e(TAG, "getSupportedFormats() failed on camera index " + i, e);
                }
                Logging.m1252d(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (SystemClock.elapsedRealtime() - jElapsedRealtime) + " ms.");
                return arrayList;
            } catch (Throwable th) {
                if (cameraOpen != null) {
                    cameraOpen.release();
            ...
            Logging.m1254e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    public static int getCameraIndex(String str) {
        Logging.m1252d(TAG, "getCameraIndex: " + str);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            if (str.equals(getDeviceName(i))) {
                return i;
            }
        }
        Native$$ExternalSyntheticBUOutline5.m554m("No such camera: ", str);
        return 0;
    }
