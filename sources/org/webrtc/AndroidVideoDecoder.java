...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
    ...
    AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String str, VideoCodecMimeType videoCodecMimeType, int i, EglBase.Context context) {
        ...
        Logging.m1354d(TAG, "ctor name: " + str + " type: " + videoCodecMimeType + " color format: " + i + " context: " + context);
    ...
    private VideoCodecStatus initDecodeInternal(int i, int i2) {
        ...
        Logging.m1354d(TAG, "initDecodeInternal name: " + this.codecName + " type: " + this.codecType + " width: " + i + " height: " + i2 + " color format: " + this.colorFormat);
        if (this.outputThread != null) {
            Logging.m1355e(TAG, "initDecodeInternal called while the codec is already running");
        ...
        try {
            ...
            try {
                ...
                Logging.m1354d(TAG, "initDecodeInternal done");
                return VideoCodecStatus.OK;
            } catch (IllegalArgumentException e) {
                e = e;
                Logging.m1356e(TAG, "initDecode failed", e);
                release();
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            ...
            Logging.m1355e(TAG, "Cannot create media decoder " + this.codecName);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    public VideoCodecStatus decode(EncodedImage encodedImage, VideoDecoder.DecodeInfo decodeInfo) {
        ...
        if (byteBuffer == null) {
            Logging.m1355e(TAG, "decode() - no input data");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        int iRemaining = byteBuffer.remaining();
        if (iRemaining == 0) {
            Logging.m1355e(TAG, "decode() - input buffer empty");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Logging.m1356e(TAG, "queueInputBuffer failed", e);
                    this.frameInfos.pollLast();
                    return VideoCodecStatus.ERROR;
                }
            } catch (IllegalStateException e2) {
                Logging.m1356e(TAG, "getInputBuffer with index=" + iDequeueInputBuffer + " failed", e2);
                return VideoCodecStatus.ERROR;
            }
        } catch (IllegalStateException e3) {
            Logging.m1356e(TAG, "dequeueInputBuffer failed", e3);
            return VideoCodecStatus.ERROR;
        }
    }
    ...
    private VideoCodecStatus releaseInternal() {
        if (!this.running) {
            Logging.m1354d(TAG, "release: Decoder is not running.");
            return VideoCodecStatus.OK;
        }
        try {
            this.running = false;
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, 5000L)) {
                Logging.m1356e(TAG, "Media decoder release timeout", new RuntimeException());
                return VideoCodecStatus.TIMEOUT;
            }
            if (this.shutdownException != null) {
                Logging.m1356e(TAG, "Media decoder release error", new RuntimeException(this.shutdownException));
    ...
    protected void deliverDecodedFrame() throws Throwable {
        ...
        try {
            ...
            if (iDequeueOutputBuffer < 0) {
                Logging.m1357v(TAG, "dequeueOutputBuffer returned " + iDequeueOutputBuffer);
                return;
            }
            ...
            Logging.m1356e(TAG, "deliverDecodedFrame failed", e);
        }
    }
    ...
    private void deliverByteFrame(int i, MediaCodec.BufferInfo bufferInfo, int i2, Integer num) throws Throwable {
        ...
        if (i7 < ((i3 * i4) * 3) / 2) {
            Logging.m1355e(TAG, "Insufficient output buffer size: " + bufferInfo.size);
            return;
        }
    ...
    private void reformat(MediaFormat mediaFormat) {
        ...
        int integer2;
        this.outputThreadChecker.checkIsOnValidThread();
        Logging.m1354d(TAG, "Decoder format changed: " + mediaFormat);
        ...
        synchronized (this.dimensionLock) {
            try {
                if (integer != this.width || integer2 != this.height) {
                    ...
                    Logging.m1358w(TAG, "Unexpected format dimensions. Configured " + this.width + "*" + this.height + ". New " + integer + "*" + integer2 + ". Skip it");
                    return;
                }
                if (this.surfaceTextureHelper == null && mediaFormat.containsKey("color-format")) {
                    this.colorFormat = mediaFormat.getInteger("color-format");
                    Logging.m1354d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
                ...
                synchronized (this.dimensionLock) {
                    try {
                        ...
                        Logging.m1354d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
    ...
    public void releaseCodecOnOutputThread() {
        this.outputThreadChecker.checkIsOnValidThread();
        Logging.m1354d(TAG, "Releasing MediaCodec on output thread");
        try {
            this.codec.stop();
        } catch (Exception e) {
            Logging.m1356e(TAG, "Media decoder stop failed", e);
        }
        try {
            this.codec.release();
        } catch (Exception e2) {
            Logging.m1356e(TAG, "Media decoder release failed", e2);
            this.shutdownException = e2;
        }
        Logging.m1354d(TAG, "Release on output thread done");
    }
