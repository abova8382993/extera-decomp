...
class AndroidVideoDecoder implements VideoDecoder, VideoSink {
    ...
    public AndroidVideoDecoder(MediaCodecWrapperFactory mediaCodecWrapperFactory, String str, VideoCodecMimeType videoCodecMimeType, int i, EglBase.Context context) {
        if (!isSupportedColorFormat(i)) {
            CharCodeKt$$ExternalSyntheticBUOutline0.m873m("Unsupported color format: ", i);
            throw null;
        }
        Logging.m1252d(TAG, "ctor name: " + str + " type: " + videoCodecMimeType + " color format: " + i + " context: " + context);
    ...
    private VideoCodecStatus initDecodeInternal(int i, int i2) {
        ...
        Logging.m1252d(TAG, "initDecodeInternal name: " + this.codecName + " type: " + this.codecType + " width: " + i + " height: " + i2 + " color format: " + this.colorFormat);
        if (this.outputThread != null) {
            Logging.m1253e(TAG, "initDecodeInternal called while the codec is already running");
        ...
        try {
            ...
            try {
                ...
                Logging.m1252d(TAG, "initDecodeInternal done");
                return VideoCodecStatus.OK;
            } catch (IllegalArgumentException e) {
                e = e;
                Logging.m1254e(TAG, "initDecode failed", e);
                release();
                return VideoCodecStatus.FALLBACK_SOFTWARE;
            ...
            Logging.m1253e(TAG, "Cannot create media decoder " + this.codecName);
            return VideoCodecStatus.FALLBACK_SOFTWARE;
        }
    }
    ...
    public VideoCodecStatus decode(EncodedImage encodedImage, VideoDecoder.DecodeInfo decodeInfo) {
        ...
        if (byteBuffer == null) {
            Logging.m1253e(TAG, "decode() - no input data");
            return VideoCodecStatus.ERR_PARAMETER;
        }
        int iRemaining = byteBuffer.remaining();
        if (iRemaining == 0) {
            Logging.m1253e(TAG, "decode() - input buffer empty");
            return VideoCodecStatus.ERR_PARAMETER;
        }
    ...
    private VideoCodecStatus releaseInternal() {
        ...
        if (!this.running) {
            Logging.m1252d(TAG, "release: Decoder is not running.");
            return VideoCodecStatus.OK;
        }
        try {
            this.running = false;
            if (!ThreadUtils.joinUninterruptibly(this.outputThread, 5000L)) {
                Logging.m1254e(TAG, "Media decoder release timeout", new RuntimeException());
                videoCodecStatus = VideoCodecStatus.TIMEOUT;
            } else if (this.shutdownException != null) {
                Logging.m1254e(TAG, "Media decoder release error", new RuntimeException(this.shutdownException));
    ...
    public class C75671 extends Thread {
        public C75671(String str) {
            super(str);
        }
    ...
    public void deliverDecodedFrame() {
        ...
        try {
            ...
            if (iDequeueOutputBuffer < 0) {
                Logging.m1255v(TAG, "dequeueOutputBuffer returned " + iDequeueOutputBuffer);
                return;
            }
            ...
            Logging.m1254e(TAG, "deliverDecodedFrame failed", e);
        }
    }
    ...
    private void deliverByteFrame(int i, MediaCodec.BufferInfo bufferInfo, int i2, Integer num) {
        ...
        if (i7 < ((i3 * i4) * 3) / 2) {
            Logging.m1253e(TAG, "Insufficient output buffer size: " + bufferInfo.size);
            return;
        }
    ...
    private VideoFrame.Buffer copyI420Buffer(ByteBuffer byteBuffer, int i, int i2, int i3, int i4) {
        if (i % 2 != 0) {
            UseCase$$ExternalSyntheticBUOutline0.m85m("Stride is not divisible by two: ", i);
            return null;
        }
        int i5 = (i3 + 1) / 2;
    ...
    private void reformat(MediaFormat mediaFormat) {
        ...
        int integer2;
        this.outputThreadChecker.checkIsOnValidThread();
        Logging.m1252d(TAG, "Decoder format changed: " + mediaFormat);
            ...
            try {
                if (integer != this.width || integer2 != this.height) {
                    if (!this.hasDecodedFirstFrame) {
                        ...
                        Logging.m1256w(TAG, "Unexpected format dimensions. Configured " + this.width + "*" + this.height + ". New " + integer + "*" + integer2 + ". Skip it");
                        return;
                    }
                ...
                if (this.surfaceTextureHelper == null && mediaFormat.containsKey("color-format")) {
                    ...
                    Logging.m1252d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
                ...
                synchronized (this.dimensionLock) {
                    try {
                        ...
                        Logging.m1252d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
    ...
    public void releaseCodecOnOutputThread() {
        ...
        try {
            ...
            Logging.m1254e(TAG, "Media decoder stop failed", e);
        }
        try {
            this.codec.release();
        } catch (Exception e2) {
            Logging.m1254e(TAG, "Media decoder release failed", e2);
