...
public class GlGenericDrawer implements RendererCommon.GlDrawer {
    ...
    public static String createFragmentShaderString(String str, int i, boolean z) {
    ...
    private void prepareShader(int i, float[] fArr, int i2, int i3, int i4, int i5, int i6, int i7, int i8) {
        ...
        if (glShaderCreateShader == null) {
            try {
                ...
                this.texMatrixLocation[i][i8] = glShaderCreateShader.getUniformLocation(TEXTURE_MATRIX_NAME);
                this.inPosLocation[i][i8] = glShaderCreateShader.getAttribLocation(INPUT_VERTEX_COORDINATE_NAME);
                this.inTcLocation[i][i8] = glShaderCreateShader.getAttribLocation(INPUT_TEXTURE_COORDINATE_NAME);
            } catch (Exception e) {
                FileLog.m1048e(e);
                return;
            }
