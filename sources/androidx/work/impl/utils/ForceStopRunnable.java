            ...
            if (i >= 30) {
                ...
                if (historicalProcessExitReasons != null && !historicalProcessExitReasons.isEmpty()) {
                    ...
                    for (int i2 = 0; i2 < historicalProcessExitReasons.size(); i2++) {
                        ApplicationExitInfo applicationExitInfoM202m = ForceStopRunnable$$ExternalSyntheticApiModelOutline0.m202m(historicalProcessExitReasons.get(i2));
                        if (applicationExitInfoM202m.getReason() == 10 && applicationExitInfoM202m.getTimestamp() >= lastForceStopEventMillis) {
                            return true;
                        }
                    }
                }
        ...
        try {
            ...
            if (z) {
                for (WorkSpec workSpec : runningWork) {
                    workSpecDao.setState(WorkInfo$State.ENQUEUED, workSpec.f76id);
                    workSpecDao.setStopReason(workSpec.f76id, -512);
                    workSpecDao.markWorkSpecScheduled(workSpec.f76id, -1L);
                }
            }
