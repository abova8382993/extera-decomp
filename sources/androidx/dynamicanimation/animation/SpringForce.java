...
public final class SpringForce {
    ...
    public SpringForce setStiffness(float f) {
        if (f <= 0.0f) {
            g$$ExternalSyntheticBUOutline1.m207m("Spring stiffness constant must be positive.");
            return null;
        }
    ...
    public SpringForce setDampingRatio(float f) {
        if (f < 0.0f) {
            g$$ExternalSyntheticBUOutline1.m207m("Damping ratio must be non-negative");
            return null;
        }
    ...
    private void init() {
        ...
        if (this.mFinalPosition == Double.MAX_VALUE) {
            Segment$$ExternalSyntheticBUOutline1.m992m("Error: Final position of the spring must be set before the animation starts");
            return;
        }
