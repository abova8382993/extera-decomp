...
public final class SpringAnimation extends DynamicAnimation<SpringAnimation> {
    ...
    public void skipToEnd() {
        if (!canSkipToEnd()) {
            ByteString$$ExternalSyntheticBUOutline0.m979m("Spring animations can only come to an end when there is damping");
        } else {
    ...
    private void sanityCheck() {
        SpringForce springForce = this.mSpring;
        if (springForce == null) {
            ByteString$$ExternalSyntheticBUOutline0.m979m("Incomplete SpringAnimation: Either final position or a spring force needs to be set.");
            return;
        }
        double finalPosition = springForce.getFinalPosition();
        if (finalPosition > this.mMaxValue) {
            ByteString$$ExternalSyntheticBUOutline0.m979m("Final position of the spring cannot be greater than the max value.");
        } else {
            if (finalPosition >= this.mMinValue) {
                return;
            }
            ByteString$$ExternalSyntheticBUOutline0.m979m("Final position of the spring cannot be less than the min value.");
        }
    }
