...
class CameraStateMachine {
    ...
    public void updateState(CameraInternal.State state, CameraState.StateError stateError) {
        ...
        if (stateError != null && stateError.getCode() == 8) {
        ...
        Logger.d("CameraStateMachine", "New public camera state " + cameraStateOnCameraPendingOpen + " from " + state + " and " + stateError);
        if (Objects.equals((CameraState) this.mCameraStates.getValue(), cameraStateOnCameraPendingOpen)) {
            return;
        }
        Logger.d("CameraStateMachine", "Publishing new public camera state " + cameraStateOnCameraPendingOpen);
