...
public class BackupHdrProfileEncoderProfilesProvider implements EncoderProfilesProvider {
    ...
    private static int deriveCodec(int i) {
        ...
        CharCodeKt$$ExternalSyntheticBUOutline0.m873m("Unexpected HDR format: ", i);
        return 0;
    }
    ...
    private static int scaleBitrate(int i, int i2, int i3) {
        ...
        if (Logger.isDebugEnabled("BackupHdrProfileEncoderProfilesProvider")) {
            Logger.m74d("BackupHdrProfileEncoderProfilesProvider", String.format("Base Bitrate(%dbps) * Bit Depth Ratio (%d / %d) = %d", Integer.valueOf(i), Integer.valueOf(i2), Integer.valueOf(i3), Integer.valueOf(iDoubleValue)));
        }
        return iDoubleValue;
    }
