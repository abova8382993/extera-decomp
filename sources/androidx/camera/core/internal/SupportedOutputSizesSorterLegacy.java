...
class SupportedOutputSizesSorterLegacy {
    ...
    List sortSupportedOutputSizes(List list, UseCaseConfig useCaseConfig) {
        if (list.isEmpty()) {
            return list;
        }
        ...
        if (targetAspectRatioByLegacyApi == null) {
            ...
            Map mapGroupSizesByAspectRatio = SupportedOutputSizesSorter.groupSizesByAspectRatio(arrayList2);
            if (targetSize != null) {
                Iterator it = mapGroupSizesByAspectRatio.keySet().iterator();
                while (it.hasNext()) {
                    SupportedOutputSizesSorter.sortSupportedSizesByFallbackRuleClosestHigherThenLower((List) mapGroupSizesByAspectRatio.get((Rational) it.next()), targetSize, true);
                }
            }
            ...
            while (i < size5) {
                ...
                i++;
                for (Size size6 : (List) mapGroupSizesByAspectRatio.get((Rational) obj2)) {
    ...
    private Rational getTargetAspectRatioByLegacyApi(ImageOutputConfig imageOutputConfig, List list) {
    ...
    private static Rational getAspectRatioGroupKeyOfTargetSize(Size size, List list) {
        if (size == null) {
            return null;
        }
