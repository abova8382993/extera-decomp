...
public class MetadataImageReader implements ImageReaderProxy, ForwardingImageProxy.OnImageCloseListener {
    ...
    /* JADX INFO: renamed from: androidx.camera.core.MetadataImageReader$1 */
    public class C02471 extends CameraCaptureCallback {
        public C02471() {
        }
        ...
        synchronized (this.mLock) {
            try {
                if (this.mMatchedImageProxies.size() < getMaxImages()) {
                    ...
                    Logger.m74d("TAG", "Maximum image number reached.");
        ...
        synchronized (this.mLock) {
            try {
                ...
                if (size >= imageReaderProxy.getMaxImages()) {
                    Logger.m74d("MetadataImageReader", "Skip to acquire the next image because the acquired image count has reached the max images count.");
                    return;
                }
                do {
                    try {
                        ...
                        Logger.m75d("MetadataImageReader", "Failed to acquire next image.", e);
