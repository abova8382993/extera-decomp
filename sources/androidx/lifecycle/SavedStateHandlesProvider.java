...
public final class SavedStateHandlesProvider implements SavedStateRegistry.SavedStateProvider {
    ...
    public Bundle saveState() {
        ...
        for (Map.Entry entry2 : getViewModel().getHandles().entrySet()) {
            ...
            if (!SavedStateReader.m2169isEmptyimpl(SavedStateReader.m2164constructorimpl(bundleSaveState))) {
                SavedStateWriter.m2173putSavedStateimpl(bundleM2171constructorimpl, str, bundleSaveState);
            }
        }
        this.restored = false;
    ...
    public final void performRestore() {
        ...
        if (bundleConsumeRestoredStateForKey != null) {
            SavedStateWriter.m2172putAllimpl(bundleM2171constructorimpl, bundleConsumeRestoredStateForKey);
        }
    ...
    public final Bundle consumeRestoredStateForKey(String key) {
        ...
        performRestore();
        Bundle bundle = this.restoredState;
        if (bundle == null || !SavedStateReader.m2165containsimpl(SavedStateReader.m2164constructorimpl(bundle), key)) {
            return null;
        }
        Bundle bundleM2167getSavedStateOrNullimpl = SavedStateReader.m2167getSavedStateOrNullimpl(SavedStateReader.m2164constructorimpl(bundle), key);
        if (bundleM2167getSavedStateOrNullimpl == null) {
            Map mapEmptyMap = MapsKt.emptyMap();
            if (mapEmptyMap.isEmpty()) {
                pairArr = new Pair[0];
            } else {
            ...
            bundleM2167getSavedStateOrNullimpl = BundleKt.bundleOf((Pair[]) Arrays.copyOf(pairArr, pairArr.length));
            SavedStateWriter.m2171constructorimpl(bundleM2167getSavedStateOrNullimpl);
        }
        ...
        return bundleM2167getSavedStateOrNullimpl;
    }
}
