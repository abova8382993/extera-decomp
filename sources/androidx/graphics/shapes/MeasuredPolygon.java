...
public final class MeasuredPolygon extends AbstractList {
    ...
    public final class MeasuredCubic {
        ...
        public final void updateProgressRange$graphics_shapes_release(float f, float f2) {
            if (f2 < f) {
                throw new IllegalArgumentException("endOutlineProgress is expected to be equal or greater than startOutlineProgress");
            }
            this.startOutlineProgress = f;
        ...
        public final Pair cutAtProgress(float f) {
            ...
            if (0.0f <= fFindCubicCutPoint && fFindCubicCutPoint <= 1.0f) {
                ...
                return TuplesKt.m1081to(new MeasuredCubic(this.this$0, (Cubic) pairSplit.component1(), this.startOutlineProgress, fCoerceIn), new MeasuredCubic(this.this$0, (Cubic) pairSplit.component2(), fCoerceIn, this.endOutlineProgress));
            }
    ...
    public final MeasuredPolygon cutAndShift(float f) {
        ...
        while (i3 < size2) {
            if (i3 == 0) {
                ...
                fPositiveModulo = i3 == this.cubics.size() + 1 ? 1.0f : Utils.positiveModulo(((MeasuredCubic) this.cubics.get(((i + i3) - 1) % this.cubics.size())).getEndOutlineProgress() - f, 1.0f);
            }
    ...
    public static final class Companion {
        ...
        public final MeasuredPolygon measurePolygon$graphics_shapes_release(Measurer measurer, RoundedPolygon polygon) {
            ...
            for (int i = 0; i < size; i++) {
                Feature feature = (Feature) polygon.getFeatures$graphics_shapes_release().get(i);
                int size2 = feature.getCubics().size();
                for (int i2 = 0; i2 < size2; i2++) {
                    if ((feature instanceof Feature.Corner) && i2 == feature.getCubics().size() / 2) {
                        arrayList2.add(TuplesKt.m1081to(feature, Integer.valueOf(arrayList.size())));
                    }
            ...
            for (int i5 = 0; i5 < size5; i5++) {
                ...
                listCreateListBuilder.add(new ProgressableFeature((mutableFloatList.get(iIntValue) + mutableFloatList.get(iIntValue + 1)) / 2, (Feature) ((Pair) arrayList2.get(i5)).getFirst()));
            }
