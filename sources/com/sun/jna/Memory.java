...
public class Memory extends Pointer implements Closeable {
    ...
    public Memory(long j) {
        this.size = j;
        if (j <= 0) {
            g$$ExternalSyntheticBUOutline1.m207m("Allocation size must be greater than zero");
            throw null;
        }
    ...
    public Memory align(int i) {
        if (i <= 0) {
            CharCodeKt$$ExternalSyntheticBUOutline0.m873m("Byte boundary must be positive: ", i);
            return null;
        }
        for (int i2 = 0; i2 < 32; i2++) {
            if (i == (1 << i2)) {
                ...
                if (j5 <= 0) {
                    g$$ExternalSyntheticBUOutline1.m207m("Insufficient memory to align to the requested boundary");
                    return null;
                }
                return (Memory) share(j4 - j3, j5);
            }
        }
        g$$ExternalSyntheticBUOutline1.m207m("Byte boundary must be a power of two");
        return null;
    }
