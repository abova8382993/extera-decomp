...
public final class Code {
    private final List catchLabels;
    private final List catchTypes;
    ...
    public Local newLocal(TypeId typeId) {
        if (this.localsInitialized) {
            throw new IllegalStateException("Cannot allocate locals after adding instructions");
        }
        Local local = Local.get(this, typeId);
        this.locals.add(local);
        return local;
    }

    public Local getParameter(int i, TypeId typeId) {
        if (this.thisLocal != null) {
            i++;
        }
        return coerce((Local) this.parameters.get(i), typeId);
    }

    public Local getThis(TypeId typeId) {
    ...
    private Local coerce(Local local, TypeId typeId) {
    ...
    void initializeLocals() {
        ...
        int iInitialize = 0;
        while (it.hasNext()) {
            iInitialize += ((Local) it.next()).initialize(iInitialize);
        }
        ArrayList arrayList = new ArrayList();
    ...
    private void splitCurrentLabel(Label label, List list) {
    ...
    public void loadConstant(Local local, Object obj) {
        loadConstantInternal(local, obj);
    }
    ...
    public void sget(FieldId fieldId, Local local) {
    ...
    private void invoke(Rop rop, MethodId methodId, Local local, Local local2, Local... localArr) {
    ...
    BasicBlockList toBasicBlocks() {
        ...
        for (int i = 0; i < this.labels.size(); i++) {
            basicBlockList.set(i, ((Label) this.labels.get(i)).toBasicBlock());
        }
