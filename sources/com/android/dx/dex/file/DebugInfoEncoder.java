...
public final class DebugInfoEncoder {
    ...
    private byte[] convert0() {
        ArrayList arrayListBuildSortedPositions = buildSortedPositions();
        ...
        while (true) {
            ...
            int address2 = iEmitPositionsAtAddress < size ? ((PositionList.Entry) arrayListBuildSortedPositions.get(iEmitPositionsAtAddress)).getAddress() : Integer.MAX_VALUE;
            ...
            if (iMin == address2) {
                emitPosition((PositionList.Entry) arrayListBuildSortedPositions.get(iEmitPositionsAtAddress));
    ...
    private int emitPositionsAtAddress(int i, ArrayList arrayList) {
        int size = arrayList.size();
        while (i < size && ((PositionList.Entry) arrayList.get(i)).getAddress() == this.address) {
            emitPosition((PositionList.Entry) arrayList.get(i));
            i++;
        }
        return i;
    }

    private void emitHeader(ArrayList arrayList, ArrayList arrayList2) {
        LocalList.Entry entry;
        String human;
        boolean z = (this.annotateTo == null && this.debugPrint == null) ? false : true;
        int cursor = this.output.getCursor();
        if (arrayList.size() > 0) {
            this.line = ((PositionList.Entry) arrayList.get(0)).getPosition().getLine();
        }
        ...
        if (!this.isStatic) {
            ...
            while (true) {
                ...
                i++;
                LocalList.Entry entry2 = (LocalList.Entry) obj;
                if (paramBase == entry2.getRegister()) {
                    this.lastEntryForReg[paramBase] = entry2;
                    break;
                }
            }
            paramBase++;
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ...
            if (z) {
                ...
                annotate(this.output.getCursor() - cursor3, "parameter " + human + " v" + paramBase);
            }
            paramBase += type.getCategory();
        }
        for (LocalList.Entry entry3 : this.lastEntryForReg) {
            if (entry3 != null && entry3.getSignature() != null) {
                emitLocalStartExtended(entry3);
            }
        }
    }
    ...
    private ArrayList buildSortedPositions() {
        ...
        Collections.sort(arrayList, new Comparator() { // from class: com.android.dx.dex.file.DebugInfoEncoder.1
    ...
    private ArrayList extractMethodArguments() {
        ArrayList arrayList = new ArrayList(this.desc.getParameterTypes().size());
        int paramBase = getParamBase();
        ...
        Collections.sort(arrayList, new Comparator() { // from class: com.android.dx.dex.file.DebugInfoEncoder.2
            @Override // java.util.Comparator
            public boolean equals(Object obj) {
                return obj == this;
            }
