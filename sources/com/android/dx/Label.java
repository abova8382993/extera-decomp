...
public final class Label {
    ...
    List catchLabels = Collections.EMPTY_LIST;
    int id = -1;
    ...
    void compact() {
        for (int i = 0; i < this.catchLabels.size(); i++) {
            while (((Label) this.catchLabels.get(i)).isEmpty()) {
                List list = this.catchLabels;
                list.set(i, ((Label) list.get(i)).primarySuccessor);
            }
        }
        while (true) {
    ...
    BasicBlock toBasicBlock() {
        ...
        for (int i2 = 0; i2 < this.instructions.size(); i2++) {
            insnList.set(i2, (Insn) this.instructions.get(i2));
        }
        insnList.setImmutable();
        IntList intList = new IntList();
        Iterator it = this.catchLabels.iterator();
        while (it.hasNext()) {
            intList.add(((Label) it.next()).id);
        }
        Label label = this.primarySuccessor;
        if (label != null) {
            i = label.id;
