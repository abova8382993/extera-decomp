...
public final class IntList extends MutabilityControl {
    ...
    public int get(int i) {
        if (i >= this.size) {
            IntList$$ExternalSyntheticBUOutline0.m236m("n >= size()");
            return 0;
        }
        try {
            return this.values[i];
        } catch (ArrayIndexOutOfBoundsException unused) {
            IntList$$ExternalSyntheticBUOutline0.m236m("n < 0");
            return 0;
        }
    }
    ...
    public void shrink(int i) {
        if (i < 0) {
            g$$ExternalSyntheticBUOutline1.m207m("newSize < 0");
        } else if (i > this.size) {
            g$$ExternalSyntheticBUOutline1.m207m("newSize > size");
        } else {
            throwIfImmutable();
            this.size = i;
        }
