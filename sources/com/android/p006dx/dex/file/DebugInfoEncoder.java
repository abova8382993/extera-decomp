...
public final class DebugInfoEncoder {
    ...
    private void emitPosition(PositionList.Entry entry) {
        ...
        if (i2 < 0) {
            GlShader$$ExternalSyntheticBUOutline1.m1250m("Position entries must be in ascending address order");
            return;
        }
        int i3 = 0;
        if (i < -4 || i > 10) {
    ...
    private static int computeOpcode(int i, int i2) {
        ...
        GlShader$$ExternalSyntheticBUOutline1.m1250m("Parameter out of range");
        return 0;
    }
    ...
    private void emitUnsignedLeb128(int i) {
        if (i < 0) {
            MathProcessor$$ExternalSyntheticBUOutline0.m1016m("Signed value where unsigned required: ", i);
        } else {
            this.output.writeUleb128(i);
        }
    }
