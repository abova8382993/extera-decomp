...
public final class SwitchList extends MutabilityControl {
    ...
    public void setDefaultTarget(int i) {
        throwIfImmutable();
        if (i < 0) {
            g$$ExternalSyntheticBUOutline1.m207m("target < 0");
        } else if (this.targets.size() != this.size) {
            GlShader$$ExternalSyntheticBUOutline1.m1250m("non-default elements not all set");
        } else {
            this.targets.add(i);
        }
    }
    ...
    public void removeSuperfluousDefaults() {
        ...
        if (i != this.targets.size() - 1) {
            g$$ExternalSyntheticBUOutline1.m207m("incomplete instance");
            return;
        }
