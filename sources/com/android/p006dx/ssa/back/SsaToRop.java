...
public class SsaToRop {
    ...
    private BasicBlockList convertBasicBlocks() {
        ...
        GlShader$$ExternalSyntheticBUOutline1.m1250m("Exit block must have no insns when leaving SSA form");
        return null;
    }

    private void verifyValidExitPredecessor(SsaBasicBlock ssaBasicBlock) {
        Rop opcode = ssaBasicBlock.getInsns().get(r1.size() - 1).getOpcode();
        if (opcode.getBranchingness() == 2 || opcode == Rops.THROW) {
            return;
        }
        GlShader$$ExternalSyntheticBUOutline1.m1250m("Exit predecessor must end in valid exit statement.");
    }
    ...
    private BasicBlock convertBasicBlock(SsaBasicBlock ssaBasicBlock) {
        ...
        if (ropLabelSuccessorList.contains(exitBlock == null ? -1 : exitBlock.getRopLabel())) {
            if (ropLabelSuccessorList.size() > 1) {
                Instance$$ExternalSyntheticBUOutline0.m1010m("Exit predecessor must have no other successors", Hex.m231u2(ssaBasicBlock.getRopLabel()));
                return null;
            }
