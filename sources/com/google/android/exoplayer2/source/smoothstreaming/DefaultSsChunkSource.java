...
public class DefaultSsChunkSource implements SsChunkSource {
    ...
    public void updateManifest(SsManifest ssManifest) {
        ...
        if (i2 == 0 || streamElement2.chunkCount == 0) {
            ...
            int i4 = this.currentManifestChunkOffset;
            if (startTimeUs <= startTimeUs2) {
                this.currentManifestChunkOffset = i4 + i2;
            } else {
                this.currentManifestChunkOffset = i4 + streamElement.getChunkIndex(startTimeUs2);
            }
        }
        this.manifest = ssManifest;
    }
    ...
    public final void getNextChunk(long j, long j2, List<? extends MediaChunk> list, ChunkHolder chunkHolder) {
        ...
        int i = 0;
        while (true) {
            exoTrackSelection = this.trackSelection;
            if (i >= length) {
                break;
            }
            mediaChunkIteratorArr[i] = new StreamElementIterator(streamElement, exoTrackSelection.getIndexInTrackGroup(i), nextChunkIndex);
            i++;
        }
        exoTrackSelection.updateSelectedTrack(j, j4, jResolveTimeToLiveEdgeUs, list2, mediaChunkIteratorArr);
