    ...
    */
    ...
    private void buildAndPrepareSampleStreamWrappers(long j) {
        ...
        for (int i = 0; i < list2.size(); i++) {
            ...
            HlsSampleStreamWrapper hlsSampleStreamWrapperBuildSampleStreamWrapper = buildSampleStreamWrapper(str, 3, new Uri[]{rendition.url}, new Format[]{rendition.format}, null, Collections.EMPTY_LIST, map2, j);
            map = map2;
        ...
        int i2 = 0;
        while (true) {
            int i3 = this.audioVideoSampleStreamWrapperCount;
            hlsSampleStreamWrapperArr = this.sampleStreamWrappers;
            if (i2 >= i3) {
                break;
            }
            hlsSampleStreamWrapperArr[i2].setIsTimestampMaster(true);
            i2++;
        }
    ...
    private static Format deriveVideoFormat(Format format) {
        ...
        return new Format.Builder().setId(format.f353id).setLabel(format.label).setContainerMimeType(format.containerMimeType).setSampleMimeType(MimeTypes.getMediaMimeType(codecsOfType)).setCodecs(codecsOfType).setMetadata(format.metadata).setAverageBitrate(format.averageBitrate).setPeakBitrate(format.peakBitrate).setWidth(format.width).setHeight(format.height).setFrameRate(format.frameRate).setSelectionFlags(format.selectionFlags).setRoleFlags(format.roleFlags).build();
    }
    ...
    private static Format deriveAudioFormat(Format format, Format format2, boolean z) {
        ...
        return new Format.Builder().setId(format.f353id).setLabel(str2).setContainerMimeType(format.containerMimeType).setSampleMimeType(MimeTypes.getMediaMimeType(codecsOfType)).setCodecs(codecsOfType).setMetadata(metadata).setAverageBitrate(z ? format.averageBitrate : -1).setPeakBitrate(z ? format.peakBitrate : -1).setChannelCount(i2).setSelectionFlags(i).setRoleFlags(i3).setLanguage(str).build();
    }
