    ...
    */
    ...
    private boolean canDiscardUpstreamMediaChunksFromIndex(int i) {
        ...
        while (true) {
            ...
            ArrayList<HlsMediaChunk> arrayList = this.mediaChunks;
            if (i2 < size) {
                if (arrayList.get(i2).shouldSpliceIn) {
                    return false;
                }
                i2++;
            } else {
    ...
    private TrackGroupArray createTrackGroupArrayWithDrmInfo(TrackGroup[] trackGroupArr) {
        for (int i = 0; i < trackGroupArr.length; i++) {
            ...
            trackGroupArr[i] = new TrackGroup(trackGroup.f369id, formatArr);
        }
    ...
    private static Format deriveFormat(Format format, Format format2, boolean z) {
        ...
        int codecCountOfType = Util.getCodecCountOfType(format.codecs, trackType);
        String str = format.codecs;
        if (codecCountOfType == 1) {
            codecsCorrespondingToMimeType = Util.getCodecsOfType(str, trackType);
            mediaMimeType = MimeTypes.getMediaMimeType(codecsCorrespondingToMimeType);
        } else {
            codecsCorrespondingToMimeType = MimeTypes.getCodecsCorrespondingToMimeType(str, format2.sampleMimeType);
    ...
    private static DummyTrackOutput createFakeTrackOutput(int i, int i2) {
        Log.m326w("HlsSampleStreamWrapper", "Unmapped track with id " + i + " of type " + i2);
    ...
    public static class EmsgUnwrappingTrackOutput implements TrackOutput {
        ...
        public EmsgUnwrappingTrackOutput(TrackOutput trackOutput, int i) {
            ...
            if (i == 1) {
                ...
                CharCodeKt$$ExternalSyntheticBUOutline0.m873m("Unknown metadataType: ", i);
                throw null;
            }
            ...
            if (!Util.areEqual(this.format.sampleMimeType, this.delegateFormat.sampleMimeType)) {
                if ("application/x-emsg".equals(this.format.sampleMimeType)) {
                    ...
                    if (!emsgContainsExpectedWrappedFormat(eventMessageDecode)) {
                        Log.m326w("HlsSampleStreamWrapper", String.format("Ignoring EMSG. Expected it to contain wrapped %s but actual wrapped format: %s", this.delegateFormat.sampleMimeType, eventMessageDecode.getWrappedMetadataFormat()));
                        return;
                    }
                    sampleAndTrimBuffer = new ParsableByteArray((byte[]) Assertions.checkNotNull(eventMessageDecode.getWrappedMetadataBytes()));
                } else {
                    Log.m326w("HlsSampleStreamWrapper", "Ignoring sample for unsupported format: " + this.format.sampleMimeType);
                    return;
                }
            }
