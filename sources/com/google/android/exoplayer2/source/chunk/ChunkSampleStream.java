...
public class ChunkSampleStream<T extends ChunkSource> implements SampleStream, SequenceableLoader, Loader.Callback<Chunk>, Loader.ReleaseCallback {
    ...
    public void seekToUs(long j) throws Throwable {
        ...
        if (baseMediaChunk != null) {
            zSeekTo = sampleQueue.seekTo(baseMediaChunk.getFirstSampleIndex(0));
        } else {
            zSeekTo = sampleQueue.seekTo(j, j < getNextLoadPositionUs());
        }
        if (zSeekTo) {
    ...
    public Loader.LoadErrorAction onLoadError(Chunk chunk, long j, long j2, IOException iOException, int i) {
        ...
        if (!this.chunkSource.onChunkLoadError(chunk, z, loadErrorInfo, this.loadErrorHandlingPolicy)) {
            ...
            Log.m326w("ChunkSampleStream", "Ignoring attempt to cancel non-cancelable load.");
            loadErrorActionCreateRetryAction = null;
        }
