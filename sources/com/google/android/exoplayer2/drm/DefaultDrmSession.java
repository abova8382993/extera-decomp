...
class DefaultDrmSession implements DrmSession {
    ...
    public void acquire(DrmSessionEventListener.EventDispatcher eventDispatcher) {
        if (this.referenceCount < 0) {
            Log.m322e("DefaultDrmSession", "Session reference count less than zero: " + this.referenceCount);
    ...
    public void release(DrmSessionEventListener.EventDispatcher eventDispatcher) {
        ...
        if (i <= 0) {
            Log.m322e("DefaultDrmSession", "release() called on a session that's already fully released.");
            return;
        }
        int i2 = i - 1;
    ...
    private void doLicense(boolean z) {
        ...
        if (this.state == 4 || restoreKeys()) {
            ...
            if (this.mode == 0 && licenseDurationRemainingSec <= 60) {
                Log.m321d("DefaultDrmSession", "Offline license has expired or will expire soon. Remaining seconds: " + licenseDurationRemainingSec);
                postKeyRequest(bArr, 2, z);
                return;
            }
    ...
    private long getLicenseDurationRemainingSec() {
        if (!AbstractC1260C.WIDEVINE_UUID.equals(this.uuid)) {
    ...
    private void onError(final Exception exc, int i) {
        ...
        Log.m323e("DefaultDrmSession", "DRM session error", exc);
    ...
    public class RequestHandler extends Handler {
        ...
        public void handleMessage(Message message) {
            ...
            try {
                ...
                Log.m327w("DefaultDrmSession", "Key/provisioning request produced an unexpected exception. Not retrying.", e2);
