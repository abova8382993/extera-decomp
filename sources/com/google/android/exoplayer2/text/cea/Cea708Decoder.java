...
public final class Cea708Decoder extends CeaDecoder {
            ...
            if (i == 2 || i == 3) {
                if (z) {
                    if (i == 3) {
                        ...
                        if (i3 != -1 && i2 != (i3 + 1) % 4) {
                            resetCueBuilders();
                            Log.m326w("Cea708Decoder", "Sequence number discontinuity. previous=" + this.previousSequenceNumber + " current=" + i2);
                        }
                        ...
                        if (dtvCcPacket2 == null) {
                            Log.m322e("Cea708Decoder", "Encountered DTVCC_PACKET_DATA before DTVCC_PACKET_START");
                        } else {
    ...
    private void processCurrentPacket() {
        ...
        if (dtvCcPacket.currentIndex != (dtvCcPacket.packetSize * 2) - 1) {
            Log.m321d("Cea708Decoder", "DtvCcPacket ended prematurely; size is " + ((this.currentDtvCcPacket.packetSize * 2) - 1) + ", but current index is " + this.currentDtvCcPacket.currentIndex + " (sequence number " + this.currentDtvCcPacket.sequenceNumber + ");");
        }
        ...
        while (true) {
            ...
            if (bits == 7) {
                ...
                if (bits < 7) {
                    Log.m326w("Cea708Decoder", "Invalid extended service number: " + bits);
                }
            }
            if (bits2 != 0) {
                int i = this.selectedServiceNumber;
                ParsableBitArray parsableBitArray2 = this.captionChannelPacketData;
                if (bits != i) {
                    parsableBitArray2.skipBytes(bits2);
                } else {
                    int position = parsableBitArray2.getPosition() + (bits2 * 8);
                        ...
                        if (bits3 == 16) {
                            ...
                            if (bits4 <= 31) {
                                ...
                                if (bits4 <= 127) {
                                    ...
                                    Log.m326w("Cea708Decoder", "Invalid extended command: " + bits4);
                                }
                                z = true;
                            }
                        } else if (bits3 <= 31) {
                            ...
                            if (bits3 <= 127) {
                                ...
                                Log.m326w("Cea708Decoder", "Invalid base command: " + bits3);
                            }
                            z = true;
                        }
                    }
                ...
                Log.m326w("Cea708Decoder", "serviceNumber is non-zero (" + bits + ") when blockSize is 0");
            }
        }
        if (z) {
    ...
    private void handleC0Command(int i) {
        if (i != 0) {
            ...
            switch (i) {
                ...
                default:
                    if (i >= 17 && i <= 23) {
                        Log.m326w("Cea708Decoder", "Currently unsupported COMMAND_EXT1 Command: " + i);
                        this.captionChannelPacketData.skipBits(8);
                    } else if (i >= 24 && i <= 31) {
                        Log.m326w("Cea708Decoder", "Currently unsupported COMMAND_P16 Command: " + i);
                        this.captionChannelPacketData.skipBits(16);
                    } else {
                        Log.m326w("Cea708Decoder", "Invalid C0 command: " + i);
                    }
                    break;
            }
        }
    ...
    private void handleC1Command(int i) {
        ...
        switch (i) {
            ...
            case 150:
            default:
                Log.m326w("Cea708Decoder", "Invalid C1 command: " + i);
                break;
            case 151:
                if (!this.currentCueInfoBuilder.isDefined()) {
                ...
                default:
                    switch (i) {
                        ...
                        case 127:
                            ...
                            break;
                        default:
                            Log.m326w("Cea708Decoder", "Invalid G2 character: " + i);
                            break;
                    }
                    break;
            }
    ...
    private void handleG3Character(int i) {
        ...
        Log.m326w("Cea708Decoder", "Invalid G3 character: " + i);
    ...
    public static final class CueInfoBuilder {
        ...
        public Cea708CueInfo build() {
            ...
            if (i2 == 0) {
                ...
                if (i2 != 3) {
                    IntegerType$$ExternalSyntheticBUOutline0.m547m("Unexpected justification value: ", this.justification);
                    return null;
                }
                alignment = Layout.Alignment.ALIGN_NORMAL;
            } else {
            ...
            boolean z2 = this.relativePositioning;
            int i4 = this.horizontalAnchor;
            if (z2) {
                f = i4 / 99.0f;
            ...
            return new Cea708CueInfo(spannableStringBuilder, alignment, f4, 0, i3, f3, r7, -3.4028235E38f, z, this.windowFillColor, this.priority);
        }
