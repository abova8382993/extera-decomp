...
public final class OpusDecoder extends SimpleDecoder<DecoderInputBuffer, SimpleDecoderOutputBuffer, OpusDecoderException> {
    ...
    public OpusDecoder(int i, int i2, int i3, List<byte[]> list, CryptoConfig cryptoConfig, boolean z) throws OpusDecoderException {
        ...
        if (bArr[18] == 0) {
            ...
            int i9 = bArr[20] & UByte.MAX_VALUE;
            System.arraycopy(bArr, 21, bArr2, 0, channelCount);
            i4 = channelCount;
            i5 = i8;
            i6 = i9;
        }
        long jOpusInit = opusInit(SAMPLE_RATE, i4, i5, i6, signedLittleEndian16, bArr2);
    ...
    public OpusDecoderException decode(DecoderInputBuffer decoderInputBuffer, SimpleDecoderOutputBuffer simpleDecoderOutputBuffer, boolean z) {
        ...
        boolean zIsEncrypted = decoderInputBuffer.isEncrypted();
        long j = this.nativeDecoderContext;
        if (zIsEncrypted) {
            iOpusDecode = opusSecureDecode(j, decoderInputBuffer.timeUs, byteBuffer, byteBuffer.limit(), simpleDecoderOutputBuffer, SAMPLE_RATE, this.cryptoConfig, cryptoInfo.mode, (byte[]) Assertions.checkNotNull(cryptoInfo.key), (byte[]) Assertions.checkNotNull(cryptoInfo.f360iv), cryptoInfo.numSubSamples, cryptoInfo.numBytesOfClearData, cryptoInfo.numBytesOfEncryptedData);
            simpleDecoderOutputBuffer2 = simpleDecoderOutputBuffer;
            ...
            decoderInputBuffer2 = decoderInputBuffer;
            opusDecoder = this;
            iOpusDecode = opusDecoder.opusDecode(j, decoderInputBuffer.timeUs, byteBuffer, byteBuffer.limit(), simpleDecoderOutputBuffer2);
        }
        if (iOpusDecode < 0) {
            if (iOpusDecode == -2) {
        ...
        ByteBuffer byteBuffer2 = (ByteBuffer) Util.castNonNull(simpleDecoderOutputBuffer2.data);
        ...
        if (!opusDecoder.experimentalDiscardPaddingEnabled || !decoderInputBuffer2.hasSupplementalData() || (discardPaddingSamples = getDiscardPaddingSamples(decoderInputBuffer2.supplementalData)) <= 0 || iOpusDecode < (iSamplesToBytes = samplesToBytes(discardPaddingSamples, opusDecoder.channelCount, opusDecoder.outputFloat))) {
            return null;
        }
