...
final class VideoTagPayloadReader extends TagPayloadReader {
    ...
    public boolean parsePayload(ParsableByteArray parsableByteArray, long j) throws ParserException {
        ...
        if (unsignedByte == 0 && !this.hasOutputFormat) {
            ...
            this.output.format(new Format.Builder().setSampleMimeType(MediaController.VIDEO_MIME_TYPE).setCodecs(avcConfig.codecs).setWidth(avcConfig.width).setHeight(avcConfig.height).setPixelWidthHeightRatio(avcConfig.pixelWidthHeightRatio).setInitializationData(avcConfig.initializationData).build());
