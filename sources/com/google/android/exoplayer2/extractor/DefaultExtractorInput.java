...
public final class DefaultExtractorInput implements ExtractorInput {
    ...
    public int read(byte[] bArr, int i, int i2) throws EOFException, InterruptedIOException {
        ...
        if (fromPeekBuffer == 0) {
            ...
            fromPeekBuffer = defaultExtractorInput.readFromUpstream(bArr, i, i2, 0, true);
        } else {
            defaultExtractorInput = this;
        }
        defaultExtractorInput.commitBytesRead(fromPeekBuffer);
    ...
    public int skip(int i) throws EOFException, InterruptedIOException {
        ...
        if (iSkipFromPeekBuffer == 0) {
            ...
            defaultExtractorInput = this;
            iSkipFromPeekBuffer = defaultExtractorInput.readFromUpstream(bArr, 0, Math.min(i, bArr.length), 0, true);
        } else {
            defaultExtractorInput = this;
        }
        defaultExtractorInput.commitBytesRead(iSkipFromPeekBuffer);
    ...
    public boolean advancePeekPosition(int i, boolean z) throws EOFException, InterruptedIOException {
        ...
        while (fromUpstream < i) {
            ...
            int i2 = i;
            boolean z2 = z;
            fromUpstream = defaultExtractorInput.readFromUpstream(this.peekBuffer, this.peekBufferPosition, i2, fromUpstream, z2);
            if (fromUpstream == -1) {
                return false;
            }
            defaultExtractorInput.peekBufferLength = defaultExtractorInput.peekBufferPosition + fromUpstream;
