...
public final class AdtsReader implements ElementaryStreamReader {
    ...
    private boolean isAdtsSyncBytes(byte b2, byte b3) {
        return isAdtsSyncWord(((b2 & UByte.MAX_VALUE) << 8) | (b3 & UByte.MAX_VALUE));
    }
    ...
    private void parseAdtsHeader() {
        ...
        if (!z) {
            int i = 2;
            int bits = parsableBitArray.readBits(2) + 1;
            if (bits != 2) {
                Log.m326w("AdtsReader", "Detected audio object type: " + bits + ", but assuming AAC LC.");
            } else {
                i = bits;
            }
            ...
            Format formatBuild = new Format.Builder().setId(this.formatId).setSampleMimeType(MediaController.AUDIO_MIME_TYPE).setCodecs(audioSpecificConfig.codecs).setChannelCount(audioSpecificConfig.channelCount).setSampleRate(audioSpecificConfig.sampleRateHz).setInitializationData(Collections.singletonList(bArrBuildAudioSpecificConfig)).setLanguage(this.language).build();
