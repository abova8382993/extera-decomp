        ...
        while (parsableByteArray.bytesLeft() > 0) {
            ...
            if (i != 0) {
                if (i == 1) {
                    ...
                    if ((unsignedByte & Opcodes.SHL_INT_LIT8) == 224) {
    ...
    private void parseStreamMuxConfig(ParsableBitArray parsableBitArray) throws ParserException {
        ...
        if (bits2 == 0) {
            ...
            if (bits == 0) {
                ...
                Format formatBuild = new Format.Builder().setId(this.formatId).setSampleMimeType(MediaController.AUDIO_MIME_TYPE).setCodecs(this.codecs).setChannelCount(this.channelCount).setSampleRate(this.sampleRateHz).setInitializationData(Collections.singletonList(bArr)).setLanguage(this.language).build();
