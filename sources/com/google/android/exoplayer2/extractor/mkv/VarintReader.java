...
final class VarintReader {
    ...
    public long readUnsignedVarint(ExtractorInput extractorInput, boolean z, boolean z2, int i) {
        if (this.state == 0) {
            ...
            int unsignedVarintLength = parseUnsignedVarintLength(this.scratch[0] & UByte.MAX_VALUE);
            this.length = unsignedVarintLength;
            if (unsignedVarintLength == -1) {
                Segment$$ExternalSyntheticBUOutline1.m992m("No valid varint length mask found");
                return 0L;
            }
            this.state = 1;
        }
