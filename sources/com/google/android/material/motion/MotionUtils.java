...
public class MotionUtils {
    ...
    public static SpringForce resolveThemeSpringForce(Context context, int i, int i2) {
        ...
        if (typedValueResolve == null) {
            typedArrayObtainStyledAttributes = context.obtainStyledAttributes(null, C1379R.styleable.MaterialSpring, 0, i2);
        } else {
            typedArrayObtainStyledAttributes = context.obtainStyledAttributes(typedValueResolve.resourceId, C1379R.styleable.MaterialSpring);
        }
        SpringForce springForce = new SpringForce();
        try {
            float f = typedArrayObtainStyledAttributes.getFloat(C1379R.styleable.MaterialSpring_stiffness, Float.MIN_VALUE);
            if (f == Float.MIN_VALUE) {
                throw new IllegalArgumentException("A MaterialSpring style must have stiffness value.");
            }
            float f2 = typedArrayObtainStyledAttributes.getFloat(C1379R.styleable.MaterialSpring_damping, Float.MIN_VALUE);
    ...
    public static TimeInterpolator resolveThemeInterpolator(Context context, int i, TimeInterpolator timeInterpolator) {
        ...
        if (typedValue.type != 3) {
            g$$ExternalSyntheticBUOutline1.m207m("Motion easing theme attribute must be an @interpolator resource for ?attr/motionEasing*Interpolator attributes or a string for ?attr/motionEasing* attributes.");
            return null;
        }
    ...
    private static TimeInterpolator getLegacyThemeInterpolator(String str) {
        if (isLegacyEasingType(str, EASING_TYPE_CUBIC_BEZIER)) {
            ...
            if (strArrSplit.length != 4) {
                IntegerType$$ExternalSyntheticBUOutline0.m547m("Motion easing theme attribute must have 4 control points if using bezier curve format; instead got: ", strArrSplit.length);
                return null;
            }
        ...
        Native$$ExternalSyntheticBUOutline5.m554m("Invalid motion easing type: ", str);
        return null;
    }
