...
final class LinearDrawingDelegate extends DrawingDelegate<LinearProgressIndicatorSpec> {
    ...
    public int getPreferredHeight() {
        S s = this.spec;
        return ((LinearProgressIndicatorSpec) s).trackThickness + (((LinearProgressIndicatorSpec) s).waveAmplitude * 2);
    }
    ...
    private void drawLine(Canvas canvas, Paint paint, float f, float f2, int i, int i2, int i3, float f3, float f4, boolean z) {
        ...
        if (i4 <= i5) {
            ...
            if (!z2) {
                ...
                float f23 = ((DrawingDelegate.PathPoint) obj).posVec[0];
                float f24 = ((DrawingDelegate.PathPoint) obj).posVec[1];
                Object obj2 = pair3.second;
                canvas.drawLine(f23, f24, ((DrawingDelegate.PathPoint) obj2).posVec[0], ((DrawingDelegate.PathPoint) obj2).posVec[1], paint);
                paint2 = paint;
                canvas2 = canvas;
            } else {
                paint2 = paint;
                ...
                calculateDisplayedPath(pathMeasure, path, pair4, f15 / f25, f16 / f25, f3, f4);
                canvas2 = canvas;
    ...
    public void drawStopIndicator(Canvas canvas, Paint paint, int i, int i2) {
        ...
        if (((LinearProgressIndicatorSpec) s).trackStopIndicatorPadding != null) {
            fFloatValue = ((LinearProgressIndicatorSpec) s).trackStopIndicatorPadding.floatValue() + (((LinearProgressIndicatorSpec) this.spec).trackStopIndicatorSize / 2.0f);
        } else {
            fFloatValue = this.displayedTrackThickness / 2.0f;
        }
        float f = actualTrackStopIndicatorSize;
        drawRoundedBlock(canvas, paint, new DrawingDelegate.PathPoint(new float[]{(this.trackLength / 2.0f) - fFloatValue, 0.0f}, new float[]{1.0f, 0.0f}), f, f, (this.displayedCornerRadius * f) / this.displayedTrackThickness);
    }
