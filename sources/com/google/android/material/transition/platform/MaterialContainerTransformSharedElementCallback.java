...
public class MaterialContainerTransformSharedElementCallback extends SharedElementCallback {
    ...
    public View onCreateSnapshotView(Context context, Parcelable parcelable) {
        ...
        if (viewOnCreateSnapshotView != null && (weakReference = capturedSharedElement) != null && this.shapeProvider != null && (view = weakReference.get()) != null && (shapeAppearanceModelProvideShape = this.shapeProvider.provideShape(view)) != null) {
            viewOnCreateSnapshotView.setTag(C1379R.id.mtrl_motion_snapshot_view, shapeAppearanceModelProvideShape);
        }
    ...
    public void onSharedElementStart(List<String> list, List<View> list2, List<View> list3) {
        if (!list2.isEmpty() && !list3.isEmpty()) {
            list2.get(0).setTag(C1379R.id.mtrl_motion_snapshot_view, list3.get(0));
        }
        if (this.entering || list2.isEmpty() || this.returnEndBounds == null) {
            return;
        }
        View view = list2.get(0);
        view.measure(View.MeasureSpec.makeMeasureSpec(this.returnEndBounds.width(), TLObject.FLAG_30), View.MeasureSpec.makeMeasureSpec(this.returnEndBounds.height(), TLObject.FLAG_30));
    ...
    public void onSharedElementEnd(List<String> list, List<View> list2, List<View> list3) {
        if (!list2.isEmpty() && (list2.get(0).getTag(C1379R.id.mtrl_motion_snapshot_view) instanceof View)) {
            list2.get(0).setTag(C1379R.id.mtrl_motion_snapshot_view, null);
        }
