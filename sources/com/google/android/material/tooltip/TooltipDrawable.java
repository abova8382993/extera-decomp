...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    private static final int DEFAULT_STYLE = C1379R.style.Widget_MaterialComponents_Tooltip;
    private static final int DEFAULT_THEME_ATTR = C1379R.attr.tooltipStyle;
    ...
    public class ViewOnLayoutChangeListenerC16741 implements View.OnLayoutChangeListener {
        public ViewOnLayoutChangeListenerC16741() {
        }
    ...
    private void loadFromAttributes(AttributeSet attributeSet, int i, int i2) {
        TypedArray typedArrayObtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(this.context, attributeSet, C1379R.styleable.Tooltip, i, i2, new int[0]);
        this.arrowSize = this.context.getResources().getDimensionPixelSize(C1379R.dimen.mtrl_tooltip_arrowSize);
        boolean z = typedArrayObtainStyledAttributes.getBoolean(C1379R.styleable.Tooltip_showMarker, true);
        this.showMarker = z;
        if (z) {
        ...
        setText(typedArrayObtainStyledAttributes.getText(C1379R.styleable.Tooltip_android_text));
        TextAppearance textAppearance = MaterialResources.getTextAppearance(this.context, typedArrayObtainStyledAttributes, C1379R.styleable.Tooltip_android_textAppearance);
        if (textAppearance != null && typedArrayObtainStyledAttributes.hasValue(C1379R.styleable.Tooltip_android_textColor)) {
            textAppearance.setTextColor(MaterialResources.getColorStateList(this.context, typedArrayObtainStyledAttributes, C1379R.styleable.Tooltip_android_textColor));
        }
        setTextAppearance(textAppearance);
        setFillColor(ColorStateList.valueOf(typedArrayObtainStyledAttributes.getColor(C1379R.styleable.Tooltip_backgroundTint, MaterialColors.layer(ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, R.attr.colorBackground, TooltipDrawable.class.getCanonicalName()), 229), ColorUtils.setAlphaComponent(MaterialColors.getColor(this.context, C1379R.attr.colorOnBackground, TooltipDrawable.class.getCanonicalName()), 153)))));
        setStrokeColor(ColorStateList.valueOf(MaterialColors.getColor(this.context, C1379R.attr.colorSurface, TooltipDrawable.class.getCanonicalName())));
        this.padding = typedArrayObtainStyledAttributes.getDimensionPixelSize(C1379R.styleable.Tooltip_android_padding, 0);
        this.minWidth = typedArrayObtainStyledAttributes.getDimensionPixelSize(C1379R.styleable.Tooltip_android_minWidth, 0);
        this.minHeight = typedArrayObtainStyledAttributes.getDimensionPixelSize(C1379R.styleable.Tooltip_android_minHeight, 0);
        this.layoutMargin = typedArrayObtainStyledAttributes.getDimensionPixelSize(C1379R.styleable.Tooltip_android_layout_margin, 0);
    ...
    private EdgeTreatment createMarkerEdge() {
        ...
        float fWidth = (float) ((((double) getBounds().width()) - (((double) this.arrowSize) * Math.sqrt(2.0d))) / 2.0d);
