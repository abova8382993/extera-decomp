package com.google.android.gms.internal.mlkit_vision_subject_segmentation;

/* JADX INFO: loaded from: classes4.dex */
public final class zzsc {
    private final String zza;
    private final String zzb;
    private final String zzc;
    private final String zzd;
    private final zzav zze;
    private final String zzf;
    private final Boolean zzg;
    private final Boolean zzh;
    private final Boolean zzi;
    private final Integer zzj;
    private final Integer zzk;

    /* synthetic */ zzsc(zzsa zzsaVar, zzsb zzsbVar) {
        this.zza = zzsaVar.zza;
        this.zzb = zzsaVar.zzb;
        this.zzc = zzsaVar.zzc;
        this.zzd = zzsaVar.zzd;
        this.zze = zzsaVar.zze;
        this.zzf = zzsaVar.zzf;
        this.zzg = zzsaVar.zzg;
        this.zzh = zzsaVar.zzh;
        this.zzi = zzsaVar.zzi;
        this.zzj = zzsaVar.zzj;
        this.zzk = zzsaVar.zzk;
    }

    public final zzav zza() {
        return this.zze;
    }

    public final Boolean zzb() {
        return this.zzg;
    }

    public final Boolean zzc() {
        return this.zzi;
    }

    public final Boolean zzd() {
        return this.zzh;
    }

    public final Integer zze() {
        return this.zzj;
    }

    public final Integer zzf() {
        return this.zzk;
    }

    public final String zzg() {
        return this.zza;
    }

    public final String zzh() {
        return this.zzb;
    }

    public final String zzi() {
        return this.zzf;
    }

    public final String zzj() {
        return this.zzc;
    }

    public final String zzk() {
        return this.zzd;
    }
}
