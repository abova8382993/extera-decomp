...
final class ReportQueue {
        ...
        synchronized (this.queue) {
            try {
                ...
                if (z) {
                    ...
                    if (isQueueAvailable()) {
                        Logger.getLogger().m529d("Enqueueing report: " + crashlyticsReportWithSessionId.getSessionId());
                        Logger.getLogger().m529d("Queue size: " + this.queue.size());
                        this.singleThreadExecutor.execute(new ReportRunnable(crashlyticsReportWithSessionId, taskCompletionSource));
                        Logger.getLogger().m529d("Closing task for report: " + crashlyticsReportWithSessionId.getSessionId());
                        taskCompletionSource.trySetResult(crashlyticsReportWithSessionId);
                        return taskCompletionSource;
                    }
                    calcStep();
                    Logger.getLogger().m529d("Dropping report due to queue being full: " + crashlyticsReportWithSessionId.getSessionId());
    ...
    public void flushScheduledReportsIfAble() {
        ...
        new Thread(new Runnable() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda1
            ...
            public final void run() {
                ReportQueue.$r8$lambda$8U76pxOta423qVuKYJ0HhfFLxJ4(this.f$0, countDownLatch);
            }
        }).start();
    ...
    public void sendReport(final CrashlyticsReportWithSessionId crashlyticsReportWithSessionId, final TaskCompletionSource<CrashlyticsReportWithSessionId> taskCompletionSource) {
        Logger.getLogger().m529d("Sending report through Google DataTransport: " + crashlyticsReportWithSessionId.getSessionId());
        ...
        this.transport.schedule(Event.ofUrgent(crashlyticsReportWithSessionId.getReport()), new TransportScheduleCallback() { // from class: com.google.firebase.crashlytics.internal.send.ReportQueue$$ExternalSyntheticLambda0
            ...
            public final void onSchedule(Exception exc) {
                ReportQueue.$r8$lambda$4lFm4kq1MZXZfcwIBR3zscs57No(this.f$0, taskCompletionSource, z, crashlyticsReportWithSessionId, exc);
            }
        });
    }
    ...
    private int calcStep() {
        ...
        int i = this.step;
        if (zIsQueueFull) {
            iMax = Math.min(100, i + iNow);
        } else {
            iMax = Math.max(0, i - iNow);
        }
        if (this.step != iMax) {
            this.step = iMax;
            this.lastUpdatedMs = now();
    ...
    public final class ReportRunnable implements Runnable {
        ...
        public void run() {
            ...
            Logger.getLogger().m529d("Delay for: " + String.format(Locale.US, "%.2f", Double.valueOf(dCalcDelay / 1000.0d)) + " s for report: " + this.reportWithSessionId.getSessionId());
