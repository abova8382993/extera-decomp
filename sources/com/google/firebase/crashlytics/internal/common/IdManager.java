...
public class IdManager implements InstallIdProvider {
    ...
    public IdManager(Context context, String str, FirebaseInstallationsApi firebaseInstallationsApi, DataCollectionArbiter dataCollectionArbiter) {
        if (context == null) {
            g$$ExternalSyntheticBUOutline1.m207m("appContext must not be null");
            throw null;
        }
        if (str == null) {
            g$$ExternalSyntheticBUOutline1.m207m("appIdentifier must not be null");
            throw null;
        }
    ...
    private static String formatId(String str) {
        return ID_PATTERN.matcher(str).replaceAll(_UrlKt.FRAGMENT_ENCODE_SET).toLowerCase(Locale.US);
    }
    ...
    public synchronized InstallIdProvider.InstallIds getInstallIds() {
        ...
        Logger.getLogger().m535v("Determining Crashlytics installation ID...");
        SharedPreferences sharedPrefs = CommonUtils.getSharedPrefs(this.appContext);
        String string = sharedPrefs.getString("firebase.installation.id", null);
        Logger.getLogger().m535v("Cached Firebase Installation ID: " + string);
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            FirebaseInstallationId firebaseInstallationIdFetchTrueFid = fetchTrueFid(false);
            Logger.getLogger().m535v("Fetched Firebase Installation ID: " + firebaseInstallationIdFetchTrueFid.getFid());
        ...
        Logger.getLogger().m535v("Install IDs: " + this.installIds);
        return this.installIds;
    }
    ...
    public FirebaseInstallationId fetchTrueFid(boolean z) {
        ...
        if (z) {
            try {
                token = ((InstallationTokenResult) Tasks.await(this.firebaseInstallations.getToken(false), 10000L, timeUnit)).getToken();
            } catch (Exception e) {
                Logger.getLogger().m538w("Error getting Firebase authentication token.", e);
                token = null;
            }
        } else {
            token = null;
        ...
        try {
            str = (String) Tasks.await(this.firebaseInstallations.getId(), 10000L, timeUnit);
        } catch (Exception e2) {
            Logger.getLogger().m538w("Error getting Firebase installation id.", e2);
        }
    ...
    private synchronized String createAndCacheCrashlyticsInstallId(String str, SharedPreferences sharedPreferences) {
        ...
        Logger.getLogger().m535v("Created new Crashlytics installation ID: " + id + " for FID: " + str);
    ...
    private String removeForwardSlashesIn(String str) {
        return str.replaceAll(FORWARD_SLASH_REGEX, _UrlKt.FRAGMENT_ENCODE_SET);
    }
