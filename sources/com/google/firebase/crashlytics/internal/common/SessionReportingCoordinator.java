...
public class SessionReportingCoordinator {
    ...
    public void persistFatalEvent(Throwable th, Thread thread, String str, long j) {
        Logger.getLogger().m535v("Persisting fatal event for session " + str);
    ...
    public void persistNonFatalEvent(Throwable th, Thread thread, EventMetadata eventMetadata) {
        Logger.getLogger().m535v("Persisting non-fatal event for session " + eventMetadata.getSessionId());
    ...
    public void persistRelevantAppExitInfoEvent(String str, List<ApplicationExitInfo> list, LogFileManager logFileManager, UserMetadata userMetadata) {
        ...
        if (applicationExitInfoFindRelevantApplicationExitInfo == null) {
            Logger.getLogger().m535v("No relevant ApplicationExitInfo occurred during session: " + str);
            return;
        }
        ...
        Logger.getLogger().m529d("Persisting anr for session " + str);
    ...
    public void finalizeSessionWithNativeEvent(String str, List<NativeSessionFile> list, CrashlyticsReport.ApplicationExitInfo applicationExitInfo) {
        Logger.getLogger().m529d("SessionReportingCoordinator#finalizeSessionWithNativeEvent");
    ...
    private CrashlyticsReport.Session.Event addLogsCustomKeysAndEventKeysToEvent(CrashlyticsReport.Session.Event event, LogFileManager logFileManager, UserMetadata userMetadata, Map<String, String> map) {
        ...
        if (logString != null) {
            ...
            Logger.getLogger().m535v("No log data to include with this event.");
        }
    ...
    public static /* synthetic */ void $r8$lambda$DUwnwpDR4ha4kVUqZB7wdkmMrec(SessionReportingCoordinator sessionReportingCoordinator, CrashlyticsReport.Session.Event event, EventMetadata eventMetadata, boolean z) {
        ...
        Logger.getLogger().m529d("disk worker: log non-fatal event to persistence");
    ...
    public boolean onReportSendComplete(Task<CrashlyticsReportWithSessionId> task) {
        if (task.isSuccessful()) {
            ...
            Logger.getLogger().m529d("Crashlytics report successfully enqueued to DataTransport: " + result.getSessionId());
            File reportFile = result.getReportFile();
            if (reportFile.delete()) {
                Logger.getLogger().m529d("Deleted report file: " + reportFile.getPath());
                return true;
            }
            Logger.getLogger().m537w("Crashlytics could not delete report file: " + reportFile.getPath());
            return true;
        }
        Logger.getLogger().m538w("Crashlytics report could not be enqueued to DataTransport", task.getException());
        return false;
    }
    ...
    private static CrashlyticsReport.ApplicationExitInfo convertApplicationExitInfo(ApplicationExitInfo applicationExitInfo) {
        ...
        try {
            ...
            Logger.getLogger().m537w("Could not get input trace in application exit info: " + applicationExitInfo.toString() + " Error: " + e);
        }
    ...
    private ApplicationExitInfo findRelevantApplicationExitInfo(String str, List<ApplicationExitInfo> list) {
        ...
        while (it.hasNext()) {
            ApplicationExitInfo applicationExitInfoM199m = ForceStopRunnable$$ExternalSyntheticApiModelOutline0.m199m(it.next());
            if (applicationExitInfoM199m.getTimestamp() < startTimestampMillis) {
                return null;
            }
            if (applicationExitInfoM199m.getReason() == 6) {
                return applicationExitInfoM199m;
            }
        }
        return null;
    }
