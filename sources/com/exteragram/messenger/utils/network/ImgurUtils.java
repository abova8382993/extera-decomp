...
public abstract class ImgurUtils {
    ...
    public static ImgurResponse uploadImage(File file) {
        ...
        try {
            ...
            if (!responseExecute.getIsSuccessful()) {
                FileLog.m1046e("Unexpected code " + responseExecute);
                return null;
            }
            try {
    ...
    public static boolean deleteImage(String str) {
        try {
            return ExteraHttpClient.INSTANCE.getClient().newCall(new Request.Builder().url("https://api.imgur.com/3/image/" + str).header("Authorization", "Client-ID " + CLIENT_ID).delete().build()).execute().getIsSuccessful();
        } catch (IOException e) {
            FileLog.m1048e(e);
            return false;
        }
