...
public class DoubleTapCell extends LinearLayout implements CustomPreferenceCell {
    private static final int[] ICON_WIDTH = {AndroidUtilities.m1036dp(12.0f), AndroidUtilities.m1036dp(12.0f)};
    ...
    public DoubleTapCell(final Context context) {
        ...
        setPadding(AndroidUtilities.m1036dp(13.0f), 0, AndroidUtilities.m1036dp(13.0f), AndroidUtilities.m1036dp(10.0f));
        paint.setStyle(Paint.Style.STROKE);
        paint.setStrokeWidth(AndroidUtilities.m1036dp(1.0f) / 2.0f);
        ...
        FrameLayout frameLayout = new FrameLayout(context) { // from class: com.exteragram.messenger.preferences.chats.components.DoubleTapCell.1
            ...
            public void onDraw(Canvas canvas) {
                ...
                while (i < 2) {
                    if (i == 0) {
                        DoubleTapCell.this.rect.set(AndroidUtilities.m1036dp(8.0f) + strokeWidth, AndroidUtilities.m1036dp(10.0f) + strokeWidth, ((getMeasuredWidth() / 2.0f) - AndroidUtilities.m1036dp(8.0f)) - strokeWidth, AndroidUtilities.m1036dp(75.0f) - strokeWidth);
                    } else {
                        canvas.translate(0.0f, AndroidUtilities.m1036dp(80.0f));
                        DoubleTapCell.this.rect.set((getMeasuredWidth() / 2.0f) + strokeWidth + AndroidUtilities.m1036dp(8.0f), AndroidUtilities.m1036dp(5.0f) + strokeWidth, (getMeasuredWidth() - AndroidUtilities.m1036dp(8.0f)) - strokeWidth, AndroidUtilities.m1036dp(70.0f) - strokeWidth);
                    }
                    ...
                    while (true) {
                        ...
                        DoubleTapCell.this.circleOutlinePaint[i2].setStrokeWidth(AndroidUtilities.m1036dp(1.5f) * DoubleTapCell.this.circleProgress[i3] * DoubleTapCell.this.circleProgress[i3]);
                        float measuredWidth = ((i == 0 ? 1 : 3) * getMeasuredWidth()) / 4.0f;
                        float measuredHeight = getMeasuredHeight() / 4.0f;
                        if (i != 0) {
                            f = -2.0f;
                        }
                        canvas.drawCircle(measuredWidth, measuredHeight + AndroidUtilities.dpf2(f), AndroidUtilities.m1036dp(25 - (i2 * 6)) * DoubleTapCell.this.circleSizeProgress[i3], DoubleTapCell.this.circleOutlinePaint[i2]);
                        i2++;
                    }
                    ...
                    drawable.setBounds(drawable.getBounds().left - AndroidUtilities.m1036dp(4.0f - (DoubleTapCell.this.iconChangingProgress[i] * 4.0f)), drawable.getBounds().top - AndroidUtilities.m1036dp(4.0f - (DoubleTapCell.this.iconChangingProgress[i] * 4.0f)), drawable.getBounds().right + AndroidUtilities.m1036dp(4.0f - (DoubleTapCell.this.iconChangingProgress[i] * 4.0f)), drawable.getBounds().bottom + AndroidUtilities.m1036dp(4.0f - (DoubleTapCell.this.iconChangingProgress[i] * 4.0f)));
    ...
    public void onMeasure(int i, int i2) {
        super.onMeasure(View.MeasureSpec.makeMeasureSpec(View.MeasureSpec.getSize(i), TLObject.FLAG_30), View.MeasureSpec.makeMeasureSpec(AndroidUtilities.m1036dp(170.0f), TLObject.FLAG_30));
    }
