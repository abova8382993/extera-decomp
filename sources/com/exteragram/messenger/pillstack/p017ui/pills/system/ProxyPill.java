...
public class ProxyPill extends BasePill implements NotificationCenter.NotificationCenterDelegate {
    ...
    public ProxyPill(Context context, Theme.ResourcesProvider resourcesProvider) {
        ...
        linearLayout.setPadding(AndroidUtilities.m1036dp(8.0f), 0, AndroidUtilities.m1036dp(10.0f), 0);
    ...
    public void onUpdateData(boolean z) {
        ...
        if (!zIsProxyEnabled || (proxyInfo = SharedConfig.currentProxy) == null) {
            this.iconView.setImageResource(C2797R.drawable.drawer_proxy_off);
            string = LocaleController.getString(C2797R.string.Proxy);
            stopLoading();
        } else if (z2) {
            long jClamp = Utilities.clamp(proxyInfo.ping, 9999L, 0L);
            this.iconView.setImageResource(C2797R.drawable.drawer_proxy_on);
            if (jClamp > 0) {
                string = LocaleController.formatString(C2797R.string.NavigationDrawerProxyPingShort, Long.valueOf(jClamp));
            } else {
                string = LocaleController.getString(C2797R.string.MenuProxyConnected);
            }
            stopLoading();
        } else {
            this.iconView.setImageResource(C2797R.drawable.drawer_proxy_off);
            string = LocaleController.getString(C2797R.string.MenuProxyConnecting);
            startLoading();
        }
    ...
    public boolean onPillLongClicked() {
        ...
        ItemOptions.makeOptions(safeLastFragment, this).add(C2797R.drawable.msg_settings, LocaleController.getString(C2797R.string.Settings), new Runnable() { // from class: com.exteragram.messenger.pillstack.ui.pills.system.ProxyPill$$ExternalSyntheticLambda0
