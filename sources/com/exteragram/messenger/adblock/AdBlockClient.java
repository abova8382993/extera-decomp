...
public abstract class AdBlockClient {
    public static CosmeticHide getCosmeticHide(String str) {
        ...
        if (cosmeticResources == null) {
            FileLog.m1046e("err cosmetic: " + str);
            return null;
        }
        FileLog.m1045d("hideSelectors: " + Arrays.toString(cosmeticResources.getHideSelectors()));
        FileLog.m1045d("proceduralActions: " + Arrays.toString(cosmeticResources.getProceduralActions()));
        FileLog.m1045d("exceptions: " + Arrays.toString(cosmeticResources.getExceptions()));
        FileLog.m1045d("injectedScript: " + cosmeticResources.getInjectedScript() + " genericHide: " + cosmeticResources.isGenericHide());
    ...
    private static String createHideScript(String[] strArr) {
        ...
        return "(function() {var parent = document.getElementsByTagName('head').item(0);var style = document.createElement('style');style.type = 'text/css';style.innerHTML = window.atob('" + Base64.getEncoder().encodeToString((AdBlockClient$$ExternalSyntheticBackport0.m243m(",", strArr) + "{display: none !important;}").getBytes()) + "');parent.appendChild(style)})()";
    }
